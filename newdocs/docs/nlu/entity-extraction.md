...
**NOTE**: The `confidence` will be set by the `CRFEntityExtractor` and the `DIETClassifier` component. The
...
**NOTE**: Composite entities are currently only supported by the DIETClassifier and CRFEntityExtractor.
