...
what pipeline is best for your use case, check out [Choosing a Pipeline](/docs/nlu/choosing-a-pipeline).
...
via the instructions [here](/docs/nlu/choosing-a-pipeline#section-supervised-embeddings-pipeline).

After you define the `supervised_embeddings` processing pipeline and generate some [NLU training data](/docs/nlu/training-data-format)
...
<a aria-hidden="true" tabindex="-1" class="anchor enhancedAnchor" id="pretrained-word-vectors"></a>
...
With the `pretrained_embeddings_spacy` [pipeline](/docs/nlu/choosing-a-pipeline#section-pretrained-embeddings-spacy-pipeline), you can use spaCy’s
...
<a aria-hidden="true" tabindex="-1" class="anchor enhancedAnchor" id="mitie"></a>

### MITIE

You can also pre-train your own word vectors from a language corpus using [MITIE](/docs/nlu/choosing-a-pipeline#section-mitie-pipeline). To do so:
...
3. Set the path of your new `total_word_feature_extractor.dat` as the `model` parameter in your
   [configuration](/docs/nlu/choosing-a-pipeline#section-mitie-pipeline).
