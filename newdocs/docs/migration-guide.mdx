...
* The [TED Policy](./core/policies#ted-policy) replaced the `keras_policy` as recommended machine
  ...
  [TED Policy](./core/policies#ted-policy) add this to the `policies` section in your `config.yml`
  ...
  Please see the docs of the [TED Policy](./core/policies#ted-policy) if you want to customize them.
...
* All pre-defined pipeline templates are deprecated. **Any templates you use will be
  ...
  Take a look at [Choosing a Pipeline](./nlu/choosing-a-pipeline) to decide on what components you should use
  in your configuration file.

* The [Embedding Policy](./core/policies#embedding-policy) was renamed to [TED Policy](./core/policies#ted-policy). The functionality of the policy stayed the same.
...
* The Embedding Intent Classifier is now deprecated and will be replaced by [DIETClassifier](./nlu/components#diet-classifier)
  in the future.
  ...
  See [DIETClassifier](./nlu/components#diet-classifier) for more information about the new component.
...
* `CRFEntityExtractor` is now deprecated and will be replaced by `DIETClassifier` in the future. If you want to
  ...
  We extracted the featurization from the component into the new featurizer [LexicalSyntacticFeaturizer](./nlu/components#lexicalsyntacticfeaturizer). Thus,
  in order to obtain the same results as before, you need to add this featurizer to your pipeline before the
  [DIETClassifier](./nlu/components#diet-classifier).
...
* If your pipeline contains `CRFEntityExtractor` and `EmbeddingIntentClassifier` you can substitute both
  components with [DIETClassifier](./nlu/components#diet-classifier). You can use the following pipeline for that:
...
* By default, the `EmbeddingIntentClassifier`, `EmbeddingPolicy`, and `ResponseSelector` will
  ...
  default since 1.3, see [Rasa 1.2 to Rasa 1.3](./migration-guide#migration-to-rasa-1-3)). This is configurable via the `ranking_length`
...
* Default parameters of `EmbeddingIntentClassifier` are changed. See [Components](./nlu/components) for details.
  Architecture implementation is changed as well, so **old trained models cannot be loaded**.
  Default parameters and architecture for `EmbeddingPolicy` are changed. See [Policies](./core/policies) for details.
...
* Default `max_history` for `EmbeddingPolicy` is `None` which means it’ll use
  ...
  for **faster training**. See [Featurization of Conversations](./api/core-featurization#featurization-conversations) for details.
...
* The scripts in `rasa.core` and `rasa.nlu` can no longer be executed. To train, test, run, … an NLU or Core
  ...
  should use instead. See all the new commands at [Command Line Interface](./user-guide/command-line-interface).
...
* All script parameter names have been unified to follow the same schema.
  ...
  in the [Command Line Interface](./user-guide/command-line-interface).
