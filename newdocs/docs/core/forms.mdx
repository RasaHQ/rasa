...
As shown in the section [Handling unhappy paths](/docs/core/forms#section-unhappy) the bot can execute any kind of
...
Note that for this story to work, your slots should be [unfeaturized](/docs/core/slots#unfeaturized-slot). If any of these slots are featurized, your story needs to
include `slot{}` events to show these slots being set. In that case, the
easiest way to create valid stories is to use [Interactive Learning](/docs/core/interactive-learning).
...
[slot mappings](/docs/core/forms#form-slot-mappings) in order to gather all of the information you need.
Other methods like [validation methods](/docs/core/forms#form-validation) are optional.

Here is an example of what it looks like:

<a aria-hidden="true" tabIndex="-1" className="anchor enhancedAnchor" id="formbot-code"></a>

```python (docs/sources/examples/formbot/actions.py)
```
...
See [Handling unhappy paths](/docs/core/forms#section-unhappy) for how to work with unexpected user input.
...
In the [above example](/docs/core/forms#formbot-code), you can see how the form uses different
...
[formbot](/docs/core/forms#formbot-code) checks if the extracted cuisine slot belongs
...
Please read [Interactive Learning with Forms](/docs/core/interactive-learning#section-interactive-learning-forms)
...
<a aria-hidden="true" tabIndex="-1" className="anchor enhancedAnchor" id="conditional-logic"></a>
...
The first thing to try is to run your bot with the `--debug` flag, see [Command Line Interface](/docs/user-guide/command-line-interface) for details.
