...
Retrieval actions are designed to make it simpler to work with [Small Talk](/docs/dialogue-elements/small-talk) and [Simple Questions](/docs/dialogue-elements/completing-tasks#simple-questions) .
...
A retrieval action uses the output of a [ResponseSelector](/docs/nlu/components#response-selector) component from NLU which learns a
retrieval model to predict the correct response from a list of candidate responses given a user message text.

<a aria-hidden="true" tabindex="-1" class="anchor enhancedAnchor" id="retrieval-training-data"></a>
...
You need to include the [ResponseSelector](/docs/nlu/components#response-selector) component in your config. The component needs a tokenizer, a featurizer and an
...
intent is to use the [Mapping Policy](/docs/core/policies#mapping-policy).
...
(see [Unhappy Paths](/docs/dialogue-elements/completing-tasks#unhappy-paths) )
