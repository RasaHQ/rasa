...
want to deploy your assistant to users, see [Deploying Your Rasa Assistant](/docs/user-guide/how-to-deploy#deploying-your-rasa-assistant).
...
All the endpoints this API exposes are documented in [HTTP API](/docs/api/http-api).
...
1. Fetch the model from a server (see [Fetching Models from a Server](/docs/user-guide/configuring-http-api#server-fetch-from-server)), or

2. Fetch the model from a remote storage (see [Cloud Storage](/docs/user-guide/cloud-storage)).
...
by enabling an authentication method: [Security Considerations](/docs/user-guide/configuring-http-api#server-security).

:::

:::note
If you are using custom actions, make sure your action server is
running (see [Start an Action Server](/docs/user-guide/command-line-interface#run-action-server)). If your actions are running
...
`RedisLockStore` (see [Lock Stores](/docs/api/lock-stores)).

:::

<a aria-hidden="true" tabindex="-1" class="anchor enhancedAnchor" id="server-fetch-from-server"></a>
...
<a aria-hidden="true" tabindex="-1" class="anchor enhancedAnchor" id="server-fetch-from-remote-storage"></a>
...
For more information see [Cloud Storage](/docs/user-guide/cloud-storage).

<a aria-hidden="true" tabindex="-1" class="anchor enhancedAnchor" id="server-ssl"></a>
...
<a aria-hidden="true" tabindex="-1" class="anchor enhancedAnchor" id="server-security"></a>
