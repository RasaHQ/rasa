...
want to deploy your assistant to users, see [Deploying Your Rasa Assistant](./how-to-deploy#deploying-your-rasa-assistant).
...
1. Fetch the model from a server (see [Fetching Models from a Server](./configuring-http-api#server-fetch-from-server)), or

2. Fetch the model from a remote storage (see [Cloud Storage](./cloud-storage)).
...
running (see [Start an Action Server](./command-line-interface#run-action-server)). If your actions are running
...
For more information see [Cloud Storage](./cloud-storage).
