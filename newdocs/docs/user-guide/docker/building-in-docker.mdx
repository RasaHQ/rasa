...
[Deploying Your Rasa Assistant](/docs/user-guide/how-to-deploy#deploying-your-rasa-assistant) to learn how to deploy your model.
...
Just like in the [tutorial](/docs/user-guide/rasa-tutorial), you’ll use the `rasa init` command to create a project.
...
```python
docker run -v $(pwd):/app rasa/rasa:1.11.0a3-full init --no-prompt
```
...
* `rasa/rasa` is the name of the docker image to run. ‘1.11.0a3-full’ is the name of the tag,
...
```python
docker run -it -v $(pwd):/app rasa/rasa:1.11.0a3-full shell
```
...
<a aria-hidden="true" tabindex="-1" class="anchor enhancedAnchor" id="model-training-docker"></a>
...
```python
docker run -v $(pwd):/app rasa/rasa:1.11.0a3-full train --domain domain.yml --data data --out models
```
...
* rasa/rasa:1.11.0a3-full: Use the Rasa image with the tag ‘1.11.0a3-full’

* `train`: Execute the `rasa train` command within the container. For more
  information see [Command Line Interface](/docs/user-guide/command-line-interface).
...
<a aria-hidden="true" tabindex="-1" class="anchor enhancedAnchor" id="choosing-a-tag"></a>

### Choosing a Tag

All `rasa/rasa` image tags start with a version number. The current version is 1.11.0a3. The tags are:
...
with different dependencies installed. See [Choosing a Pipeline](/docs/nlu/choosing-a-pipeline) for more dependency information
...
To create more sophisticated assistants, you will want to use [Custom Actions](/docs/core/actions#custom-actions).
...
```python
docker run -v $(pwd):/app rasa/rasa:1.11.0a3-full train
```
...
```python
docker run -d -v $(pwd)/actions:/app/actions --net my-project --name action-server rasa/rasa-sdk:1.10.2
```
...
* rasa/rasa-sdk:1.10.2 : Uses the Rasa SDK image with the tag 1.10.2
...
```python
docker run -it -v $(pwd):/app -p 5005:5005 --net my-project rasa/rasa:1.11.0a3-full shell
```
...
model you created with Rasa X via one of our [recommended deployment methods](/docs/user-guide/how-to-deploy#recommended-deployment-methods).
Or, you can do a [Rasa-only deployment in Docker Compose](/docs/user-guide/docker/deploying-in-docker-compose#deploying-rasa-in-docker-compose).
