...
also known as a message broker, to forward Rasa [Events](/docs/api/events) from the Rasa server to other services.
...
types, check out the [Events](/docs/api/events) docs).

<a aria-hidden="true" tabIndex="-1" className="anchor enhancedAnchor" id="event-brokers-pika"></a>
...
```yml (../data/test_endpoints/event_brokers/pika_endpoint.yml)
```
...
```yml (../data/test_endpoints/event_brokers/kafka_plaintext_endpoint.yml)
```

If using SSL protocol, the endpoints file should look like:

```yml (../data/test_endpoints/event_brokers/kafka_ssl_endpoint.yml)
```
