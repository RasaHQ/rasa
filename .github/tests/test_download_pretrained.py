...
def test_download_pretrained_lmf_exists_no_params():
    lmf_specs = download_pretrained.get_model_weights_from_config(CONFIG_FPATH)
...
def test_download_pretrained_lmf_exists_with_model_name():
    ...
    with tempfile.NamedTemporaryFile("w+") as fp:
        yaml.dump(config, fp)
        fp.seek(0)
        lmf_specs = download_pretrained.get_model_weights_from_config(fp.name)
    assert lmf_specs[0].model_weights == DEFAULT_MODEL_WEIGHTS["roberta"]
...
def test_download_pretrained_unknown_model_name():
    ...
    with tempfile.NamedTemporaryFile("w+") as fp:
        ...
        with pytest.raises(KeyError):
            download_pretrained.get_model_weights_from_config(fp.name)
...
def test_download_pretrained_multiple_model_names():
    ...
    assert lmf_specs[1].model_weights == DEFAULT_MODEL_WEIGHTS["roberta"]


def test_download_pretrained_with_nondefault_weight():
    yaml = YAML(typ="safe")
    config = yaml.load(CONFIG_FPATH)
