...
class CRFRoleExtractor(EntityExtractor):
    ...
    def __init__(
        ...
        super(CRFRoleExtractor, self).__init__(component_config)
    ...
    def train(
        ...
        if training_data.entity_examples:
            ...
            # do not filter out pre-trained entities since we can still train a role for them
            # convert entity examples to role examples
            role_examples = self.create_role_examples(training_data.entity_examples)

            # convert the dataset into features
            # this will train on ALL examples, even the ones
            # without annotations
            dataset = self._create_dataset(role_examples)
    ...
    def process(self, message: Message, **kwargs: Any) -> None:
        ...
        message.set(
            "roles", message.get("roles", []) + extracted, add_to_output=True
        )
    ...
    def extract_roles(self, message: Message) -> List[Dict[Text, Any]]:
        ...
        if self.ent_tagger is not None:            
            ...
            with_roles = self.add_roles_to_entities(role_message, message)
            return with_roles.get("entities", [])
        else:
            return []
    ...
    def load(
        ...
        cached_component: Optional["CRFRoleExtractor"] = None,
        **kwargs: Any
    ) -> "CRFRoleExtractor":
