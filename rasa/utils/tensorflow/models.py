...
class RasaModel(tf.keras.models.Model):
    ...
    def fit(
        ...
        logger.debug(
            "NOTE: just before it would usually say `Building tensorflow train graph`"
        )
        (
    ...
    def _get_tf_call_model_function(
        ...
        if eager:
            logger.debug("NOTE: eager, not building tf graph")
    ...
    def _get_tf_train_functions(
        ...
        logger.debug("NOTE: _get_tf_train_functions")

        def train_dataset_function(_batch_size: int) -> tf.data.Dataset:
            logger.debug("NOTE: train_dataset_function")
