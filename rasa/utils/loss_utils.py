...
def log_t(u, t):
    ...
    return tf.cond(
        tf.equal(t, 1.0), lambda: tf.log(u), functools.partial(_internal_log_t, u, t)
    )
...
def exp_t(u, t):
    ...
    return tf.cond(
        tf.equal(t, 1.0), lambda: tf.exp(u), functools.partial(_internal_exp_t, u, t)
    )
...
def compute_normalization_binary_search(activations, t, num_iters=10):
  ...
  """
    ...
    _, lower, upper = tf.while_loop(
        iter_condition, iter_body, [0, lower, upper], maximum_iterations=num_iters
    )
    logt_partition = (upper + lower) / 2.0
  ...
  """
    if t2 == 1.0:
        ...
        if t1 == 1.0:
            return normalization_constants + tf.reduce_sum(
                tf.multiply(labels, tf.log(labels + 1e-10) - activations),
                -1,
                keep_dims=True,
            )
        else:
    ...
    if t1 == 1.0:
        return tf.reduce_sum(
            tf.multiply(
                tf.log(labels + 1e-10)
                - tf.log(tf.pow(shifted_activations, 1.0 / one_minus_t2)),
                labels,
            ),
            -1,
            keep_dims=True,
        ...
        return tf.reduce_sum(
            ...
            - 1.0
            / beta
            * (tf.pow(labels, beta) - tf.pow(shifted_activations, beta / one_minus_t2)),
            -1,
        )
...
def tempered_sigmoid(activations, t, num_iters=5):
  ...
  """
    ...
    normalization_constants = tf.cond(
        ...
        tf.equal(t, 1.0),
        lambda: tf.log(tf.reduce_sum(tf.exp(internal_activations), -1, keep_dims=True)),
