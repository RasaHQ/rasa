...
class HangoutsOutput(OutputChannel):
                    ...
                    "sections":[
                        {
                            "widgets":[
                                {
                                "textParagraph":{"text": message["text"]}
                                }
                            ]
                        }
                    ]
    ...
    def _text_button_card(self, text: Text, buttons: List) -> Dict:
        ...
        for b in buttons:
            try: 
                ...
                logger.error("Buttons must be a list of dicts with 'title' and 'payload' as keys")
    ...
    async def _persist_message(self, message: Dict) -> None:
        ...
        if self.messages.get("text"): msg_state = "text"
        elif self.messages.get("cards"): msg_state = "cards"
        else: msg_state = None

        if message.get("text"): msg_new = "text"
        elif message.get("cards"): msg_new = "cards"
        else: raise Exception("Your message to Hangouts channel must either contain 'text' or 'cards'!")
        ...
        elif msg_new == "text": new_messages = {"text": message.get("text")}
        else: new_messages = message
    ...
    async def send_text_with_buttons(self, recipient_id: Text, text: Text, buttons: List, **kwargs) -> None:
    ...
    async def send_custom_json(self, recipient_id: Text, json_message: Dict, **kwargs) -> None:
        ...
        async def receive(request: Request) -> HTTPResponse:
            ...
            try:
                await on_new_message(
                    UserMessage(
                        text, collector, sender_id, input_channel=input_channel, metadata={"room": room_name}
                    )
                )
