...
class RasaChatInput(RestInput):
    ...
    async def _extract_rasax_username(self, req: Request) -> Optional[Text]:
        """Fetch username from the Rasa X Admin API."""
        jwt_payload = None
        if req.headers.get("Authorization"):
            jwt_payload = await self._decode_bearer_token(req.headers["Authorization"])
    ...
    def blueprint(
        ...
        async def receive(request: Request) -> HTTPResponse:
            ...
            rasa_x_username = await self._extract_rasax_username(request)
            ...
            metadata["rasa_x_username"] = rasa_x_username 
