...
import rasa.shared.nlu.constants as nlu_constants
...
class Domain:
    ...
    def get_active_states(self, tracker: "DialogueStateTracker") -> State:
        ...
        state = {
            core_constants.USER: self._get_user_sub_state(tracker),
            core_constants.SLOTS: self._get_slots_sub_state(tracker),
            core_constants.PREVIOUS_ACTION: self._get_prev_action_sub_state(tracker),
            core_constants.ACTIVE_LOOP: self._get_active_loop_sub_state(tracker),
        }
    ...
    def _get_user_sub_state(
        ...
        if entities:
            sub_state[nlu_constants.ENTITIES] = entities
        else:
            sub_state.pop(nlu_constants.ENTITIES, None)
    ...
    def _get_slots_sub_state(
        ...
        for slot_name, slot in tracker.slots.items():
            if slot is not None and slot.as_feature():
                if slot.value == core_constants.SHOULD_NOT_BE_SET:
                    slots[slot_name] = core_constants.SHOULD_NOT_BE_SET
    ...
    def _get_active_loop_sub_state(
        ...
        active_loop = tracker.active_loop.get(core_constants.LOOP_NAME)
        if active_loop:
            return {core_constants.LOOP_NAME: active_loop}
        else:
            return {}

    def _get_featurized_entities(self, latest_message: events.UserUttered) -> Set[Text]:
        intent_name = latest_message.intent.get(nlu_constants.INTENT_NAME_KEY)
    ...
    def add_categorical_slot_default_value(self) -> None:
        ...
        for featurization.
        """
        for slot in [s for s in self.slots if type(s) is slots.CategoricalSlot]:
    ...
    def add_knowledge_base_slots(self) -> None:
        ...
        base slots.
        """
        if core_constants.DEFAULT_KNOWLEDGE_BASE_ACTION in self.action_names:
            logger.warning(
                "You are using an experiential feature: Action '{}'!".format(
                    core_constants.DEFAULT_KNOWLEDGE_BASE_ACTION
                )
            )
            slot_names = [s.name for s in self.slots]
            knowledge_base_slots = [
                core_constants.SLOT_LISTED_ITEMS,
                core_constants.SLOT_LAST_OBJECT,
                core_constants.SLOT_LAST_OBJECT_TYPE,
            ]
            for s in knowledge_base_slots:
                if s not in slot_names:
                    self.slots.append(slots.UnfeaturizedSlot(s))
    ...
    def persist_specification(self, model_path: Text) -> None:
        ...
        io_utils.create_directory_for_file(domain_spec_path)

        metadata = {"states": self.input_states}
        io_utils.dump_obj_as_json_to_file(domain_spec_path, metadata)
    ...
    def load_specification(cls, path: Text) -> Dict[Text, Any]:
        ...
        specification = json.loads(io_utils.read_file(metadata_path))
    ...
    def as_yaml(self, clean_before_dump: bool = False) -> Text:
        ...
        return io_utils.dump_obj_as_yaml_to_string(domain_data)
        ...
        if missing_templates:
            for template in missing_templates:
                io_utils.raise_warning(
                    ...
                    f"check your domain.",
                    docs=constants.DOCS_URL_DOMAINS + "#responses",
                )
