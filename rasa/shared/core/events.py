...
class SessionPaused(SkipEventInMDStoryMixin):
    """Indicates that the session is paused.

    This event is triggered through Rasa API to indicate that the session
    should be paused.
    Subsequently, this event can be used to trigger custom logic
    in event processing pipelines connected to event brokers.
    """

    type_name = "session_paused"
    ...
    def __init__(
        ...
        """Creates event to indicate that session is paused.

        Args:
            reason: Reason for pausing the session.
            timestamp: When the event was created.
        """
        self.reason = reason
    ...
    def __repr__(self) -> Text:
        """Returns event as string for debugging."""
        return f"{SessionPaused.__name__}(reason: {self.reason})"
    ...
    def __eq__(self, other: Any) -> bool:
        """Compares object with other object."""
        if not isinstance(other, SessionPaused):
            return NotImplemented

        return self.reason == other.reason and self.timestamp == other.timestamp

    def apply_to(self, tracker: "DialogueStateTracker") -> None:
        """This event does not modify the tracker state."""
        pass

    def as_dict(self) -> Dict[str, Any]:
        """Returns serialized event."""
        serialized = super().as_dict()
        serialized.update({"reason": self.reason})
        return serialized

    @classmethod
    def _from_parameters(cls, parameters: Dict[str, Any]) -> SessionPaused:
