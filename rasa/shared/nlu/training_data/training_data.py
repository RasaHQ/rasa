...
class TrainingData:
    ...
    def split_nlu_examples(
        ...
        num_classes = len(self.number_of_examples_per_intent.items()) - len(self.retrieval_intents) + len(self.number_of_examples_per_response)
        ...
        def _split_class(
            ...
            exact_train_count = int((_running_count + len(_examples)) * train_frac) - _running_train_count
            ...
            return (_running_count + len(_examples), _running_train_count + approx_train_count)
        ...
        for response, _ in sorted(self.number_of_examples_per_response.items(), key=operator.itemgetter(1)):
            examples = [
                e
                for e in training_examples
                if e.get(INTENT_RESPONSE_KEY) and e.get(INTENT_RESPONSE_KEY) == response
            ]
            running_count, running_train_count = _split_class(examples, running_count, running_train_count)
            training_examples = training_examples - set(examples)

        # Again for intents:
        for intent, _ in sorted(self.number_of_examples_per_intent.items(), key=operator.itemgetter(1)):
            examples = [
                e
                for e in training_examples
