...
async def train_in_chunks_async(
...
async def _do_training_in_chunks(
    file_importer: TrainingDataImporter,
    output_path: Text,
    train_path: Text,
    ...
    if fingerprint_comparison_result.should_retrain_core():
        await _train_core_in_chunks_with_validated_data(
            file_importer,
            output=output_path,
            train_path=train_path,
            fixed_model_name=fixed_model_name,
...
async def _train_nlu_in_chunks_with_validated_data(
    file_importer: TrainingDataImporter,
    output: Text,
    train_path: Optional[Text] = None,
    fixed_model_name: Optional[Text] = None,
