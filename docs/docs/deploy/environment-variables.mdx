...
When running Rasa Pro, the `RASA_PRO_LICENSE` environment variable needs to contain
...
| `RASA_ENVIRONMENT`                | The Rasa environment for the [`PikaEventBroker`](../production/event-brokers.mdx#pika-event-broker) and the [`KafkaEventBroker`](../production/event-brokers.mdx#kafka-event-broker).                                 |                   |
