...
Before you define a form, you need to add the [Rule Policy](./policies.mdx#rule-policy)
...
To define a form, add it to the `forms` section in your [Domain](./domain.mdx).
For example:
...
The top-level key underneath `forms` is the name of your form, in this case `birthday_form`.

For each [slot](./domain.mdx#slots) you want your form to fill, list the name of slot
and specify one or more [slot mappings](./forms.mdx#slot-mappings) for that slot.
The example above fills only one slot, `age`, from an extracted entity `age`.

Your assistant will use responses you define to ask the user for information. 
For each slot in your form, write a [response](./responses.mdx) with a name
that follows one of the formats below: 
...
```yaml-rasa
responses:
  utter_ask_birthday_form_age:
    - text: "How old are you turning?"
```
