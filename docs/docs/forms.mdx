...
For example, if you do not want any of the slots of the specific form to be filled with
intent `chitchat` then you would need to define the following (after the form_name and
before the definition of the slot mappings):

```yaml-rasa
forms:
  restaurant_form:
    global_not_intent: chitchat
    required_slots:
        cuisine:
          - type: from_entity
            entity: cuisine
