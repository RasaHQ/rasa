...
There is also an optional parameter that can be defined in each form which is called
`global_not_intent` parameter. It can be added under the form_name and its value will
affect all slots that are present in that form. More specifically, it will update/add
the `not_intent` key of all individual slots with the specified value (or list of values)
found in the global parameter.

For example, if you want to fill the `not_intent` key of all slots with `intent` then
you would need to define the following after the form_name and before the definition
of the slot mappings:
    ...
    required_slots:
        ...
        num_people:
          - type: from_entity
            entity: number
```
Note that if you decide to use the `global_not_intent` parameter then the keyword
`required_slots` should precede the definition of slot mappings as shown in the above
example. This global parameter is very convenient in case you want to fill the `not_intent`
key of all slots of a form with the same value.
...
The `not_intent` key of individual slots can also be defined with the help of `global_not_intent` parameter as shown in the docs on [Session Configuration (Domain)](./domain#session-configuration)
