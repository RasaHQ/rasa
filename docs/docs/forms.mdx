...
1. Define a method `extract_<slot_name>` for every custom slot which you want to
   extract.
2. Optional: If a custom slot is not part of your mapped slots in the domain,
   override `required_slots` to include the additional custom slot.

The following example shows the implementation of a form which extracts a custom slot
