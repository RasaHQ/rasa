...
1. Define a method `extract_<slot_name>` for every custom slot mapping which you want to
   extract.
2. Optional: If a slot is not part of your mapped slots in the domain,
   override `required_slots` to make sure the additional slot is considered by the form.

The following example shows the implementation of a form which extracts a slot
`outdoor_seating` in addition to the slots which were mapped in the domain.
...
in the domain, which is not filled yet. If you use
...
override `required_slots` to fit your dynamic behavior. Please remember to implement
a method `extract_<slot name>` for every slot which isn't defined in the slot mappings
of your domain as it is described [here](forms.mdx#custom-slot-mappings).
