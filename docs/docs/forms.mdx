...
1. Define a method `extract_<slot_name>` for every slot that should be mapped in a
   custom way.
2. Make sure that in the domain file you list for your form only those slots that don't
   use custom mappings.
...
`outdoor_seating` in a custom way, in addition to the slots whose mappings are defined
in the domain.
