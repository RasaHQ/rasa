...
stray from the happy path. The way this works with forms is that a form will
automatically raise an `ActionExecutionRejection` if
...
In addition you can also return an `ActionExecutionRejected` event as part of your
custom validations or slot mappings to reject the form execution.

You need to handle actions that might cause `ActionExecutionRejection` errors
