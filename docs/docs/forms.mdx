...
`FormValidationAction`. When using the `FormValidationAction`, two things are needed to
extract customs slots:

1. Override `required_slots`. It should return a list of slot names which your form
   requires to be filled.
2. Define a method `extract_<slot_name>` for every custom slot which you want to
   extract.

The following example shows the implementation of a custom slot mapping which sets
the slot `outdoor_seating` based on whether the keyword `outdoor` was present in the
last user message.
...
### Changing the Next Slot to Request

By default Rasa Open Source will ask for the next
[`requested_slot`](forms.mdx#the-requested_slot-slot) defined in the slot mappings
in our domain, which is not filled. If you use
[custom slot mappings](forms.mdx#custom-slot-mappings) and the `FormValidationAction`,
it will ask for the first empty slot returned by your `required_slots` method. If all
slots in `required_slots` are filled, it will check if all domain slot mappings are
filled as well and then notify the form that it can be deactivated.

You can change the way the next slot is requested by overriding `next_requested_slot`.
Return `SlotSet(requested_slot", "my_slot")` to ask the user to fill slot `my_slot`
next. Return `SlotSet("requested_slot": None)` to notify Rasa Open Source that all
required slots are filled and that the form should be deactivated.
...
class ValidateRestaurantForm(FormValidationAction):
    ...
    async def next_requested_slot(
        ...
        for slot_name in await self.required_slots(dispatcher, tracker, domain):
...
a previous slot was filled.

If you are using the Rasa SDK, we recommend you to use the `FormValidationAction` and
override `required_slots` to fit your dynamic behavior.
The example below will ask the user if they want to sit in
the shade or in the sun in case they told us that they want to sit outside.
...
class ValidateRestaurantForm(FormValidationAction):
    ...
    async def required_slots(
        ...
        required_slots = ["outdoor_seating"]
        if tracker.slots.get("outdoor_seating") is True:
            ...
            required_slots.append("shade_or_sun")

        return required_slots
```

`required_slots` is called with the latest slot values during the execution of
[`next_requested_slot`](forms.mdx#changing-the-next-slot-to-request) which means that
the `FormValidation` action will take the latest state in account when deciding which
