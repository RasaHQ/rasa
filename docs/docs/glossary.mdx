...
## Retrieval Intent
  ...
  A special instance of an intent which can be divided into smaller sub-intents. Each sub-intent has a fixed response and hence the context of the conversation does not matter when a user expresses one of these sub-intents.
...
## [Story](./stories.mdx)
  ...
  A representation of a conversation between a user and a bot. A story includes the intent / entities of the users' messages as well as the sequence of actions to be performed by the bot
