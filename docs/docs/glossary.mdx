...
## [Policy](./policies.mdx)
  ...
  Rasa Open Source components that predict the dialogue system’s next action. Policies make decisions about how the conversation flow should proceed. A typical configuration includes multiple policies, and the policy with the highest confidence decides the next action to be taken in the conversation.
...
## User Goal
  ...
  The overall goal that a user wants to achieve in a conversation, e.g. looking up the answer to a question, booking an appointment, or purchasing an insurance policy.
