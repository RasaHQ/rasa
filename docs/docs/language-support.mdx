...
Rasa's ``supervised_embeddings`` pipeline can be used on training data in **any language**.
...
via the instructions [here](./tuning-your-model.mdx#supervised-embeddings).
