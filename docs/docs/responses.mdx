...
When interacting with the assistant through the shell or Rasa X, messages starting with `/` are sent straight to the
...
`entities` is a comma separated key-value pair of entity type and values to pass along with the intent.
For example, `/inform{"ORG":"Rasa", "GPE":"Germany"}` will pass the intent `inform` and the entities `Rasa`
and `Germany` which are of type `ORG` and `GPE` respectively to the `RegexInterpreter`. 
:::
