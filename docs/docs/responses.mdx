...
`RegexInterpreter`, which expects NLU input in a shortened `/intent{entities}` format, e.g.
```yaml-rasa {4-8}
payload: "/inform{{\"planet\": \"saturn\"}}"
```

In the example of this section, if the user clicks a button, the user input
