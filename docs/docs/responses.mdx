...
title: Responses
abstract: Responses are messages that your assistant can send to the user. A response
 can be as simple as pure text, or can include other content like images or buttons.
---
## Defining Responses

Responses go under the `responses` key in your [Domain](./domain.mdx) file. Each
 response name needs to start with `utter_`.

Here's is a minimal example of a response called `utter_greet`:

```yaml-rasa {6-9} title="domain.yml"
version: "2.0"
...
:::note
Responses are important when your assistant uses
[Retrieval Actions](./retrieval-intents#training-data).
:::
