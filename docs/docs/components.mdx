...
* **Configuration**
  ...
  You can specify what language model to load via the parameter `model_name`. See the below table for a list of
  available popular architectures with default weights.
  ...
  pipeline:
    - name: LanguageModelFeaturizer
      # Name of the language model architecture to use (optional if `model_weights` is specified)
      model_name: "bert"
      # Pre-Trained weights to be loaded (defaults to the weights in the above table for supported `model_name`)
  ...
  Apart from the default pretrained model weights, further models can be used from the
  [HuggingFace model hub](https://huggingface.co/models) provided that the model has pretrained Tensorflow weights
  (make sure that the file `tf_model.h5` exists in the "Files and Versions" section). To use such a model, it is enough
  to specify the `model_weights` parameter as the HuggingFace model identifier. For example, the model found at
  ...
  pipeline:
    - name: LanguageModelFeaturizer
      model_weights: allenai/longformer-base-4096
  ```
  ...
  :::note
  While we make an effort to support as many HuggingFace models as possible, please note that some architectures will
  not work due to how Rasa's internal tokenization interacts with the HuggingFace tokenizers. The following models are
  currently not supported:

  BertGeneration, Bartphor, Bertweet, BlenderbotSmall, ByT5, Canine, CLIP, CTRL, LayoutLMv2, LayoutXLM, LED, MBart50,
  Perceiver, Phobert, Reformer, Speech2Text, Speech2Text2, T5, Tapas, TransfoXL, Wav2Vec2CTC, Wav2Vec2, XLMProphetNet

  If you encounter any issues with other models than the ones listed above, please let us know on
  [GitHub](https://github.com/RasaHQ/rasa/issues/new/choose) or the [Rasa Community Forum](https://forum.rasa.com/).
  Typical errors would be the component failing to load, or the cleanup of subtoken delimiters not working, so that you
  would see additional characters such as `@@` or `##` in extracted entities. You can also check the
  `test_tokenizer_prefixes_cleanup` [unit test](https://github.com/RasaHQ/rasa/blob/main/tests/nlu/featurizers/test_lm_featurizer.py),
  and extend it for the model weights you want to use.
  :::
