...
:::note
  ...
  set of entity types. For example, use [Duckling](components#ducklinghttpextractor) to extract dates and times, and
  [DIETClassifier](components#dietclassifier-1) to extract person names. Otherwise, if multiple extractors
  target the same entity types, it is very likely that entities will be extracted multiple times.

  For example, if you use two or more general purpose extractors like [MitieEntityExtractor](components#mitieentityextractor),
  [DIETClassifier](components#dietclassifier-1), or [CRFEntityExtractor](components#crfentityextractor),
  the entity types in your training data will be found and
  extracted by all of them. If the [slots](domain#slots) you are filling with your entity types are of type text,
  ...
  [custom component](components#custom-components) that resolves conflicts in entity
...
* **Description**
  ...
  :::note
  When you use this extractor in combination with [MitieEntityExtractor](components#mitieentityextractor),
  [CRFEntityExtractor](components#crfentityextractor), or [DIETClassifier](components#dietclassifier-1) it can
  ...
  [entity extractor section](components#entity-extractors) for more info on multiple extraction.

  In the case where you seem to need both this extractor and another of the aforementioned statistical extractors,
  we advise you to instead add the [RegexFeaturizer](components#regexfeaturizer)
  before the extractors in your pipeline and annotate all your entity examples in the training data.
  This way, your statistical extractors will receive additional signal about the presence of regex matches
  and will be able to determine when to rely on these matches and when not to.
  :::
