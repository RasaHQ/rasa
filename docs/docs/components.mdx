...
* **Configuration**
  ...
  * `model_confidence`:
    ...
    2. `inner`: Confidences are in the range `[0, 1]`. Computed similarities are normalized with a linear function.
    It is recommended to try using `inner` as the value for `model_confidence` as it should be easier to tune thresholds for fallback then.
...
|                                 |                  | 2. `inner` - Dot product similarity between input and intent |
|                                 |                  | embeddings. Confidence for each intent will be in the range  |
|                                 |                  | `[0,1]`                                                      |
...
|                                 |                   | 2. `inner` - Dot product similarity between input and        |
|                                 |                   | response label embeddings. Confidence for each label is in   |
|                                 |                   | the range `[0, 1]`.                                          |
+---------------------------------+-------------------+--------------------------------------------------------------+
```
