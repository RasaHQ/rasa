...
Installations on Apple Silicon have some 
[limitations listed further down](#m1--m2-apple-silicon-limitations).
...
Rasa installations on Apple Silicon use Apple Metal leading to reduced training times
for local development.

Currently, not all of Rasa's dependencies support Apple Silicon natively. This leads
to the follow restrictions

* Duckling can currently not be run as a docker container on Apple Silicon. If you want
...
* Rasa can not be run in a Docker container on Apple Silicon, only native installations
  ...
  supported as an opperating system running on aarch64. We expect a future
