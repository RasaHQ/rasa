...
poetry install
```
## Upgrading Rasa Packages

Upgrade an already installed Rasa packages to the latest version from PyPI:

```bash
pip install --upgrade rasa
```
To Download a specific version, use required PyPI package version number:
