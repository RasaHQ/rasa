...
Rasa installations on Apple Silicon *don't* use [Apple Metal](https://developer.apple.com/metal/) by default.
We found that using the GPU on Apple Silicon increased training time for the
[`DIETClassifier`](components#DIETClassifier) and [`TEDPolicy`](components#TEDPolicy) dramatically.
