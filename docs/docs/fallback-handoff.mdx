...
### 4. Defining an ultimate fallback action (optional)
...
For example, the following snippet is an implementation of a custom action which does the same as `action_default_fallback` but 
dispatches a different template `utter_fallback_template`:
