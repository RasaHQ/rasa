...
[from real conversations](conversation-driven-development.mdx). 
...
To use the FallbackClassifier, add it to your NLU pipeline in config.yml.
...
To give the bot a chance to figure out what the user wants, you will usuallly want it to attempt to disambiguate the user's message by asking
clarifying questions. The Two-Stage Fallback is made to handle low NLU confidence in multiple stages
using the following sequence:

- A user messsage is classified with low confidence

    - The user is asked to confirm the intent

- The user confirms or denies the intent
...
- The user rephrases their intent
...
- The user confirms or denies the rephrased intent
...
meant and for asking the user to rephrase. To customise the behaviour of these actions,
see the [documentation default actions](default-actions.mdx).
...
out-of-scope messages and add a form of fallback behaviour. If you want to add human 
