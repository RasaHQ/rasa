...
Rasa has capability to send compressed body in HTTP requests for custom actions.
By default, this option is off to keep backward compatibility with older versions of
custom action servers which did not receive compressed body in HTTP request for custom actions.
To enable this option set environment variable COMPRESS_ACTION_SERVER_REQUEST to True.

Rasa SDK 3.2.2, 3.3.1, 3.4.1 and onwards,
support both compressed and non-compressed body in HTTP request for
