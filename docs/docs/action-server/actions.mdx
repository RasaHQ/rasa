...
You can control weather an action should receive a domain or not.
For more see [domain](#domain) subsection.
...
# endpoints.yml
action_endpoint:
 url: "http://localhost:5055/webhook" #URL to your action server1
...
To specify if an action needs a domain add `{send_domain: true}` to custom action in the list
