...
Visit [selective-domain](../building-assistants/domain.mdx#select-which-actions-should-receive-domain)
...
[documentation on rich responses](../building-assistants/responses.mdx#rich-responses).
...
There are special action types that are automatically triggered under certain circumstances, namely [default actions](../building-assistants/default-actions.mdx)
and [slot validation actions](../building-assistants/slot-validation-actions.mdx).
...
Please see the [docs on default actions](../building-assistants/default-actions.mdx) for the expected behavior of each action.
