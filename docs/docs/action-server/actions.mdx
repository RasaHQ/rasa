...
Visit [selective-domain](../concepts/domain.mdx#select-which-actions-should-receive-domain)
...
[documentation on rich responses](../concepts/responses.mdx#rich-responses).
...
There are special action types that are automatically triggered under certain circumstances, namely [default actions](../concepts/default-actions.mdx)
and [slot validation actions](../nlu-based-assistants/slot-validation-actions.mdx).
...
Please see the [docs on default actions](../concepts/default-actions.mdx) for the expected behavior of each action.
