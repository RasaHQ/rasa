...
With the use of Pluggy, you can now create additional sanic extensions by accessing the app object created by the action server. The hook implemented provides you access 
to sanic app object created by the rasa sdk when starting the action server. Using this app object, you can now extend Sanic features such as middlewares, listeners, background tasks
