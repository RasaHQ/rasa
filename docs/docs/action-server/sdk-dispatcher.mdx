...
- `json_message`: A custom json payload as a dictionary. It can be used to send [channel specific responses](../concepts/responses.mdx).
...
- `response`: The name of a response to return to the user. This response should
  be specified in your assistants [domain](../concepts/domain.mdx).
...
- `**kwargs`: arbitrary keyword arguments, which can be
  used to specify values for [variable interpolation in response variations](../concepts/responses.mdx). For example,
