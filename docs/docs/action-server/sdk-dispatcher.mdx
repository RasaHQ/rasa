...
- `json_message`: A custom json payload as a dictionary. It can be used to send [channel specific responses](../building-assistants/responses.mdx).
...
- `response`: The name of a response to return to the user. This response should
  be specified in your assistants [domain](../building-assistants/domain.mdx).
...
- `**kwargs`: arbitrary keyword arguments, which can be
  used to specify values for [variable interpolation in response variations](../building-assistants/responses.mdx). For example,
