...
[slot mapping's `conditions`](../building-assistants/domain.mdx#mapping-conditions).
...
With this option, you do not need to specify the `action` key in the [custom slot mapping](../building-assistants/domain.mdx#custom-slot-mappings),
since the default action [`action_extract_slots`](../building-assistants/default-actions.mdx#action_extract_slots) runs `action_validate_slot_mappings`
