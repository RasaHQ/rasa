...
With this option, you do not need to specify the `action` key in the [custom slot mapping](../concepts/domain.mdx#custom-slot-mappings),
since the default action [`action_extract_slots`](../nlu-based-assistants/default-actions.mdx#action_extract_slots) runs `action_validate_slot_mappings`
