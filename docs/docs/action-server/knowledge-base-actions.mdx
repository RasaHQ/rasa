...
* we will use [synonyms](../training-data-format.mdx#synonyms) extensively
...
In order to query the knowledge base to list down any kind of object, the user's request needs to include the object type.
...
attribute of interest. The user is not required to query the knowledge base to list down objects of any type prior to this.
