...
abstract: Rasa rules are a type of training data used to train your assistant's dialogue management model.
...
If a user sends a message with intent `greet` later in the conversation, the rule will not match.
