...
* [Fallback behavior](./fallback-handoff.mdx):
  In combination with the [FallbackClassifier](./components/intent-classifiers.mdx#fallbackclassifier),
...
After training, `RulePolicy` will perform a check to make sure that the rules don't
contradict the stories and each other.
For example the following two rules are contradicting:
...
:::caution
Rules are not meant to hardcode a state machine.
Rules are restricted to have only one user turn, but there can still be multiple
bot events, including e.g. a form being filled and its subsequent submission.
:::
