...
in any [channel specific response variations](../responses.mdx#channel_specific_response_variations) and the name you
...
The `blueprint` method
needs to create a [sanic blueprint](https://sanic.readthedocs.io/en/v20.3.0/sanic/blueprints.html#blueprints)
that can be attached to a sanic server. 
Your blueprint should have at least the two routes: `health` on the route `/`,
and `receive` on the route `/webhook`.

As part of your implementation of the `receive` endpoint, you will need to tell
Rasa core to handle the user message. You do this by calling
...
The `InputChannel` class provides a default implementation of the method `get_metadata`.
If you want to change the way metadata is extracted for an
existing channel, you can overwrite the `get_metadata` method. 
