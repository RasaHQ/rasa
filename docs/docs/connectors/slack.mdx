...
4. Head over to **Basic Information** to gather the **Signing Secret**.
   ...
   Copy the signing secret into your `configuration.yml` as the value for `slack_signing_secret`:
...
## Receiving Messages
Make sure you've gone through [Sending Messages](./slack.mdx#sending-messages) first
and you have a ready to go `credentials.yml`.
...
[test channels using ngrok](../messaging-and-voice-channels#testing-channels-on-your-local-machine)
...
3. As a last step, you'll need to **Subscribe to bot events** on the same page.
   ...
   (If you missed to grant all needed permissions to your app while setting up
...
Your bot is now ready to go and will receive webhook notifications about new mewssages.
...
want to use Slacks "Interactive" elements (buttons or menus), you'll need to do
