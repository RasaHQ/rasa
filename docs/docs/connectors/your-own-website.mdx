...
Metadata can be added to the message by adding an object named `metadata` to the message. The `metadata_key` setting can be used to supply metadata on another object name. For example:
