...
#### Receiving location data via twilio (whatsapp)

You can receive location data from user from this channel you need to do the following.

1. Make an intent name locationData and two entities/slots one for Latitude and one for Longitude respectively.
...
2. Once user send the location message it will Trigger the locationData intent you can later handle everything in custom actions.
...
class ActionLocationData(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        if tracker.latest_message['entities'][0]['entity'] == 'Latitude':
            Latitude = tracker.latest_message['entities'][0]['value']

        if tracker.latest_message['entities'][1]['entity'] == 'Longitude':
            Longitude = tracker.latest_message['entities'][1]['value']

        dispatcher.utter_message(text="I got Location")

        return [SlotSet("Latitude", Latitude), SlotSet("Longitude", Longitude)]
  ```