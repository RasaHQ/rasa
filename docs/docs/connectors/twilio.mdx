...
You can receive location data (WhatsApp Location) from user from this channel, you just need to do the following.

1. Make an intent name locationData and two entities and slots, one for Latitude and one for Longitude respectively.
...
2. Once user send the location message it will Trigger the locationData intent and Slots will be set from entities (Note: no need for training data for entities, its being handled through the channel).