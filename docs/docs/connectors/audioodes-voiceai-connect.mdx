...
To get your credentials, create you need to create a bot on the [VoiceAI connect portal](https://voiceaiconnect.audiocodes.io/).

1. Select **Bots** in the left side bar.
2. Click on the **+** sign to create a new bot
3. Select **Rasa** in the platforms.
4. Then set the bot URL a choose a token value.

## Setting credentials

The token value chosen above will be used in the credentials.
...
| `keep_alive`    | 120              | For each ongoing conversation, VoiceAI will periodically verifies the conversation is still active on the Rasa side.        |
...
Your bot can reply with text messages like with any other channel. The text-to-speech engine will convert it and deliver it to the user.
...
Note that only text messages are allowed. Images, attachments, and buttons cannot be used with a voice channel.
...
You can send events from Rasa to VoiceAI to make changes to the current call configuration.
For example, you might want to receive a signal when the user stays silent for more than 5 seconds, or you might need to customize how DTMF digits are sent by VoiceAI Connect.
...
There is a lot of different events which are exhaustively documented in the [VoiceAI Connect documentation](https://techdocs.audiocodes.com/voice-ai-connect/#VAIG_Combined/voiceai_connect.htm?TocPath=VoiceAI%2520Connect%257C_____0).
We will only look at an example so you understand how these work and how to use them.
...
- rule: Set pin code
  steps:
  - intent: set_pin_code # User says "I want to change my pin code"
  - action: utter_config_no_user_input # Send the noUserInput configuration event
  - action: utter_config_dtmf_pin_code # Send the DTMF format configuration event
  - action: pin_code_form # A standard Rasa form to collect the pin code from the user
  - ...
```

In the domain, we can add the `utter_config...` responses:
...
```yaml
  ...
  - custom:
      ...
      name: config
      sessionParams:
        userNoInputTimeoutMS: 5000 # If user stays silent for 5 seconds or more, the notification will be sent
        userNoInputRetries: 2 # If you want to allow for more than one notification during a call
        userNoInputSendEvent: true # Enable the noUserInput notification
```
...
```yaml
  ...
  - custom:
      ...
      name: config
      sessionParams:
        dtmfCollect: true # Enable grouped collection (i.e will send all digit at once instead of one payload per digit)
        dtmfCollectInterDigitTimeoutMS: 5000 # If more than 5 secs have passed since a digit was pressed, the input is considered completed and will be sent to the bot
        dtmfCollectMaxDigits: 6 # If 6 digits are collected, VoiceAI will send those 6 digits even if the user keeps pressing buttons
        dtmfCollectSubmitDigit: "#" # If the user presses '#' the input is considered completed and will be sent to the bot
```
...
Here's an example of how to configure the `pin_code` slot in a form:
...
Notice that `vaig_event_noUserInput` was declared in the `not_intent` field. 

Since the `vaig_event_noUserInput` intent is sent by VoiceAI Connect when the user stays silent as per our configuration,
we must deactivate the form so we can gracefully pick up the conversation from a rule or a story.
