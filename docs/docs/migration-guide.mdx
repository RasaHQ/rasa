...
* `LabelTokenizerSingleStateFeaturizer` is deprecated.
  ...
  add a `Tokenizer`  with `intent_tokenization_flag: True` and `CountVectorsFeaturizer`
...
* `BinarySingleStateFeaturizer` is deprecated and will be removed in the future.
  Please use `SingleStateFeaturizer` with nlu pipeline where `intent_tokenization_flag`
