...
It is not possible to load previously trained models if they were trained with `model_confidence=cosine` setting. Please make sure to retrain a
...
Rasa Open Source `2.3.0` introduced the option of using cosine similarities for model confidences by setting `model_confidence=cosine`. Some post-release experiments revealed that doing so can change the order of predicted labels. Since this is wrong, the option `model_confidence=cosine` has been removed and is no longer available. If the assistant was trained with that option, it should be re-trained by either removing the `model_confidence` option from the configuration or setting it to `inner`.

Setting `model_confidence=inner` will now produce confidence values in the range `[0,1]` and is helpful in tuning fallback thresholds.
...
2. `inner` - Dot product similarity between input and label embeddings. Confidence for each label will be in the range `[0,1]`.
The default value is `softmax`, but we recommend trying `inner` as it should be easier to tune thresholds for fallback with it.
