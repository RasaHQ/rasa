...
The [`ConcurrentRedisLockStore`](./lock-stores.mdx#concurrentredislockstore) is a new lock store using Redis for persistence.
This lock store can handle receiving messages in parallel for the same conversation ID by different Rasa instances.

To switch from the `RedisLockStore` to the `ConcurrentRedisLockStore`, specify the parameter (new in 3.3) `concurrent_mode` as `true` in `endpoints.yml`:
...
Note that omitting the `concurrent_mode` parameter or setting it to `false` will default to using the `RedisLockStore`.
...
### Changes to Custom NLU Components: `EntityExtractorMixin` renamed to `EntityExtractor`

As of 3.3, the `EntityExtractorMixin` has been renamed back to `EntityExtractor`.

<Tabs values={[{"label": "Rasa 3.2 (old)", "value": "old"}, {"label": "Rasa 3.3 (new)", "value": "new"}]}  groupId="3-0-nlu-entity-extractor" defaultValue="new">
  <TabItem value="old">
  ...
  class MyNLUComponent(GraphComponent, EntityExtractor):
      ...
  ```
