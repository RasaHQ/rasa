...
Rasa Open Source 3.0 changed the way how [NLU components](components) and
[policies](policies) are trained and run during inference. As part of these changes
...
Please read the updated guide on custom components [here](custom-components) before
continuing to follow the step-by-step guide to migrate your own custom components.
:::

**Type Annotations**

It is now required to use
[type annotations](https://docs.python.org/3/library/typing.html) in your custom NLU
components and policies. Rasa Open Source uses these type annotations to validate that
your components are compatible and correctly configured. As outlined in the custom
[components guide](custom-components) it is not allowed to use
...
the [model configuration](model-configuration) is passed in as a dictionary instead
...
Policies are responsible for persisting themselves. Previously the `persist` method
was called from the outside. Use the provided `model_storage` and `resource`
(see [component persistence](custom-components#model-persistence) for more details)
to persist your component at the end of the training and then return the `resource`
as result of your policy's `train` method.
...
Similar as you had to use the `model_storage` and `resource` to persist the policy
in the previous step, you need to use `model_storage` and `resource` in your policy's
`load` method to instantiate it.

:::info
It is no longer required to persist your policy's configuration yourself. The config passed
...
If you were using a custom [end-to-end policy](stories#end-to-end-training) in Rasa
Open Source 2, you had to use the `interpreter` parameter to featurize the tracker
events manually. If your component requires end-to-end features in Rasa Open Source 3
you need to [register](custom-components#registering-components-with-the-model-configuration)
your component with type `ComponentType.POLICY_WITH_END_TO_END_SUPPORT`. The features
...
data actually contains [end-to-end training data](stories#end-to-end-training).
:::
...
Before you can use your custom policy you have to register your component using the
`DefaultV1Recipe.register` decorator. If your component requires end-to-end features
specify the component type `POLICY_WITH_END_TO_END_SUPPORT`. Otherwise, use
`POLICY_WITHOUT_END_TO_END_SUPPORT`. Specify `is_trainable=True` if the `train`
method of your policy should be called during the training. If your component is only
...
Rasa Open Source allows excluding conversation parts which are completely handled by
[rules](rules). In Rasa Open Source 2 the necessary information was passed onto the
policies using the `set_shared_policy_states` method which set the policy attribute
`_rule_only_data`. Rasa Open Source passes this data in via the
`predict_action_probabilities` method. The passed `rule_only_data` can be `None`
in case the [`RulePolicy`](policies#rule-policy) is not part of your model
configuration.
...
[`GraphComponent` interface](custom-components#the-graphcomponent-interface):
- `SparseFeaturizer`
- `DenseFeaturizer`
- `IntentClassifier`
- `EntityExtractor`
...
The `EntityExtractor` class was renamed to `EntityExtractorMixing`:
...
[`GraphComponent` interface](custom-components#the-graphcomponent-interface). The
passed in configuration is your components default configuration including any updates
...
NLU components are responsible for persisting themselves.
Previously the `persist` method was called from the outside.
You should use the provided `model_storage` and `resource`
(see [component persistence](custom-components#model-persistence) for more details)
to persist your component at the end of the training and then return the `resource`
as result of your component's `train` method.
...
Similar as you had to use the `model_storage` and `resource`
(see [component persistence](custom-components#model-persistence) for more details)
to persist the NLU
component in the previous step, you need to use these now to load the persisted
component.

:::info
It is no longer required to persist your component's config yourself. The config passed
into `load` will contain the configuration which your model was trained with.
:::
...
Components like [tokenizers](components#tokenizers) or
[featurizers](components#featurizers) augment the training data with their
output during the model training. Their output is required by components later in the
pipeline. Typically, featurizers require *tokenized* messages as well as intent
classifiers require *featurized* training data to train themselves. Rasa Open Source
3.0 makes these different purposes explicit. Previously both component training and
training data augmentation were done as part of the `train` method. In Rasa Open Source
3.0 they are split in `train` and `process_training_data`:
...
messages during inference. Unless your model supports batch predictions the easiest
...
Before you can use your custom NLU component you have to register your component using the
`DefaultV1Recipe.register` decorator. The component types correspond to the existing
parent classes:
...
* If your component provides a pretrained model which then should be used by other
  components during training and inference use `ComponentType.MODEL_LOADER`

Specify `is_trainable=True` if the `train` method of your policy should be called
during the training.
...
If your component requires a pretrained model such as a [Spacy](components#spacynlp) or
[Mitie](components#mitienlp) language model, specify this with the `model_from`
