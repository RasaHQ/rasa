...
You can also use [slot validation actions](./slot-validation-actions.mdx) to either validate slots with predefined
...
For slots that should be filled only in the context of a form, add mapping conditions to specify which form(s) should
be active. Adding `conditions` is required to preserve the behavior of slot mappings from 2.0, since without them
