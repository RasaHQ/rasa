...
#### New `global_not_intent` parameter in Forms

We added a new parameter under Forms with the name `global_not_intent`. This parameter
can be used in case the user never wants to fill any slots of a form with the specified
intent, eg. chitchat. Please see https://rasa.com/docs/rasa/forms for examples and more
