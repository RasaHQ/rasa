...
Rasa Open Source 3.0 changed the way how [NLU components](components.mdx) and
...
(see [graph component persistence](custom-graph-components.mdx#model-persistence) for more details)
...
If you were using a custom [end-to-end policy](stories.mdx#end-to-end-training) in Rasa
...
you need to [register](custom-graph-components.mdx#registering-graph-components-with-the-model-configuration)
...
[rules](rules.mdx). In Rasa Open Source 2 the necessary information was passed onto the
