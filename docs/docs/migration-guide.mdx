...
action `action_extract_slots`. This action runs in the background automatically after each user turn.
Like `action_listen`, it should not be included in stories.

Each slot in the `slots` section must include the `mappings` key. The same keys used for predefined mappings in 2.0 are
available in 3.0. Additionally, you can define slots with custom mappings implemented in a custom action, for example:
...
:::note
The `required_slots` of a form has been converted in a list of slot names because slot mappings are now relocated
from `forms` to `slots` in the domain.
:::
...
The initial result of this migration could look like this:
...
In case you'd prefer certain slots to be filled only within the context of a form, you can add mapping conditions like so:
