...
[model configuration](model-configuration). As of now Rasa Open Source only supports
the `default.v1` recipe and will continue using this even if you don't specify this
in the model configuration. To avoid breaking changes in the future it is recommended
...
Please read the updated guide on custom graph components [here](custom-graph-components) before
...
[components guide](custom-graph-components) it is not allowed to use
...
(see [graph component persistence](custom-graph-components#model-persistence) for more details)
...
events manually. If your policy requires end-to-end features in Rasa Open Source 3
you need to [register](custom-graph-components#registering-components-with-the-model-configuration)
your policy with type `ComponentType.POLICY_WITH_END_TO_END_SUPPORT`. The features
...
[`GraphComponent` interface](custom-graph-components#the-graphcomponent-interface):
- `SparseFeaturizer`
- `DenseFeaturizer`
- `IntentClassifier`
- `EntityExtractor`
...
[`GraphComponent` interface](custom-graph-components#the-graphcomponent-interface). The
...
(see [component persistence](custom-graph-components#model-persistence) for more details)
...
(see [component persistence](custom-graph-components#model-persistence) for more details)
to persist the NLU
component in the previous step, you need to use these to load the persisted
NLU component.

:::info
It is no longer required to persist your NLU component's config yourself. The config passed
...
NLU Components like [tokenizers](components#tokenizers) or
...
* If your NLU component provides a pretrained model which then should be used by other
...
If your NLU component requires a pretrained model such as a [Spacy](components#spacynlp) or
