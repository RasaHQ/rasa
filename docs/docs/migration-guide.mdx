...
The [`ConcurrentRedisLockStore`](./lock-stores.mdx#concurrentredislockstore) is the new Redis implementation of the abstract interface `LockStore`.
This implementation is able to handle messages received in parallel for the same conversation id by different Rasa instances.

In order to switch from using the `RedisLockStore` to the `ConcurrentRedisLockStore`, specify the parameter (new in 3.3) `concurrent_mode` as `true` in `endpoints.yml`:
...
Note that not specifying the `concurrent_mode` parameter or setting it to `false` will default to using the `RedisLockStore` implementation.

No database migration is required when switching to the `ConcurrentRedisLockStore`, and you may use the same Redis database number.
You may want to delete all the preexisting keys if using the same Redis database number.
The reason for this is because what used to be stored by the `RedisLockStore`(i.e., serialised [TicketLock](https://rasa.com/docs/rasa/reference/rasa/core/lock/#ticketlock-objects) instances) is no longer stored by the new `ConcurrentRedisLockStore`.
The `ConcurrentRedisLockStore` instead stores individual `Ticket` instances, as well as the last issued ticket number.
The mechanism for concurrent message handling of the same conversation id recreates the `TicketLock` from the persisted `Ticket` instances.
...
This applies to the case where the model is re-trained with the new rasa open source
version without changing the configuration, random seeds, and data as well as to the
case where a model trained with a previous version of rasa open source is loaded with
