...
[NLU training data](./building-classic-assistants/nlu-training-data.mdx) in JSON format is deprecated and will be
removed in Rasa 4.0.
...
[model configuration](./building-classic-assistants/model-configuration.mdx). As of now Rasa only supports
...
Rasa 3.0 changed the way [NLU components](./building-classic-assistants/components.mdx) and
[policies](./building-classic-assistants/policies.mdx) are trained and run during inference. As part of these changes
...
Please read the updated guide on custom graph components [here](./building-assistants/custom-graph-components.mdx) before
...
[components guide](./building-assistants/custom-graph-components.mdx) it is not allowed to use
...
[`GraphComponent` interface](./building-assistants/custom-graph-components.mdx#the-graphcomponent-interface):
...
[`GraphComponent` interface](./building-assistants/custom-graph-components.mdx#the-graphcomponent-interface). The
...
See [component persistence](./building-assistants/custom-graph-components.mdx#model-persistence) for more details.
...
NLU Components like [tokenizers](./building-classic-assistants/components.mdx#tokenizers) or
[featurizers](./building-classic-assistants/components.mdx#featurizers) augment the training data with their
...
If your NLU component requires a pretrained model such as a [Spacy](./building-classic-assistants/components.mdx#spacynlp) or
[Mitie](./building-classic-assistants/components.mdx#mitienlp) language model you have to specify the NLU component which
...
the [model configuration](./building-classic-assistants/model-configuration.mdx) is passed in as a dictionary instead
...
as result of your policy's `train` method. See [graph component persistence](./building-assistants/custom-graph-components.mdx#model-persistence) for more details.
...
To use a custom [end-to-end policy](./building-classic-assistants/stories.mdx#end-to-end-training) in Rasa
Open Source 2, you had to use the `interpreter` parameter to featurize the tracker
events manually. In Rasa 3.0,
you need to [register](./building-assistants/custom-graph-components.mdx#registering-graph-components-with-the-model-configuration) a policy that requires end-to-end features with type `ComponentType.POLICY_WITH_END_TO_END_SUPPORT`. The features
...
data actually contains [end-to-end training data](./building-classic-assistants/stories.mdx#end-to-end-training).
:::
...
Rasa allows excluding [forms](./building-classic-assistants/forms.mdx) or [slots](./building-assistants/domain.mdx#slots) which
are completely handled by
[rules](./building-classic-assistants/rules.mdx) from becoming features in other policies.
...
in case the [`RulePolicy`](./building-classic-assistants/policies.mdx#rule-policy) is not part of your model
configuration.
...
- `SklearnPolicy` should be replaced with [TEDPolicy](./building-classic-assistants/policies.mdx#ted-policy).
  It is recommended to use the [default TEDPolicy config](./building-classic-assistants/model-configuration.mdx#suggested-config) as a starting point.
...
action [`action_extract_slots`](./building-assistants/default-actions.mdx#action_extract_slots). This action runs in the background
...
You can use [slot validation actions](./building-assistants/slot-validation-actions.mdx) to either validate slots with predefined
...
See the docs on [mapping conditions](./building-assistants/domain.mdx#mapping-conditions) for more information.
...
For slots that should be filled only in the context of a form, add [mapping conditions](./building-assistants/domain.mdx#mapping-conditions)
...
of this custom action as per the strong recommendation listed in the [dynamic form documentation](./building-classic-assistants/forms.mdx#dynamic-form-behavior).

To extract custom slots that are not defined in any form's `required_slots`, you should now use a global [custom slot mapping](./building-assistants/domain.mdx#custom-slot-mappings)
...
[`action_extract_slots`](./building-assistants/default-actions.mdx#action_extract_slots) may need to be reset within the context of your
...
[`CountVectorsFeaturizer`](./building-classic-assistants/components.mdx#countvectorsfeaturizer) or
`number_additional_patterns` for [`RegexFeaturizer`](./building-classic-assistants/components.mdx#regexfeaturizer).
...
intent or intents. Please see the [Forms documentation](./building-classic-assistants/forms.mdx) for examples and more
...
- The terminology `template` is deprecated and replaced by `response`. Support for `template` from the NLG response will be removed in Rasa 3.0.0. Please see [here](./production/nlg.mdx) for more details.
...
- The field `template_name` will be deprecated in Rasa 3.0.0. Please use `utter_action` instead. Please see [here](./building-classic-assistants/components.mdx#selectors) for more details.
- The field `response_templates` will be deprecated in Rasa 3.0.0. Please use `responses` instead. Please see [here](./building-classic-assistants/components.mdx#selectors) for more details.
...
The default value is `softmax`, but we recommend trying `linear_norm`. This should make it easier to [tune thresholds for triggering fallback](./building-classic-assistants/fallback-handoff.mdx#fallbacks).
...
Once the assistant is re-trained with the above configuration, users should also [tune fallback confidence thresholds](./building-classic-assistants/fallback-handoff.mdx#fallbacks).
...
[Policies](./building-classic-assistants/policies.mdx) now require a `**kwargs` argument in their constructor and `load` method.
Policies without `**kwargs` will be supported until Rasa version `3.0.0`.
However when using [incremental training](./building-assistants/command-line-interface.mdx#incremental-training)
...
`ConveRTTokenizer` is now deprecated. [ConveRTFeaturizer](./building-classic-assistants/components.mdx#convertfeaturizer) now implements
...
[LanguageModelFeaturizer](./building-classic-assistants/components.mdx#languagemodelfeaturizer) now implements their behaviour.
...
With the introduction of [rules](./building-classic-assistants/rules.mdx) and the [RulePolicy](./building-classic-assistants/policies.mdx#rule-policy),
...
can follow the documentation on [FAQs](./building-classic-assistants/chitchat-faqs.mdx) to convert your mapped
...
[Rule Policy](./building-classic-assistants/policies.mdx#rule-policy) in your model configuration:
...
In addition, you need to add a [rule](./building-classic-assistants/rules.mdx) to specify which action to run
...
See the documentation on [fallback](./building-classic-assistants/fallback-handoff.mdx#fallbacks) for more
information.
...
In addition you need to add a [rule](./building-classic-assistants/rules.mdx) to activate the Two-Stage Fallback for
...
See the [fallback](./building-classic-assistants/fallback-handoff.mdx#fallbacks) documentation for more
information.

### Forms

As of version 2.0 the logic for [forms](./building-classic-assistants/forms.mdx) has been moved from the
...
[slot validation](./building-classic-assistants/forms.mdx#validating-form-input) can be handled with a `FormValidationAction`.
...
Start the migration by removing the FormPolicy and adding the [RulePolicy](./building-classic-assistants/policies.mdx#rule-policy)
...
in the domain as described in the documentation on [forms](./building-classic-assistants/forms.mdx#defining-a-form):
...
See the [forms](./building-classic-assistants/forms.mdx) documentation for more details.
...
on what that looks like in the [chitchat and FAQs documentation](./building-classic-assistants/chitchat-faqs.mdx).
...
[Slots](./building-assistants/domain.mdx#slots) of type `unfeaturized` are
...
type [any](./building-assistants/domain.mdx#any-slot). This slot type is always ignored during a conversation
...
Please see the updated [slots documentation](./building-assistants/domain.mdx#slots) for more information.

### Conversation sessions

[Conversation sessions](./building-assistants/domain.mdx#session-configuration) are now enabled by default
if your [Domain](./building-assistants/domain.mdx) does not contain a session configuration. Previously a
...
This section is only relevant if you explicitly defined [featurizers](./building-classic-assistants/policies.mdx#featurizers)
...
The deprecated [event brokers](./production/event-brokers.mdx) FileProducer, KafkaProducer, PikaProducer
...
with [DIETClassifier](./building-classic-assistants/components.mdx#dietclassifier).
...
with [TEDPolicy](./building-classic-assistants/policies.mdx#ted-policy). It accepts the same configuration parameters.
...
- The [TED Policy](./building-classic-assistants/policies.mdx#ted-policy) replaced the `keras_policy` as recommended machine
  ...
  [TED Policy](./building-classic-assistants/policies.mdx#ted-policy) add this to the `policies` section in your `config.yml`
  ...
  Please see the docs of the [TED Policy](./building-classic-assistants/policies.mdx#ted-policy) if you want to customize them.
...
- All pre-defined pipeline templates are deprecated. **Any templates you use will be
  ...
  Take a look at [Tuning Your Model](./building-classic-assistants/tuning-your-model.mdx) to decide on what components you should use
  in your configuration file.

- The Embedding Policy was renamed to [TED Policy](./building-classic-assistants/policies.mdx#ted-policy). The functionality of the policy stayed the same.
...
- The Embedding Intent Classifier is now deprecated and will be replaced by [DIETClassifier](./building-classic-assistants/components.mdx#dietclassifier)
  in the future.
  ...
  See [DIETClassifier](./building-classic-assistants/components.mdx#dietclassifier) for more information about the new component.
...
- `CRFEntityExtractor` is now deprecated and will be replaced by `DIETClassifier` in the future. If you want to
  ...
  We extracted the featurization from the component into the new featurizer [LexicalSyntacticFeaturizer](./building-classic-assistants/components.mdx#lexicalsyntacticfeaturizer). Thus,
  in order to obtain the same results as before, you need to add this featurizer to your pipeline before the
  [DIETClassifier](./building-classic-assistants/components.mdx#dietclassifier).
...
- If your pipeline contains `CRFEntityExtractor` and `EmbeddingIntentClassifier` you can substitute both
  components with [DIETClassifier](./building-classic-assistants/components.mdx#dietclassifier). You can use the following pipeline for that:
...
- Default parameters of `EmbeddingIntentClassifier` are changed. See
  ...
  Default parameters and architecture for `EmbeddingPolicy` are changed. See [Policies](./building-classic-assistants/policies.mdx) for details.
...
- Default `max_history` for `EmbeddingPolicy` is `None` which means it'll use
  ...
  for **faster training**. See [Featurizers](./building-classic-assistants/policies.mdx#featurizers) for details.
...
- The scripts in `rasa.core` and `rasa.nlu` can no longer be executed. To train, test, run, … an NLU or Core
  ...
  should use instead. See all the new commands at [Command Line Interface](./building-assistants/command-line-interface.mdx).
...
- All script parameter names have been unified to follow the same schema.
  ...
  in the [Command Line Interface](./building-assistants/command-line-interface.mdx).
