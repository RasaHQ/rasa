...
to implement a `create` method. During the policy instantation the configuration from
...
constructor but instead returned by the method `get_default_config`:
...
was called from the outside. You should use the provided `model_storage` and `resource`
...
<Tabs values={[{"label": "Rasa 2.0 (old)", "value": "old"}, {"label": "Rasa 3.0 (new)", "value": "new"}]}  groupId="3-0-policy-persistence" defaultValue="new">
  ...
  class MyPolicy(Policy):
      ...
      def persist(self, path: Union[Text, Path]) -> None:
          ...
          file = Path(path) / "model_data.json"
          rasa.shared.utils.io.create_directory_for_file(file)
          rasa.shared.utils.io.dump_obj_as_json_to_file(file, self.get_model_data())
      ...
  ```
  ...
  class MyPolicy(Policy):
      ...
      def persist() -> None:
          with self._model_storage.write_to(self._resource) as directory:
              ...
              file = directory / "model_data.json"
              rasa.shared.utils.io.dump_obj_as_json_to_file(file, self.get_model_data())
      ...
  ```
...
in the previous step, you need to use these now to load the persisted policy.

:::info
It is no longer required to persist your policy's config yourself. The config passed
...
by the method `get_default_config`:
...
**Persisting a Trained Component**
...
<Tabs values={[{"label": "Rasa 2.0 (old)", "value": "old"}, {"label": "Rasa 3.0 (new)", "value": "new"}]}  groupId="3-0-nlu-persistence" defaultValue="new">
  ...
  class MyNLUComponent(IntentClassifier):
      ...
      def persist(self, file_name: Text, model_dir: Text) -> Dict[Text, Any]:
          file = Path(model_dir) / "{file_name}.model_data.json"
          rasa.shared.utils.io.create_directory_for_file(file)
          rasa.shared.utils.io.dump_obj_as_json_to_file(file, self.get_model_data())
...
**Instantiating a Trained Component**
Similar as you had to use the `model_storage` and `resource` to persist the NLU
...
Components like tokenizers or featurizers train augment the training data with their
output during the model training as their output is required by components later in the
...
3.0 makes these different purposes explicit. While both component training and
training data augmentation where done as part of the `train` method previously, they
are now split in `train` and `process_training_data`:
...
**Handling Lists of Messages During Inference**
...
parameter in the `register` decorator. The model will then be passed to your models
