...
:::note
If you have custom validation actions extending `FormValidationAction` which override `required_slots` method, it is highly
recommended you double-check the dynamic form behavior of your migrated assistant, in case slots set by the default action
[`action_extract_slots`](./default-actions.mdx#action_extract_slots) might need to be reset within the context of your form by the custom validation actions.
