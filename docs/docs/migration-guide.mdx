...
[`ConcurrentRedisLockStore`](./lock-stores.mdx#concurrentredislockstore) is the new Redis implementation of the abstract interface `LockStore`.
...
In order to replace the use of `RedisLockStore` with `ConcurrentRedisLockStore`, specify the new parameter `concurrent_mode` as `true` in `endpoints.yml`:
