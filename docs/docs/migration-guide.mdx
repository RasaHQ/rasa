...
A lot has changed in version 2.0, we hope for the better! Make sure you read
...
some manual conversion. If you have any feedback about these updates, please post them
...
making backups of your existing files using the `.bak` suffix.
...
defined in the domain. Any customisations around requesting slots or slot validation
can be handled with a `FormValidationAction`.
...
Start the migration by remoivng the FormPolicy and adding the [RulePolicy](./policies.mdx#rule-policy)
