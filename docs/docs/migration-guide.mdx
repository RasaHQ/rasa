...
Please read the updated guide on custom graph components [here](./concepts/custom-graph-components.mdx) before
...
[components guide](./concepts/custom-graph-components.mdx) it is not allowed to use
...
[`GraphComponent` interface](./concepts/custom-graph-components.mdx#the-graphcomponent-interface):
...
[`GraphComponent` interface](./concepts/custom-graph-components.mdx#the-graphcomponent-interface). The
...
See [component persistence](./concepts/custom-graph-components.mdx#model-persistence) for more details.
...
as result of your policy's `train` method. See [graph component persistence](./concepts/custom-graph-components.mdx#model-persistence) for more details.
...
you need to [register](./concepts/custom-graph-components.mdx#registering-graph-components-with-the-model-configuration) a policy that requires end-to-end features with type `ComponentType.POLICY_WITH_END_TO_END_SUPPORT`. The features
...
Rasa allows excluding [forms](./building-classic-assistants/forms.mdx) or [slots](./concepts/domain.mdx#slots) which
...
action [`action_extract_slots`](./concepts/default-actions.mdx#action_extract_slots). This action runs in the background
...
You can use [slot validation actions](./concepts/slot-validation-actions.mdx) to either validate slots with predefined
...
See the docs on [mapping conditions](./concepts/domain.mdx#mapping-conditions) for more information.
...
For slots that should be filled only in the context of a form, add [mapping conditions](./concepts/domain.mdx#mapping-conditions)
...
To extract custom slots that are not defined in any form's `required_slots`, you should now use a global [custom slot mapping](./concepts/domain.mdx#custom-slot-mappings)
...
[`action_extract_slots`](./concepts/default-actions.mdx#action_extract_slots) may need to be reset within the context of your
...
However when using [incremental training](./concepts/command-line-interface.mdx#incremental-training)
...
[Slots](./concepts/domain.mdx#slots) of type `unfeaturized` are
...
type [any](./concepts/domain.mdx#any-slot). This slot type is always ignored during a conversation
...
Please see the updated [slots documentation](./concepts/domain.mdx#slots) for more information.

### Conversation sessions

[Conversation sessions](./concepts/domain.mdx#session-configuration) are now enabled by default
if your [Domain](./concepts/domain.mdx) does not contain a session configuration. Previously a
...
- The scripts in `rasa.core` and `rasa.nlu` can no longer be executed. To train, test, run, … an NLU or Core
  ...
  should use instead. See all the new commands at [Command Line Interface](./concepts/command-line-interface.mdx).
...
- All script parameter names have been unified to follow the same schema.
  ...
  in the [Command Line Interface](./concepts/command-line-interface.mdx).
