...
* All pre-defined pipeline templates are deprecated. **Any templates you use will be
  ...
  Take a look at [Choosing a Pipeline](./choosing-a-pipeline.mdx) to decide on what components you should use
...
* Default parameters of `EmbeddingIntentClassifier` are changed. See [Components](./components.mdx) for details.
...
* Default `max_history` for `EmbeddingPolicy` is `None` which means it’ll use
  ...
  for **faster training**. See [Featurization of Conversations](./core-featurization.mdx#featurization-conversations) for details.
