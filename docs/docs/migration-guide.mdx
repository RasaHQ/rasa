...
With the introduction of [Rules](./core/rules.mdx), the following policies are
deprecated:
...
If you previously used the [Fallback Policy](./core/policies.mdx#fallback-policy), the following model
...
[Two-Stage-Fallback Policy](./core/policies.mdx#two-stage-fallback-policy), the following model
configuration would translate as follows given a previous configuration like this:
...
As of Rasa Open Source 2.0 the logic for [Forms](./core/forms.mdx) has been moved from the
