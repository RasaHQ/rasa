...
intent, eg. chitchat. Please see the [Forms documentation](forms.mdx) for examples and
more information on how to use it in your `domain.yml` file.

So before, if a user did not want to fill any slots of a form with a specified intent
we would have to define it (with the `not_intent` parameter) in every slot
