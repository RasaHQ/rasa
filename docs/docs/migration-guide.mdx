...
With the introduction of [Rules](./core/rules.mdx) the following policies were
deprecated:

- [Mapping Policy](./core/policies.mdx#mapping-policy)
- [Fallback Policy](./core/policies.mdx#fallback-policy)
- [Two-Stage-Fallback Policy](./core/policies.mdx#two-stage-fallback-policy)
- [Form Policy](./core/policies.mdx#form-policy)

<a aria-hidden="true" tabIndex="-1" className="anchor enhancedAnchor" id="migrate-mapping-policy-to-rule-policy"></a>

#### Migrating from the Mapping Policy

If you previously used the [Mapping Policy](./core/policies.mdx#mapping-policy), you
can follow the documentation on [FAQs](./core/rules.mdx#faqs) to convert your mapped
...
<a aria-hidden="true" tabIndex="-1" className="anchor enhancedAnchor" id="migrate-fallback-policy-to-rule-policy"></a>

#### Migrating from the Fallback Policy

If you previously used the [Fallback Policy](./core/policies.mdx#fallback-policy), then the following model
...
In addition you need to add a [Rule](./core/rules.mdx) to specify which action to run
...
Please see the [Fallback Actions](./core/fallback-actions) documentation for more
information.

<a aria-hidden="true" tabIndex="-1" className="anchor enhancedAnchor" id="migrate-two-stage-policy-to-rule-policy"></a>

#### Migrating from the Two-Stage-Fallback Policy

If you previously used the
[Two-Stage-Fallback Policy](./core/policies.mdx#two-stage-fallback-policy), then the following model
configuration would translate as follows given a previous configuration like this:
...
In addition you need to add a [Rule](./core/rules.mdx) to activate the Two-Stage Fallback for
...
<a aria-hidden="true" tabIndex="-1" className="anchor enhancedAnchor" id="migrate-form-policy-to-rule-policy"></a>

#### FormPolicy

As of Rasa Open Source 2.0 the logic for [Forms](./core/forms.mdx) was moved from the
Rasa SDK to Rasa Open Source to ease implementations of custom action libraries.
...
[Domain](./core/domains.mdx) of your bot. Consider a custom form action like this:
...
Start the migration by adding the [RulePolicy](./core/policies.mdx#rule-policy) to your
model configuration:
...
[Forms](./core/forms.mdx#form-domain):
...
Please see [Forms](./core/forms.mdx) if you have further customizations in your
``FormAction``.
...
* The [TED Policy](./core/policies.mdx#ted-policy) replaced the `keras_policy` as recommended machine
  ...
  [TED Policy](./core/policies.mdx#ted-policy) add this to the `policies` section in your `config.yml`
  ...
  Please see the docs of the [TED Policy](./core/policies.mdx#ted-policy) if you want to customize them.
...
* All pre-defined pipeline templates are deprecated. **Any templates you use will be
  ...
  Take a look at [Choosing a Pipeline](./nlu/choosing-a-pipeline.mdx) to decide on what components you should use
  in your configuration file.

* The [Embedding Policy](./core/policies.mdx#embedding-policy) was renamed to [TED Policy](./core/policies.mdx#ted-policy). The functionality of the policy stayed the same.
...
* The Embedding Intent Classifier is now deprecated and will be replaced by [DIETClassifier](./nlu/components.mdx#diet-classifier)
  in the future.
  ...
  See [DIETClassifier](./nlu/components.mdx#diet-classifier) for more information about the new component.
...
* `CRFEntityExtractor` is now deprecated and will be replaced by `DIETClassifier` in the future. If you want to
  ...
  We extracted the featurization from the component into the new featurizer [LexicalSyntacticFeaturizer](./nlu/components.mdx#lexicalsyntacticfeaturizer). Thus,
  in order to obtain the same results as before, you need to add this featurizer to your pipeline before the
  [DIETClassifier](./nlu/components.mdx#diet-classifier).
...
* If your pipeline contains `CRFEntityExtractor` and `EmbeddingIntentClassifier` you can substitute both
  components with [DIETClassifier](./nlu/components.mdx#diet-classifier). You can use the following pipeline for that:
...
* Default parameters of `EmbeddingIntentClassifier` are changed. See [Components](./nlu/components.mdx) for details.
  Architecture implementation is changed as well, so **old trained models cannot be loaded**.
  Default parameters and architecture for `EmbeddingPolicy` are changed. See [Policies](./core/policies.mdx) for details.
...
* Default `max_history` for `EmbeddingPolicy` is `None` which means it’ll use
  ...
  for **faster training**. See [Featurization of Conversations](./api/core-featurization.mdx#featurization-conversations) for details.
...
* The scripts in `rasa.core` and `rasa.nlu` can no longer be executed. To train, test, run, … an NLU or Core
  ...
  should use instead. See all the new commands at [Command Line Interface](./user-guide/command-line-interface.mdx).
...
* All script parameter names have been unified to follow the same schema.
  ...
  in the [Command Line Interface](./user-guide/command-line-interface.mdx).
