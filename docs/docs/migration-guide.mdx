...
The reason for this is because what used to be stored by the `RedisLockStore`(i.e., serialized [TicketLock](https://rasa.com/docs/rasa/reference/rasa/core/lock/#ticketlock-objects) instances) is no longer stored by the new `ConcurrentRedisLockStore`.
