...
Rasa Open Source can take care of automatically migrating the Mapping Policy,
the Fallback Policy and the Two-Stage-Fallback Policy for you. To do so, run the
following command:
...
This command will take care of updating your `config.yml`, `domain.yml` and `rules.yml`, while
making backups of your pre-existing files using the `.bak` suffix.

You can follow the steps below if you wish to do this migration manually.
Please note that you would still need to [migrate the Form Policy](#migrating-from-the-form-policy) manually.
:::
...
After removing the triggers from your domain file, you can finally replace the
Mapping Policy with the [Rule Policy](./policies.mdx#rule-policy) in your model configuration:
...
Please see the [Fallbacks](./fallback-handoff.mdx#fallbacks) documentation for more
information.
...
#### Migrating from the Form Policy
...
* `LabelTokenizerSingleStateFeaturizer` is deprecated and will be removed in the future.
  Please use `SingleStateFeaturizer` with appropriate NLU pipeline.
  ...
  to the NLU pipeline. Please update your configuration file.
  ...
  Please train both NLU and Core (using `rasa train`) to use a trained tokenizer and
  featurizer for Core featurization.

* `BinarySingleStateFeaturizer` is deprecated and will be removed in the future.
  If you train both NLU and Core (using `rasa train`),
  use `SingleStateFeaturizer` with NLU pipeline
  ...
  Alternatively, you can train a Core model without NLU using `rasa train core`.

### Unfeaturized Slots

[Slots](domain.mdx#slots) of type [`unfeaturized`](domain.mdx#unfeaturized-slot) are
deprecated and will be removed in Rasa Open Source 3.0. To ignore slot values during
a conversation, set the `influence_conversation` property of the slot to `false`.

The following snippet is an example of a deprecated `unfeaturized` slot usage:
...
With Rasa Open Source 2.0 you can now specify the expected data type `text` and
