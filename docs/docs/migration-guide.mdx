...
Please read the updated guide on custom graph components [here](./concepts/components/custom-graph-components.mdx) before
...
[components guide](./concepts/components/custom-graph-components.mdx) it is not allowed to use
...
[`GraphComponent` interface](./concepts/components/custom-graph-components.mdx#the-graphcomponent-interface):
...
[`GraphComponent` interface](./concepts/components/custom-graph-components.mdx#the-graphcomponent-interface). The
...
See [component persistence](./concepts/components/custom-graph-components.mdx#model-persistence) for more details.
...
as result of your policy's `train` method. See [graph component persistence](./concepts/components/custom-graph-components.mdx#model-persistence) for more details.
...
you need to [register](./concepts/components/custom-graph-components.mdx#registering-graph-components-with-the-model-configuration) a policy that requires end-to-end features with type `ComponentType.POLICY_WITH_END_TO_END_SUPPORT`. The features
...
See the docs on [mapping conditions](./building-classic-assistants/domain.mdx#mapping-conditions) for more information.
...
For slots that should be filled only in the context of a form, add [mapping conditions](./building-classic-assistants/domain.mdx#mapping-conditions)
...
- There are numerous HTTP API endpoint changes which can be found [here](./production/http-api.mdx).
