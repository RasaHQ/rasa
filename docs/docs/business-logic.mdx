...
You define a form in the domain file by specifying [slot mappings](forms.mdx#slot-mappings) for each 
...
that would be used for a search. The actions taken after the 
form is submitted is determined by a rule for submitting the form (see the rules in the next section).
