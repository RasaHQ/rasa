...
These slots need to be added to your domain's `slots` section, along with [slot mappings](forms.mdx#slot-mappings) which define how the slots can be filled. For any slot filled `from_entity`, the entity also needs to be added
...
the `outdoor_seating` slot would already be filled when the form was activated. Therefore the form would not prompt the user for their outdoor seating preference. See [mapping conditons](./domain.mdx#mapping-conditions) for more information.
