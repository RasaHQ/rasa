...
We will start by defining the `SalesForm` as a new class in the file called `actions/actions.py`.
...
* `actions/actions.py`: Define the form action, including the `required_slots`, `slot_mappings` and `submit` methods
