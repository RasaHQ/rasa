...
CLI argument. You must pass the public key to the `--jwt-secret` argument since this is an alias for the public key, and also
specify the algorithm to the `--jwt-method` argument:

```bash
rasa run \
    --enable-api \
    --jwt-secret thisismypublickey \
    --jwt-private-key thisismyprivatekey \
    --jwt-method RS512
```
