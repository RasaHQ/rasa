...
[Data validation](./validate-files.mdx) verifies that there are no mistakes or
...
Data validation includes [story structure validation](./validate-files.mdx#test-story-files-for-conflicts).
