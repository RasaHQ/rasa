...
[Data validation](./validate-files) verifies that there are no mistakes or
...
Data validation includes [story structure validation](./validate-files#test-story-files-for-conflicts).
...
If it passes the CI tests, then you can also [upload the trained model](./setting-up-ci-cd#uploading-a-model)
...
Testing your trained model on [test conversations](./testing-your-assistant#end-to-end-testing) is the best way to have confidence in how your assistant
...
[test your action code](./setting-up-ci-cd#testing-action-code) in a seperate step.
...
[full NLU evaluation](./testing-your-assistant#nlu-evaluation). You’ll want to compare
...
set approach, it is best to [shuffle and split your data](./command-line-interface#train-test-split) using `rasa data split` as part of this CI step, as
...
If you ran [end-to-end tests](./setting-up-ci-cd#test-the-assistant) in your CI pipeline,
...
You can automate
[building and uploading a new image for your action server](./how-to-deploy#building-an-action-server-image),
