...
To deploy Rasa X and your assistant together, see the [Recommended Deployment Methods](../how-to-deploy#recommended-deployment-methods).
...
[Messaging and Voice Channels](../messaging-and-voice-channels) in `credentials.yml`. For example, to add a
...
an action server image, see [Building an Action Server Image](../how-to-deploy#building-an-action-server-image).
...
corresponds to a [configured channel](./deploying-in-docker-compose#docker-compose-configuring-channels):
...
[Tracker Store](../../api/tracker-stores).
...
* [RedisTrackerStore](../../api/tracker-stores#redis-tracker-store)

* [MongoTrackerStore](../../api/tracker-stores#mongo-tracker-store)

* [Custom Tracker Store](../../api/tracker-stores#custom-tracker-store)
