...
Where applicable, entities are annotated in training examples using the
syntax:
...
Both [retrieval actions](../core/retrieval-actions) and [responses](#responses) go under the `responses` key. Responses can include text, images and buttons. In this example, `utter_greet` and 
`utter_cheer_up` are bot responses, and `faq/language` is a retrieval
action:
