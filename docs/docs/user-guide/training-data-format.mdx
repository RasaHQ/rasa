...
All YAML training data files should specify the `version` key
...
entity nor the intent, it is a human readable description for you
...
don't define entities nor intents! They only provide patterns to
...
or the last step in a story. If they are the last step in a stories, that story
