...
to be parsed correctly. If you don't specify a version key in your training data file, Rasa will assume you are using the latest training data format specification. Training data files with a version greater than is currently
...
However, its also possible to use an extended format if you need to include
...
Training examples can include [entities])(../glossary#entity), which are annotated with the entity name. 
In addition to the entity name, you can assign synonyms, roles, or groups to an entity.

The abbreviated syntax for annotating an entity is: 
