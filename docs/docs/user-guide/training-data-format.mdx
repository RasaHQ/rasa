...
See the [documentation for domains](../core/domains) to learn more.
...
to be parsed correctly. Training data files with no `version` specified will be assumed to
be in the format of the latest version of Rasa Open Source. Training data files with a version greater than is currently
...
pieces of information that can be extracted from a user's message. The other components
...
However, its also possible to use an extended format if you want to include
metadata for your examples:
...
The `metadata` key can contain arbitrary key-value data and the parser will not read its value. 
Metadata can be useful for storing information relevant to an example, but not used by 
the NLU pipeline. In the example above, the sentiment of 
the example is recorded as metadata.

### Entities

Where applicable, [entities])(../glossary#entity) can be annotated in training examples using either the
abbreviated syntax:
...
The third example above includes a [synonym](#synonyms).
You can assign synonyms, roles, or groups to an entity using the
syntax:
...
pipeline contains the [`EntitySynonymMapper`](../nlu/components#entitysynonymmapper) component. You
should define synonyms when there are multiple ways users refer to the same
thing.

#### Example 

For example, let's say you had an entity `account`, and you expect the
...
That means that you need to provide examples of the variations 
on a synonym in your training examples so
...
extraction when using the [RegexFeaturizer](../nlu/components#regexfeaturizer) component in the pipeline.

2. They can be used to extract entities from a user message using the rule-based
[`RegexEntityExtractor`](../nlu/components#regexentityextractor) component in the pipeline.
...
detection of that entity (using the `RegexFeaturizer`) or to
extract the entities from the user message directly (using the
`RegexEntityExtractor`). For a bank account number that is 10-12 digits long,
...
These regexes are processed identically to the regular
...
**Stories** are representations of conversations between a user and a conversational
assistant. They are used to train the dialogue management model and can help it
learn to generalize to unseen conversation paths.

Stories are composed of:
  
  - name: The name is arbitrary and not used in training; 
...
a set of required slots and ask the user for this information. The "happy path" 
...
Checkpoints are ways to connect stories together. They can be either the first or the last step in a story. If they are the last step in a stories, that story will be connected to each other story that starts with the checkpoint of the same name when the model is trained.
...
**[Rules](../core/rules)** describe parts of conversations that should always follow
the same path and are used to train the [RulePolicy](../core/policies#rule-policy). 
