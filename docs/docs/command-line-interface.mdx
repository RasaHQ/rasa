...
Note that CLI config has the highest precedence, and hence following command will set log level to INFO (due to `--verbose`) and no debug messages will be seen (library level configuration will not have an effect):
