...
Our data augmentation strategy works by feeding the generated paraphrases and then evaluating whether adding a subset
of these can improve intent classification performance.
...
annotations during NLU training or to disable the enitity recognition option in the
[DIETClassifier](./components.mdx#dietclassifier) config.
