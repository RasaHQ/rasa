...
By default, the validator searches for errors in the data, e.g. two intents that have some
identical training examples.
Furthermore, the validator checks if you have any stories where different assistant actions follow from the same 
...
Note: The `rasa data validate` and `rasa data validate stories` commands do **not** test if your [rules](./rules.mdx) 
are consistent with your stories. 

To interrupt validation even for minor issues once they are identified, use the `--fail-on-warnings` flag.
