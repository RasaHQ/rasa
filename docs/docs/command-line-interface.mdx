...
2. The domain supplied should be exactly the same as the domain used to train the model initially. 
That means no new intents, actions, entities or slots can be added to the domain.
Only new training examples can be added.
