...
new training dataset consisting of the additional training examples. This will help reduce the
...
requires less number of epochs to train ML components like `DIETClassifier`, `ResponseSelector` and `TEDPolicy`.
It's suggested to either use a model configuration for finetuning
which defines lesser number of epochs than before or use the flag
`--epoch-fraction`. `--epoch-fraction` will use a fraction of the epochs specified for each ML component
in the given model configuration file. For e.g., if the `DIETClassifier` is configured to use 100 epochs,
specifying `--epoch-fraction 0.5` will only use 50 epochs for finetuning.

You can also finetune an only NLU and only Core model by using
`rasa train nlu --finetune` and `rasa train core --finetune` respectively.

Conditions for a model to be finetunable are as follows:

1. The current configuration supplied should be exactly the same as the
configuration used to train the model which is being finetuned.
The only parameter that you can change is `epochs` for the individual ML components of the pipeline.

2. The domain of the assistant hasn't changed which means no new
intents, actions, entities and slots have been added to the training data.
Adding new training examples for each of these is valid.
