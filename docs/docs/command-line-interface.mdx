...
directory labelled `original_domain`.

Note that the slots in the migrated domain will contain mapping conditions if these slots are part of a form's
`required_slots`.

:::caution
Exceptions will be raised that will terminate this process if invalid domain files are provided, slots or forms
are missing from your original files or if the slots or forms sections are spread across multiple domain files.
