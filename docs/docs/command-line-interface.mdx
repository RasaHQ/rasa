...
The `--validate-before-training` flag will run a validation check on your training data before training the model.
This is the same validation that is also run when you run `rasa data validate`.
Flags `--fail-on-validation-warnings` and `--validation-max-history` are used to configure the validation and are only
used when the `--validate-before-training` flag is set.
