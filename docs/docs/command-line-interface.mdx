...
to write command-line commands. The command that your plugin provides, is
determined based on the name of your plugin's executable. Everything after the first dash
will be considered the plugin name. This means an executable `rasa-my-plugin` will
be available via `rasa my-plugin`.

The following snippet is the example of a plugin which prints `Hello world!` if you
run `rasa hello` without additional parameters.
...
Then add your plugin to your `PATH` as described
[here](command-line-interface.mdx#installing-a-plugin).
