...
|`rasa data suggest nlu` |Performs data augmentation by paraphrasing and suggests utterances that can be added to the original training data.                       |
...
You can perform data augmentation by first [generating paraphrases](https://github.com/RasaHQ/paraphraser/tree/master) and subsequently run the following command in order to get data suggestions that can be added to the training data:
...
The paraphrases should be pre-processed and supplied with similarity scores in order to ensure that noise is minimised (see the [paraphraser repository](https://github.com/RasaHQ/paraphraser/tree/master) for more details).

:::note
Usage of `rasa data suggest nlu` requires a set of generated paraphrases from [https://github.com/RasaHQ/paraphraser](https://github.com/RasaHQ/paraphraser/tree/master).
:::
...
You need to specify additional arguments for `rasa data suggest nlu`, e.g. the location of your generated paraphrases, your training and evaluation data, as well as an `intent_report.json` from a trained model (without data augmentation) and its config file:

```bash
rasa data suggest nlu --help
```
