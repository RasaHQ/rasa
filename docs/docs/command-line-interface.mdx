...
Please, note that if [`IntentTEDPolicy`](./policies.mdx#intentted-policy) is
included in the pipeline, `action_unlikely_intent` can be predicted at any conversation turn.
You can then choose to confirm or deny the action prediction, but this choice doesn't matter
as running this action doesn't have any implications.
