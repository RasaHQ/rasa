...
| `rasa data validate`    | Checks the domain, NLU, business logic and conversation data for inconsistencies.                                  |
...
To learn more about the checks performed by the validator on the business logic, continue reading in the [next section](./command-line-interface.mdx#validate-flows).
...
- verify whether natural language conditions used in branch steps or `collect` step rejections are valid `pypred` expressions
- determine whether slots used in flows are defined in the domain
- disallow `list` slots from being used in flows
- disallow `dialogue_stack` internal slot from being used in flows

For every failure, the validator will log an error and exit the command.
