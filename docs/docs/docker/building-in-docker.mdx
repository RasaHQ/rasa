...
If you don't have a Rasa project yet, you can build one in Docker without having to install Rasa
...
<li>
  rasa/rasa:{variables.release}-full: Use the Rasa image with the tag '
  {variables.release}-full'
</li>

  <li><inlineCode>train</inlineCode>: Execute the <inlineCode>rasa train</inlineCode> command within the container. For more
  information see <a href={useBaseUrl("/building-assistants/command-line-interface")}>Command Line Interface</a>.</li>
</ul>
...
<p>
  All <inlineCode>rasa/rasa</inlineCode> image tags start with a version number.
  The current version is {variables.release}. The tags are:
</p>
...
To create more sophisticated assistants, you will want to use [Custom Actions](../building-assistants/actions.mdx#custom-actions).
...
- <code>rasa/rasa-sdk:{variables.rasa_sdk_version}</code> : Uses the Rasa SDK image
  with the tag {`${variables.rasa_sdk_version}`}
