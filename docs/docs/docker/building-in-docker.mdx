...
{`docker run -v $(pwd):/app --tmpfs /tmp rasa/rasa:${variables.release}-full init --no-prompt`}</code></pre>
...
<ul>
  ...
  <li><inlineCode>--tmpfs /tmp</inlineCode> mounts /tmp as a temporary file system that is cleared when
    the container is terminated.</li>
...
<pre><code parentName="pre" className="language-bash">
{`docker run -it -v $(pwd):/app --tmpfs /tmp rasa/rasa:${variables.release}-full shell`}</code></pre>
...
<pre><code parentName="pre" className="language-bash">
{`docker run -v $(pwd):/app --tmpfs /tmp rasa/rasa:${variables.release}-full train --domain domain.yml --data data --out models`}</code></pre>
...
<pre><code parentName="pre" className="language-bash">
{`docker run -v $(pwd):/app --tmpfs /tmp rasa/rasa:${variables.release}-full train`}</code></pre>
...
<pre><code parentName="pre" className="language-bash">
{`docker run -it -v $(pwd):/app --tmpfs /tmp -p 5005:5005 --net my-project rasa/rasa:${variables.release}-full shell`}</code></pre>
