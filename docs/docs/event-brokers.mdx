...
It is possible to use an celery as an event broker. Connections to message brokers are established 
using celery's existing connection handler, Kombu. Celery allows connections to different brokers, ie 
Redis, SQS, and RabbitMQ. To explore the connection detail, please see the SQLAlchemy documentation on SQL dialects.
Adding optional keyword arguments for task calls are also allowed by adding the arguments as in here.
### Adding a Celery Event Broker Using the Endpoint Configuration
#### Task is in another code base
```yaml-rasa title="endpoints.yml"
event_broker:
  type: celery
  broker_url: rediss://:password@localhost:6379/0
  queue_task_name: tasks.event_logger
  countdown: 2
  priority: 1
```
