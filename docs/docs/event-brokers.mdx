...
Your custom event broker class will have to override these base class methods that require implementation:
- `from_endpoint_config`: creates an `EventBroker` object from the endpoint configuration.
The method requires the following signature:

 ```python
  @classmethod
  async def from_endpoint_config(
      cls: YourCustomEventBrokerClass,
      broker_config: EndpointConfig,
      event_loop: Optional[AbstractEventLoop] = None,
  ) -> Optional[YourCustomEventBrokerClass]:
...
- `is_ready`: determine whether or not the event broker is ready.
- `close`: close the connection to an event broker.

### Configuration

In your `endpoints.yml` put in the module path to your custom event broker and the parameters you require:
