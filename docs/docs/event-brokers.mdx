...
If `GSSAPI` is used as the `sasl_mechanism`, the `sasl_kerberos_service_name` and
`sasl_kerberos_domain_name` can be optionally configured. By default,
`sasl_kerberos_service_name` is set to `kafka`.
