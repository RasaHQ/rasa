...
title: Components
abstract:
  Components make up your NLU pipeline and work sequentially to process user input
...
- **Configuration**
  ...
  **Configuring for incremental training**
  ...
  [incremental training](../building-assistants/command-line-interface.mdx#incremental-training), the
...
[^1]:
    Note that `model_confidence: cosine`is deprecated in 2.3.4 (see [changelog](../changelog.mdx#234---2021-02-26))
...
extracted by all of them. If the [slots](../building-assistants/domain.mdx#slots) you are filling with your entity types are of type `text`,
...
- **Description**
  ...
  To use this component, your training data should contain [retrieval intents](../building-assistants/glossary.mdx#retrieval-intent). To define these,
  checkout [documentation on NLU training examples](./training-data-format.mdx#training-examples) and
  [documentation on defining response utterances for retrieval intents](../building-assistants/responses.mdx#defining-responses).
...
See the [guide on custom graph components](../building-assistants/custom-graph-components.mdx) for a complete guide on custom components.
