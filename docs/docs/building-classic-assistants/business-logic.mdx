...
[Forms](./building-classic-assistants/forms.mdx) work by prompting the user for information until it has gathered all required information.
The information is stored in [slots](../building-assistants/domain.mdx#slots). Once all the required slots are filled,
...
These slots need to be added to your domain's `slots` section, along with [slot mappings](../building-assistants/domain.mdx#slot-mappings) which define how the slots can be filled. For any slot filled `from_entity`, the entity also needs to be added
...
the `outdoor_seating` slot would already be filled when the form was activated. Therefore the form would not prompt the user for their outdoor seating preference. See [mapping conditions](../building-assistants/domain.mdx#mapping-conditions) for more information.
...
you define [`responses`](../building-assistants/domain.mdx#responses) called `utter_ask_{slotname}` in your domain:
...
A form's [happy path](../building-assistants/glossary.mdx#happy--unhappy-paths) should be defined as a [rule](rules.mdx) which means you'll need to add the [RulePolicy](./policies.mdx#rule-policy)
to your policies:
