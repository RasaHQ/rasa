...
abstract:
  Stories are a type of training data used to train your assistant's dialogue management
...
to predict the next action based on a _combination_ of both the intent and
entities (you can, however, change this behavior using the
[use_entities](../building-assistants/domain.mdx#ignoring-entities-for-certain-intents) attribute).

### Actions

All actions executed by the bot, including [responses](../building-assistants/responses.mdx) are listed
