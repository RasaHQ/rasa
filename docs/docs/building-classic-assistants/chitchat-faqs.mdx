...
[FAQs](../building-assistants/glossary.mdx#faqs) and [chitchat](../building-assistants/glossary.mdx#chitchat) are two cases where the conversational assistant responds with a fixed set of messages, and the assistant should always answer the same way,
...
<Chat caption="Chitchat">
  ...
  <ChatBotText>
    Rasa X/Enterprise is a tool to learn from real conversations and improve
    your assistant.
  </ChatBotText>
</Chat>
...
Instead of writing 20 rules, you can use a single action, e.g. `utter_faq` to handle all FAQs with a single rule by grouping them together under a single [retrieval intent](../building-assistants/glossary.mdx#retrieval-intent)
called e.g. `faq`.
...
normal bot [response](../building-assistants/domain.mdx#responses). For the chitchat intents
