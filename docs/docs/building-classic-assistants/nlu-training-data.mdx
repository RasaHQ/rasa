...
:::note
The `/` symbol is reserved as a delimiter to separate [retrieval intents](../concepts/glossary.mdx#retrieval-intent) from response text identifiers. Make sure not
...
[Entities](../concepts/glossary.mdx#entity) are structured pieces of information inside a user message.
...
To fill slots from entities with a specific role/group, you need to define a `from_entity` [slot mapping](../concepts/domain.mdx#slot-mappings)
...
[domain file](../concepts/domain.mdx#entities).
