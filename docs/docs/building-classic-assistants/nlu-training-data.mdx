...
The goal of NLU (Natural Language Understanding) is to extract structured information from user messages. This usually includes the user's [intent](../building-assistants/glossary.mdx#intent) and any
[entities](../building-assistants/glossary.mdx#entity) their message contains. You can
...
:::note
The `/` symbol is reserved as a delimiter to separate [retrieval intents](../building-assistants/glossary.mdx#retrieval-intent) from response text identifiers. Make sure not
...
[Entities](../building-assistants/glossary.mdx#entity) are structured pieces of information inside a user message.
...
To fill slots from entities with a specific role/group, you need to define a `from_entity` [slot mapping](../building-assistants/domain.mdx#slot-mappings)
...
[domain file](../building-assistants/domain.mdx#entities).
