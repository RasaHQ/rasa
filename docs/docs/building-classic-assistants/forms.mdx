...
Define a form by adding it to the `forms` section in your [domain](../building-assistants/domain.mdx).
...
looking for a [response](../building-assistants/responses.mdx) called
...
As of 3.0, [slot mappings](../building-assistants/domain.mdx#slot-mappings) are defined in the `slots` section of the domain.
...
Note specifically the role of [Mapping Conditions](../building-assistants/domain.mdx#mapping-conditions) and the
[unique entity mapping](../building-assistants/domain.mdx#unique-from_entity-mapping-matching) constraint.
:::
...
Forms are fully customizable using [Custom Actions](../building-assistants/actions.mdx#custom-actions).
...
You can implement a [Custom Action](../building-assistants/actions.mdx#custom-actions) `validate_<form_name>`
...
If none of the predefined [Slot Mappings](../building-assistants/domain.mdx#slot-mappings) fit your use
case, you can use the
[Custom Action](../building-assistants/actions.mdx#custom-actions) `validate_<form_name>` to write your own
...
slot of type [`text`](../building-assistants/domain.mdx#text-slot). The value of the `requested_slot` will be
...
to one of the bot's questions with another question, like _why do you need to know that?_
...
interactive learnin to build these stories.
