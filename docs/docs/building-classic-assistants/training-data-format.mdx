...
The [domain](../building-assistants/glossary.mdx#domain) uses
...
the definitions for [responses](./responses.mdx) and [forms](./building-classic-assistants/forms.mdx).
See the [documentation for the domain](./building-assistants/domain.mdx) for information on how to format your domain file.
...
[NLU](../building-assistants/glossary.mdx#nlu) training data consists of example user utterances categorized by
[intent](../building-assistants/glossary.mdx#intent). Training examples can also include [entities](../building-assistants/glossary.mdx#entity). Entities are structured
...
Training examples are grouped by [intent](../building-assistants/glossary.mdx#intent) and listed under the
...
If you want to specify [retrieval intents](../building-assistants/glossary.mdx#retrieval-intent), then your NLU examples will look as follows:
...
[Entities](../building-assistants/glossary.mdx#entity) are structured pieces of information that can be extracted from a user's message.
...
policies learn to predict the next action based on a _combination_ of
...
1. [Responses](../building-assistants/domain.mdx#responses): start with `utter_` and
...
2. [Custom actions](../building-assistants/custom-actions.mdx): start with `action_`, run
...
A [form](../building-assistants/glossary.mdx#form) is a specific kind of custom action that contains the logic to loop over
...
Once defined, you should specify the [happy path](../building-assistants/glossary.mdx#happy--unhappy-paths)
for a form as a [rule](./building-classic-assistants/forms.mdx). You should include interruptions of forms or
...
**[Slots](../building-assistants/domain.mdx#slots)** act as the bots memory.
Slots are **set** by either the default action [`action_extract_slots`](../building-assistants/default-actions.mdx#action_extract_slots) according to the
[slot mappings](../building-assistants/domain.mdx#slot-mappings) specified in the domain, or by custom actions.
...
[slot type](../building-assistants/domain.mdx#slot-types) and whether the value can or should
