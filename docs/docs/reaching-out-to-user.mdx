...
To have an event from some other device change the course of an ongoing conversation, you can 
...
For example, the following post request would inject the intent `EXTERNAL_dry_plant` and the `plant` entity and value
into the conversation with the `conversation_id` "`user123`":
...
there won't be any NLU training examples for it. You'll also need to define an intent to get the `conversation_id` 
for the POST request. You can add NLU training examples for this intent if you want, or you can just add the intent 
and trigger it by entering `/tell_id`:
...
You'll need two rules for the dry plant example: one to get the `conversation_id` back, and one
...
To try out the dry plant notification example, you'll need to start both `rasa shell` 
and the action server, since you need a custom action to tell you your `conversation_id`.

Once you have retrained your model with the data above, you can try it out by 
getting the `conversation_id` back from the assistant:

```yaml
User: /tell_id
  Bot: Your conversation ID is user1234
```

Using the conversation ID from the bot's response, run this POST request in a separate terminal window:
...
In your shell, you should see the bot respond to the request:
...
send it a message like "Remind me to call Paul". If `Paul` is extracted as a `name` 
...
the request to the right `conversation_id` automatically after a certain amount of time using 
...
- intent: ask_remind_call
  examples: |
    ...
    - later I have to call Daksh
    - later I have to call Anna
- intent: ask_forget_reminders
  examples: |
    - cancel all reminders
    - cancel that please
    - please cancel that reminder
    - forget the reminder
```

By adding `SpacyNLP` and `SpacyEntityExtractor` to your pipeline in your config file, you won't need to annotate any of the 
names in your training data, since Spacy has a `name` dimension:
