...
The best way to improve an assistant is with frequent [incremental updates](https://rasa.com/docs/rasa-x/fix-problems).
...
Rasa X makes it easy to [add test conversations based on real conversations](https://rasa.com/docs/rasa-x/test-assistant/#how-to-create-tests).
...
[full NLU evaluation](./testing-your-assistant.mdx#nlu-evaluation). You’ll want to compare
...
set approach, it is best to [shuffle and split your data](./command-line-interface.mdx#train-test-split) using `rasa data split` as part of this CI step, as
