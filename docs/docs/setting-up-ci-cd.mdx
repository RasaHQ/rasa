...
smallest of those values in the above validator command. Otherwise, drop the `--max-history` flag.
...
Note: Running `rasa data validate` does **not** test if your [rules](./rules.mdx) are consistent with your stories. 
