...
If the model passes the CI tests, then you can also [upload the trained model](./setting-up-ci-cd.mdx#uploading-a-model)
...
format, test conversations allow you to provide entire conversations and test that, given certain
...
The command to run test conversations looks like:
...
a specific test conversations file or directory with the `--stories` argument.
...
Note: Running test conversations does **not** execute your action code. You will need to
...
If you ran [test conversations](./setting-up-ci-cd.mdx#test-the-assistant) in your CI pipeline,
