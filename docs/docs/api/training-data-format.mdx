...
Both [retrieval actions](#retrieval-actions) and [bot responses](#responses) require message templates to send back to the user. These are listed under the `responses` key. 
Responses can include text, buttons, images, or a custom output payload.
...
responses:
  ...
  faq/other_language:
  - text: |
     I can only do English right now.
