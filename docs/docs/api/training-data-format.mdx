...
for a form should be specified as a [rule](rules#form-policy), but interruptions of forms or 
...
In the example above, `restaurant_form` is specified as an action step, and then two `form` steps.
The second `form` step indicates 
that all required slots are filled; once this condition is met, the next step in the story should 
be taken.
