...
<<<<<<< HEAD
Frequently Asked Question (FAQ) assistants are the simplest assistants to build and a good place to get started.
FAQs and chitchat (e.g. "What's your name?") are two cases where the conversational assistant's answer is a single message,
=======
[FAQs](./glossary.mdx#faqs) and [chitchat](./glossary.mdx#chitchat) are two cases where the conversational assistant's answer is a single message,
>>>>>>> 33dbe38fad441c1a14698b0bb2dd941c87fbcc56
and the assistant should always answer the same way,
...
## Using Retrieval Actions for FAQs and Chitchat
...
It's recommended to start with separate models for each `retrieval_intent`. You can merge them later if you find it works better for your use case.
...
Be sure to update your domain file to include the added `faq` intent.
...
- [ ] Add `ResponseSelector` to your pipeline in `config.yml`
