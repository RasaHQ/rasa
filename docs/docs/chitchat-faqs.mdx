...
Whether you are building a strictly FAQ bot, or a contextual assistant, you usually
want to handle FAQ and chitchat without regard for context. This page is a guide on the configuration
...
Since you always want the same response for the same question, the only policy you need to add to your config
is the [`RulePolicy`](#rule-policy). That doesn't mean you can't include other policies; only that they aren't 
necessary for handling FAQs and chitchat. 
...
While you could use only regular intent classification and action prediction for each intent, you
can also take advantage of the `ResponseSelector` in your NLU pipeline to simplify your rules and stories.

The `ResponseSelector` builds a response retrieval model to to directly predict a bot response from a set of candidate responses.
...
The [ResponseSelector](./components/selectors.mdx#response-selector) NLU component is designed to make it easier to handle
