...
FAQs and chitchat are involve non-contextual questions with single-message answers.
...
An FAQ bot would handle only these kinds of questions, but even with a contextual assistant, you usually
want to handle FAQ and chitchat without regard for context. This page is a guide on the components, policies
