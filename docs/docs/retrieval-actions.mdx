...
A retrieval action uses the output of a [ResponseSelector](../components/selectors.mdx#response-selector) component from NLU which learns a
...
You need to include the [ResponseSelector](../components/selectors.mdx#response-selector) component in your config. The component needs a tokenizer, a featurizer and an
