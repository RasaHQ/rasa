...
response key suffix(for e.g. - `chitchat/ask_name`) as the correct label for that user message. However, the
retrieval model can also be configured to use the text of the response message as the label for training the model.
In that case, if you change the text of these responses, you have to retrain your
...
Rasa uses a naming convention to match the retrieval intent names `chitchat`
to their corresponding retrieval action.
...
Another example - correct action name for `faq` would be `respond_faq`
...
you need to include a separate `ResponseSelector` component in the config:
