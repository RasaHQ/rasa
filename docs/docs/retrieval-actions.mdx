...
By this convention, the `utter_chitchat` action is configured as a response to the `chitchat`
retrieval intent, and `utter_faq` is a response  to `faq`. These actions do not need to be
...
like `utter_chitchat` and `utter_faq`. To train separate retrieval models for each of the retrieval intents,
