...
A retrieval action uses the output of a [ResponseSelector](./components/selectors.mdx#responseselector) 
...
Unlike regular responses, response names for retrieval action responses don't need to start with
`utter_`. Instead, they should correspond to the names of the intents from the training data.
