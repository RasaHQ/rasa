...
`rasa data validate`](../concepts/command-line-interface.mdx#rasa-data-validate).
...
See the [CLI documentation on `rasa test`](../concepts/command-line-interface.mdx#rasa-test) for
...
set approach, it is best to [shuffle and split your data](../concepts/command-line-interface.mdx#rasa-data-split)
...
each sub-intent of a [retrieval intent](../concepts/glossary.mdx#retrieval-intent) and provides an overall average.
...
To run the end-to-end tests locally or in the CI pipeline, use the [`rasa test e2e` command](../concepts/command-line-interface.mdx#rasa-test-e2e).
