...
`rasa data validate`](../building-assistants/command-line-interface.mdx#rasa-data-validate).
...
See the [CLI documentation on `rasa test`](../building-assistants/command-line-interface.mdx#rasa-test) for
more configuration options.

:::caution Testing Custom Actions
[Custom Actions](../building-assistants/custom-actions.mdx) are not executed as part of test stories. If your custom
...
set approach, it is best to [shuffle and split your data](../building-assistants/command-line-interface.mdx#rasa-data-split)
...
each sub-intent of a [retrieval intent](../building-assistants/glossary.mdx#retrieval-intent) and provides an overall average.
...
This file contains all test stories for which [`action_unlikely_intent`](../building-assistants/default-actions.mdx#action_unlikely_intent)
...
trigger the conversation path after an `action_unlikely_intent`](../building-assistants/default-actions.mdx#customization-1) but the ensemble of
...
with different pre-configured contexts, execute [custom actions](../action-server/actions.mdx), verify [response](../building-assistants/responses.mdx)
texts or names, and assert when [slots](../building-assistants/domain.mdx#slots) are filled.
...
- verifying response texts or names: you can assert that the bot response text (including [interpolated responses](../building-assistants/responses.mdx#using-variables-in-responses)
  with slot values and [conditional response variations](../building-assistants/responses.mdx#conditional-response-variations)) or `utter` name
  is as expected.
...
To run the end-to-end tests locally or in the CI pipeline, use the [`rasa test e2e` command](../building-assistants/command-line-interface.mdx#rasa-test-e2e).
