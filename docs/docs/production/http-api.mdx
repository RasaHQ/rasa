...
abstract:
  You can use the HTTP API to interact with a running Rasa server. With the API, you can train models,
...
by enabling an authentication method. See [Security Considerations](#security-considerations).
...
`RedisLockStore` (see [Lock Stores](./production/lock-stores.mdx).
