...
If you are not using GPUs (CPU only), you can set the environment variable
`TF_DETERMINISTIC_OPS=1`. However, there is no deterministic GPU implementation of
`tf.sparse.sparse_dense_matmul`, therefore, training or testing may fail if you are using
GPUs and have set `TF_DETERMINISTIC_OPS=1`.
