...
performance when trained on a GPU across multiple runs. This even applies to situations where training is run
multiple times on the same training data with the same config
and random seeds set appropriately while being evaluated on a standard held-out test set.
Internal experiments have shown the following fluctuation in model's performance when trained and evaluated on a held-out
test set of a variety of datasets (experiments were run 5 times on each dataset):

```
+-----------------------+------------+------------------+------------------+-------------------+
| Task,                 | Number of  | Average standard | Minimum standard | Maximum standard  |
| Metric                | datasets   | deviation        | deviation        | deviation         |
+-----------------------+------------+------------------+------------------+-------------------+
...
You can expect similar fluctuations in the model's performance when you evaluate them on your dataset.
