...
You can use multi-intent classification either to predict multiple intents (e.g. `check_balances+transfer_money`), or to model hierarchical intent structure (e.g. `feedback+positive` being more similar to `feedback+negative` than `chitchat`).
...
:::note
The model will not predict any combination of intents for which examples are not explicitly given in training data. This is by design as accounting for every possible intent combination would result in combinatorial explosion of the number of intents. You should only add those combinations of intents for which you see enough examples coming in from real users.
...
When using a multi-intent, the intent is featurized for dialogue management using multi-hot encoding. That means the featurization of `check_balances+transfer_money` will overlap the featurization of each individual intent. Policies like TEDPolicy, which can generalize from training stories, can then make a prediction based on the multi-intent even if it does not explicitly appear in any stories. 
