...
* **Description**
  ...
  In its default configuration, it uses the retrieval intent with the response key(for e.g.- `faq/ask_name`) as the label for training.
...
* **Configuration**
  ...
  * `weight_sparsity`:
  ...
  In addition, the retrieval model can also be configured to use the text of the response messages as the training label
