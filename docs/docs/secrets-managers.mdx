...
## Secrets Managers
Secrets managers safeguard credentials used to authenticate access to external services,
like databases.
...
You can also encrypt credentials at rest.
...
Rasa Pro will try to renew the token 5 seconds before it expires.
If the token's time-to-live (TTL) is less than 5 seconds, we will try to renew it after 1 second,
...
Toe following environment variables are available:
...
### How to configure Tracker Store to read authentication credentials from Vault Secrets Manager
