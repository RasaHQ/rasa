...
configure namespace with the `VAULT_NAMESPACE` environment variable or `namespace` key in secrets_manager section of
`endpoints.yml` file.
To learn more about namespaces,
check out the [Vault namespace docs](https://developer.hashicorp.com/vault/docs/enterprise/namespaces).
:::
