...
title: Domains
description: Define intents, entities, slots and actions in Rasa to build contextual AI Assistants and chatbots using open source bot framework Rasa.
---
...
extracted by any [entity extractor](../nlu/entity-extraction.mdx) in your
NLU pipeline.
...
[Slots](./slots.mdx) hold information you want to keep track of during a conversation.
...
[Here](./slots.mdx#slot-classes) you can find the full list of slot types defined by
...
see [Custom Actions](./actions.mdx#custom-actions)).

<a aria-hidden="true" tabIndex="-1" className="anchor enhancedAnchor" id="domain-responses"></a>

## Responses

Responses are messages the bot will send back to the user. 
Regular responses should start with `utter_` so that they can be used directly as actions.
[Retrieval actions](../core/retrieval-actions) also go under the `responses` section, and are
written as `<retrieval_model>/<retrieval_intent>` e.g. `faq/language` or `chitchat/askweather`.
Responses can include text, images and buttons. A text response would be defined in the domain as e.g.
...
There are two ways to use these responses:

1. The response can directly be used as an action in a story:
...
For each response, you can have multiple **response variations** (see [Variations](./domains.mdx#variations)).
...
[Customizing the session start action](../api/rasa-sdk.mdx#custom-session-start) shows you how to do that.
