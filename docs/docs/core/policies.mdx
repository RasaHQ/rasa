...
You can also leave this key empty. Then the
[Suggested Config](../api/model-configuration.mdx#suggested-config)
...
The `FallbackPolicy` invokes a [fallback action](./fallback-actions.mdx) if at least one of the following occurs:
...
slots in the form are filled. For more information, see [Forms](./forms.mdx).
