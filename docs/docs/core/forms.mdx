...
As shown in the section [Handling unhappy paths](./forms.mdx#section-unhappy) the bot can execute any kind of
...
Note that for this story to work, your slots should be [unfeaturized](./slots.mdx#unfeaturized-slot). If any of these slots are featurized, your story needs to
include `slot{}` events to show these slots being set. In that case, the
easiest way to create valid stories is to use [Interactive Learning](./interactive-learning.mdx).
...
[slot mappings](#form-slot-mappings) in order to gather all of the information you need.
Other methods like [validation methods](#form-validation) are optional.
...
See [Handling unhappy paths](./forms.mdx#section-unhappy) for how to work with unexpected user input.
...
In the [above example](#formbot-code), you can see how the form uses different
...
[formbot](#formbot-code) checks if the extracted cuisine slot belongs
...
The first thing to try is to run your bot with the `--debug` flag, see [Command Line Interface](../user-guide/command-line-interface.mdx) for details.
