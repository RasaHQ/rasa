...
As shown in the section [Handling unhappy paths](./forms#section-unhappy) the bot can execute any kind of
...
Note that for this story to work, your slots should be [unfeaturized](./slots#unfeaturized-slot). If any of these slots are featurized, your story needs to
include `slot{}` events to show these slots being set. In that case, the
easiest way to create valid stories is to use [Interactive Learning](./interactive-learning).
...
See [Handling unhappy paths](./forms#section-unhappy) for how to work with unexpected user input.
...
Please read [Interactive Learning with Forms](./interactive-learning#section-interactive-learning-forms)
...
The first thing to try is to run your bot with the `--debug` flag, see [Command Line Interface](../user-guide/command-line-interface) for details.
