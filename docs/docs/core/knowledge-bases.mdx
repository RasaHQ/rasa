...
description: Leverage information from knowledge bases inside conversations using ActionQueryKnowledgeBase in open source bot framework Rasa.
---

<a aria-hidden="true" tabIndex="-1" className="anchor enhancedAnchor" id="knowledge-base-actions"></a>
...
(see [Creating Your Own Knowledge Base](./knowledge-bases#custom-knowledge-base)).
...
If it doesn’t, you’ll have to [customize your InMemoryKnowledgeBase](./knowledge-bases#customize-in-memory-knowledge-base).
...
* we will use [synonyms](../nlu/training-data-format#entity-synonyms) extensively
...
* `object_type`: Whenever a training example references a specific object type from your knowledge base, the object type should
  be marked as an entity. Use [synonyms](../nlu/training-data-format#entity-synonyms) to map e.g. `restaurants` to `restaurant`, the correct
...
* `mention`: If the user refers to an object via “the first one”, “that one”, or “it”, you should mark those terms
  ...
  in [resolving mentions](./knowledge-bases#resolve-mentions).
...
it is important that you use [Entity Synonyms](../nlu/training-data-format#entity-synonyms) to map “the first one” in your NLU data to “1”:
...
`KnowledgeBase` implementation (see [Customizing the InMemoryKnowledgeBase](./knowledge-bases#customize-in-memory-knowledge-base)).
...
* `set_ordinal_mention_mapping()`: The ordinal mention mapping is needed to resolve an ordinal mention, such as
  ...
  If you want to learn more about how this mapping is used, check out [Resolve Mentions](./knowledge-bases#resolve-mentions).
...
[Customizing the InMemoryKnowledgeBase](./knowledge-bases#customize-in-memory-knowledge-base).
