...
* we will use [synonyms](../nlu/training-data-format.mdx#entity-synonyms) extensively
...
* `object_type`: Whenever a training example references a specific object type from your knowledge base, the object type should
  be marked as an entity. Use [synonyms](../nlu/training-data-format.mdx#entity-synonyms) to map e.g. `restaurants` to `restaurant`, the correct
...
it is important that you use [Entity Synonyms](../nlu/training-data-format.mdx#entity-synonyms) to map “the first one” in your NLU data to “1”:
