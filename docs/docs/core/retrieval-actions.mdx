...
Retrieval actions are designed to make it simpler to work with [Small Talk](../dialogue-elements/small-talk) and [Simple Questions](../dialogue-elements/completing-tasks#simple-questions) .
...
A retrieval action uses the output of a [ResponseSelector](../nlu/components#response-selector) component from NLU which learns a
...
You need to include the [ResponseSelector](../nlu/components#response-selector) component in your config. The component needs a tokenizer, a featurizer and an
...
intent is to use the [Mapping Policy](./policies#mapping-policy).
...
(see [Unhappy Paths](../dialogue-elements/completing-tasks#unhappy-paths) )
