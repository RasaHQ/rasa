...
[use_entities](./domains#use-entities) attribute).

:::caution
`/` symbol is reserved as a delimiter to separate retrieval intents from response text identifiers.
Refer to `Training Data Format` section of [Retrieval Actions](./retrieval-actions) for more details on this format.
If any of the intent names contain the delimiter, the file containing these stories will be considered as a training
file for [ResponseSelector](../nlu/components#response-selector) model and will be ignored for training Core models.
...
You can read more about events [here](../api/events).
...
way to write these stories is to use [interactive learning](./interactive-learning).
...
into a single file for evaluation. Read more about [Testing Your Assistant](../user-guide/testing-your-assistant)
