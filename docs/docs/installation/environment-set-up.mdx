...
It's worth noting that Python `3.10` is only available for versions `3.4.x` onwards.
Although, on Apple Silicon, Python `3.10` isn't functional in `3.4.x` but will be supported starting from `3.5.x`.
