...
If you don't know which recipe to choose, use the default recipe to bootstrap your project fast. If later you find that you need more powerful finetuning options, you can always change your recipe to be a graph recipe.

:::

## Graph Configuration File Structure

Graph recipes share `recipe` and `language` keys with the same meaning. Similarities end there as graph recipes do not have `pipeline` or `policies` keys but they do have `train_schema` and `predict_schema` keys for determining the graph nodes during train and predict runs respectively. 

Here's an example graph recipe:

```yaml-rasa title="config.yml" (docs/sources/data/configs_for_docs/graph_config_short.yml)
```

:::note graph targets
For NLU, default target name of `run_RegexMessageHandler` will be used, while for core (dialogue management) the target will be called `select_prediction`. Make sure you have graph nodes with relevant names in your schema definitions.

In a similar fashion, note that the default resource needed by the first graph node is fixed to be `__importer__` (representing configuration, training data etc.) for training task and it is `__message__` for prediction task. Make sure your first nodes make use of these dependencies.
...
components](custom-graph-components) pages to find out how to write your
