...
*   If the assistant was trained with [`IntentTEDPolicy`](./policies.mdx#intenttedpolicy) included in the pipeline,
    ...
    running a [standalone script] which does the following:
    ...
    - Run `rasa test` on the fetched conversations and filter conversations containing `action_unlikely_intent`
      in a separate warnings file. Read more on [how to interpret these warnings](./testing-your-assistant.mdx#interpreting-the-generated-warnings).
    ...
    corrections if `TEDPolicy` subsequently failed) as training stories will help in more robust action prediction
    by policies such as `TEDPolicy`. Users are encouraged to [adjust the `tolerance` parameter of
    `IntentTEDPolicy`](./policies.mdx#tuning-the-tolerance-parameter) to control how
    severely "surprising" a conversation should be to be included in the warnings file.
