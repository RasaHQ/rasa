...
title: Model Storage
abstract: |
  After you've trained a model for your assistant, you can use it to run your
  bot.
---
...
Loading the model from your local disk will be used as a fallback if you
don't specify how a model should be loaded.
...
Rasa will check all the models in that directory and load the one which was trained
most recently.

Rasa will assume `models/` to be the default model directory and
will use it in case no `--model` argument is specified. The two following calls
...
#### Configuration of Rasa Open Source
...
```
The server will query `url` for a zipped model every `wait_time_between_pulls`
seconds.

If you want to pull the model just once from the server, you can set the time
...
`endpoints.yml`, e.g. in the above examples this would be
`http://my-server.com/models/default`. You can use any URL.
...
- return a status code of `200`, a zipped Rasa Model and set the `ETag` header in
  the response to the hash of the model.
- return a status code of `304` and an empty response if the `If-None-Match`
  header of the request matches the model you want your server to return.

Rasa Open Source uses the `If-None-Match` and `ETag` headers fro caching. Setting
...
If you are running Rasa on a Google App Engine and Compute Engine, the auth
...
1. Checkout the [GCS documentation](https://cloud.google.com/docs/authentication/getting-started#auth-cloud-implicit-python)
   and follow the descriptions on "Creating a service account" and
   "Setting the environment variable".
2. After you completed the GCS instructions, you should have set an environment
