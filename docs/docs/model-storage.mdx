...
1. Fetch the model from a server (see [Fetching Models from a Server](./model-storage.mdx#fetching-models-from-a-server)), or

2. Fetch the model from a remote storage (see [Remote Storage](./model-storage.mdx#fetching-models-from-remote-storage)).
...
The model will be downloaded from the remote storage system and stored in a temporary directory on your local storage system.
Rasa supports using [S3](https://aws.amazon.com/s3/) ,
[GCS](https://cloud.google.com/storage/) and [Azure Storage](https://azure.microsoft.com/services/storage/)
to save your models, or you can write a custom module to connect to remote storage.

Before you save your models to the cloud, they should be gzipped under the file name `{MODEL_NAME}.tar.gz`.
The model should be stored in the root folder of the storage service.
...
Support for other remote storage can be achieved by using an external library or implementing
