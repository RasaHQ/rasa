...
Rasa uses telemetry to report anonymous usage information. This information is
essential to help improve Rasa Open Source for all users.
For the team working on Rasa Open Source it is important to understand how
the product is used. It allows us to properly prioritize our research efforts
...
You can opt out of telemetry reporting at any time using
```bash
rasa telemetry disable
```

or by setting `RASA_TELEMETRY_ENABLED=false` as an environment variable.

If you want to enable reporting again, you can use
```bash
...
- We will be able to know which languages, pipelines and policies are used.
  This will enable us direct our research efforts towards projects
  that have the most impact on text and dialogue handling.
...
by setting the environment variable `RASA_TELEMETRY_DEBUG=true`, e.g.
```bash
RASA_TELEMETRY_DEBUG=true rasa train
```
When you set `RASA_TELEMETRY_DEBUG` no information will be send to any server,
...
- Rasa machine ID: This is generated with UUID and stored in the global Rasa
  config at `~/.config/rasa/global.yml` and sent as `metrics_id`
- One-way hash of the current working directory or a hash of the git remote (`project`)
