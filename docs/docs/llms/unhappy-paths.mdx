---
id: unhappy-paths
sidebar_label: Manage Unhappy Paths
title: Managing Unhappy Paths
abstract: |
...
Flows map to distinct business functions, creating directed graphs. While 
flow designers primarily focus on linear [happy paths](../glossary.mdx#happy--unhappy-paths), 
Rasa provides a robust way to manage deviations, simplifying design complexity.

### Default Behaviors

Rasa ships a **default behavior for every unhappy path**. Override 
defaults by creating a flows with the same name of a pattern, like `pattern_correction`.
...
Cancellations arise when users halt a flow midway.
...
Users sometimes request pauses or reflection periods.

**Example:** User asks for time to think.

<Chat caption="User requests a moment">
...
Engage in off-topic interactions without hampering the primary flow.

**Example:** A user indulges in casual conversation.
...
Each of these unhappy paths can be configured using Rasa's YAML-based 
domain and flow system. You can overwrite a default behavior by creating
a flow with the same name as the pattern:

- `pattern_continue_interrupted` for digressions.
- `pattern_correction` for corrections.
- `pattern_cancel_flow` for cancellations.
- `pattern_deliberation` for deliberations.
...
Since mose of these patterns interrupt another flow, they should be 
...
### Behaviour dependent on the current flow

You can change a patterns behaviour depending
  ...
  pattern_cancel_flow:
    ...
    steps:
      - id: "decide_cancel_step"
        next: 
        - if: "context.canceled_name != 'my_flow'"
          then: "inform_user"
...
In the above example, the `inform_user` step is skipped if the flow that
was interrupted is called `my_flow`.

## Reference: Default Pattern Configuration

For reference, here is the complete default configuration for unhappy paths:
...
flows:  
  ...
  pattern_correction:
    description: Confirm a correction of a slot value.
