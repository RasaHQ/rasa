...
By utilizing LLMs, we can effectively handle ["unhappy paths"—instances](../concepts/glossary.mdx#happy--unhappy-paths) where
user interactions diverge from the expected or desired course. Meanwhile,
business logic or 'Flows' are used to manage ["happy paths"—scenarios](../concepts/glossary.mdx#happy--unhappy-paths) where
...
- We specify 'happy paths' using business logic. This means setting out clear,
  ...
  circumstances using [Flows](../concepts/flows.mdx).
...
1. [Business Logic with Flows](../concepts/flows.mdx)
