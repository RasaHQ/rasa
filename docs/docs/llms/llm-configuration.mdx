...
1. [intent classification instructions](./llm-intent.mdx#llm--embeddings)
2. [rephrasing instructions](./llm-nlg.mdx#llm-configuration)
3. [intentless policy instructions](./llm-intentless.mdx#llm--embeddings)
4. [docsearch instructions](./llm-docsearch.mdx#llm--embeddings)
...
- `deployment` : you must create a deployment on azure for using embeddings with azure openai. In this case,
  you must also add `deployment` parameter to the config.yml
- `engine`: in case, you are using the chat models from azure openai, you must also create a new openAI deployment
  on azure, and provide the name of the deployment in the config.yml using `engine` parameter.
- `chunk_size`: some azure plans might restrict you from sending larger chunks of text for embeddings. by default, `chunk_size` is 1000
  but this can also be configured to a lower value in the config.yml
