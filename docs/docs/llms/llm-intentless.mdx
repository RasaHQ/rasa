...
Using the `IntentlessPolicy`, our bot already provides a good 
question-answering experience, understanding many different ways 
...
You can specify the openai models used for generation and embedding.

#### Embedding Model
The `embeddings.model_name` property in the `config.yml` file configures the
...
#### LLM Model
The `llm.model_name` property in the `config.yml` file configures the
LLM model used:
