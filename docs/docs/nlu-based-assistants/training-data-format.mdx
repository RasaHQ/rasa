...
The [domain](../nlu-based-assistants/glossary.mdx#domain) uses
...
the definitions for [responses](../concepts/responses.mdx) and [forms](./forms.mdx).
See the [documentation for the domain](../concepts/domain.mdx) for information on how to format your domain file.
...
[NLU](../nlu-based-assistants/glossary.mdx#nlu) training data consists of example user utterances categorized by
[intent](../nlu-based-assistants/glossary.mdx#intent). Training examples can also include [entities](../nlu-based-assistants/glossary.mdx#entity). Entities are structured
...
Training examples are grouped by [intent](../nlu-based-assistants/glossary.mdx#intent) and listed under the
...
If you want to specify [retrieval intents](../nlu-based-assistants/glossary.mdx#retrieval-intent), then your NLU examples will look as follows:
...
[Entities](../nlu-based-assistants/glossary.mdx#entity) are structured pieces of information that can be extracted from a user's message.
...
1. [Responses](../concepts/domain.mdx#responses): start with `utter_` and
...
2. [Custom actions](../concepts/custom-actions.mdx): start with `action_`, run
...
A [form](../nlu-based-assistants/glossary.mdx#form) is a specific kind of custom action that contains the logic to loop over
...
Once defined, you should specify the [happy path](../nlu-based-assistants/glossary.mdx#happy-path--conversation-repair)
for a form as a [rule](./forms.mdx). You should include interruptions of forms or
other conversation repair in stories so that the model can
...
**[Slots](../concepts/domain.mdx#slots)** act as the bots memory.
Slots are **set** by either the default action [`action_extract_slots`](../concepts/default-actions.mdx#action_extract_slots) according to the
[slot mappings](../concepts/domain.mdx#slot-mappings) specified in the domain, or by custom actions.
...
[slot type](../concepts/domain.mdx#slot-types) and whether the value can or should
