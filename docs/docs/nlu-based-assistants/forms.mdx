...
looking for a [response](../concepts/responses.mdx) called
...
[Writing Stories / Rules for Conversation Repair cases](./forms.mdx#writing-stories--rules-for-conversation-repair-cases) on how to
...
### Writing Stories / Rules for Conversation Repair cases
...
```yaml-rasa
rules:
- rule: Example of conversation repair
  condition:
  # Condition that form is active.
  - active_loop: restaurant_form
  steps:
  # This case handles the case of an intent `chitchat`.
  - intent: chitchat
...
Forms are fully customizable using [Custom Actions](../concepts/actions.mdx#custom-actions).
...
You can implement a [Custom Action](../concepts/actions.mdx#custom-actions) `validate_<form_name>`
...
want to handle conversation repair differently, depending on what slot is
