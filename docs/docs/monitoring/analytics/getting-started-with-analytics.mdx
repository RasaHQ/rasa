...
These instructions assume you have running a PostgreSQL deployment in [CloudSQL](https://cloud.google.com/sql/docs/postgres).
You can however use [Datastream for BigQuery](https://cloud.google.com/datastream/docs) to stream data from PostgreSQL
hosted anywhere on any cloud.

[Prerequisites](https://cloud.google.com/datastream/docs/before-you-begin) to using DataStream include:
- creating a Google Cloud account where you create your Google Cloud project
- enabling billing for the project
- enabling the Datastream API
- assigning Datastream Admin role to your user account
...
You can follow closely the [quickstart Google Cloud documentation](https://cloud.google.com/datastream/docs/quickstart-replication-to-bigquery) on replicating data from PostgreSQL to BigQuery with Datastream.
...
You can synch your PostgreSQL deployment manually or via an automated [partner solution](https://docs.snowflake.com/en/user-guide/ecosystem-etl.html).

The instructions for manual synch include the following steps:
...
- Copy staged files to Snowflake tables using `COPY INTO` [command](https://docs.snowflake.com/en/sql-reference/sql/copy-into-table.html).
  ...
  Alternatively you can also benefit of [this plugin](https://cdap.atlassian.net/wiki/spaces/DOCS/pages/694157985/Cloud+Storage+to+Snowflake+Action) to load data to an existing Snowflake table.
