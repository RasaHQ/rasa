...
You can create a PostgreSQL instance using Amazon Relational Database Service (RDS).
...
Next, create a PostgreSQL DB instance which is the environment that will run your PostgreSQL database to which the analytics pipeline will stream the transformed conversational data.
The instructions you must follow are listed in the **Creating a PostgreSQL DB instance** [section](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/CHAP_GettingStarted.CreatingConnecting.PostgreSQL.html#CHAP_GettingStarted.Creating.PostgreSQL)
...
in the DBAPI format specified in **Connect an assistance** [section](#connect-an-assistant).

#### Redshift

If you prefer instead to set up Amazon Redshift as your choice of data lake, you will need to stream the data from the
PostgreSQL database within the Amazon RDS DB instance created at the [**PostgreSQL step**](#postgresql) to the Redshift target.
