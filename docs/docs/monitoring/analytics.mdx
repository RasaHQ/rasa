...
In the process, you must decide on a database authentication option from the below:
...
- [**Password and IAM DB authentication**](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/UsingWithRDS.IAMDBAuth.html) to use IAM users and roles for authentication fo database users.
- [**Password and Kerberos authentication**](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/postgresql-kerberos.html)

You will need these database credentials to build the DB URL in the DBAPI format specified in the [**Connect an assistance** section](#connect-an-assistant).
...
PostgreSQL source database within the Amazon RDS DB instance created at the [**PostgreSQL step**](#postgresql) to the Redshift target or connect directly to Amazon Redshift.
...
You must update the `RASA_ANALYTICS_DB_URL` to the Redshift target [endpoint](https://docs.aws.amazon.com/dms/latest/userguide/CHAP_Endpoints.Creating.html).
...
Before you do so, you should take into account the following considerations:

- Although Redshift is based on PostgreSQL, Redshift does not support all the latest PostgreSQL features, data types and functions: read the [Amazon documentation](https://docs.aws.amazon.com/redshift/latest/dg/c_redshift-and-postgres-sql.html) on core differences.
- This has an immediate knock-on effect on performance: streaming historical data directly to Redshift could take up to **REPLACE ME WITH INT** times longer than streaming directly to or via the PostgreSQL RDS instance.
...
a public API. [Internal tables](#internal-tables) should be considered private and may change
without notice.
...
#### `template_name` name of the template used to generate message
...
#### `event_id` id of the event of this action excecution
...
policy, a memoization policy, or a ML policy.
...
increasing sequence numbers. The sequence number of this slot change is
the same as the one of the underlying event.
...
the value is set to the one of the last change.
