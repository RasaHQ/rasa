...
To trace requests in Rasa Pro, you can use
[Jaeger](https://www.jaegertracing.io/) as a backend, or use
the [OTEL Collector (OpenTelemetry Collector)](https://opentelemetry.io/docs/collector/) or [Sentry](https://sentry.io)
...
Each of the backends support the following options:

- `service_name`: is the OpenTelemetry service name used in the traces
- `show_utterance`: will add the user utterance as the tracing tag `message_text`. It is `False` by default.
- `show_slots`: will add all slots and their values as tracing tags. It is `False` by default.
...
[Sentry](https://sentry.io) APM is supported via the [SentrySpanProcessor](https://docs.sentry.io/platforms/python/performance/instrumentation/opentelemetry/).

:::info Disable Telemetry
Because Rasa uses Sentry to collect anonymous usage telemetry, you must disable the usage stats to so that the tracing can be sent to your Sentry instance.
Set the following environment variables before you start rasa pro:
 
```sh
export RASA_TELEMETRY_ENABLED=false
export RASA_PRO_TELEMETRY_ENABLED=false
```
:::

To configure the Sentry span collector, specify the `type` as `sentry` along with your Sentry DSN endpoint.

```yaml
tracing:
  type: sentry
  endpoint: https://123@456.ingest.sentry.io/789
  server_name: test
  debug: True
