...
Slots which influence the conversation need to be added to your stories or rules. This also
applies for the case if the slot was set by a [custom action](./actions.mdx).
...
[run your action server](https://rasa.com/docs/action-server/running-action-server) in a separate terminal window.
