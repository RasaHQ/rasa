...
user is treated as a new user and the bot sends onboarding messaages. If it is false, the bot sends different messages
...
that controls how many steps in a dialgoue the model looks at to decide which
action to take next. 
