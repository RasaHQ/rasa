...
:::caution Removed in 3.0

Slot Auto-fill functionality was removed in 3.0 and replaced with a new explicit mechanism of
setting slots after every user message.
You will need to explicitly define slot mappings for each slot in the `slots` section of `domain.yml`.
In addition, stories must include the `slot_was_set` step.
Please read through the migration guide to update your assistant.
...
Slots set by a custom action or policy should contain a mapping of type `custom`, you can
find out more in [Custom Slot Mappings](./domain.mdx#custom-slot-mappings).

Please see [Mapping Conditions](./domain.mdx#mapping-conditions) if you need to indicate that
a slot mapping is applicable only within the context of a form.
...
:::note
Within the context of a form, a slot with `from_entity` mapping will be set only if the mapping
is unique, unless the slot is requested. You can find out more in [Forms](./forms.mdx#unique-entity-mapping-matching).
:::
...
A slot with `from_trigger_intent` mapping must always include mapping conditions
and be listed in the form's `required_slots`.
...
If you prefer to extract and set a slot only within the context of a form, you can indicate
the name of the form in the `conditions` key of a mapping. Conditions list the form name(s)
...
:::note
If conditions are not included in a slot mapping, the slot is eligible to be set universally
outside the context of any forms. However if a slot is also listed in a form's `required_slots`,
the slot can also be requested by the form if unset at that moment.
:::

### Custom Slot Mappings

A custom slot mapping is useful when none of the predefined mappings fit your use case.
For example, if you use custom policies, custom actions or the custom validation action to extract and validate slots,
you will have to define the slot mapping in the domain as `custom`.

In order to write your own extraction and / or validation code, we recommend you to create a custom action inheriting
from the `ValidationAction` interface from the Rasa SDK and defining the `extract_<slot_name>` and / or
`validate_<slot_name>` methods. The name of the action is already implemented and
should not be overridden: `action_validate_slot_mappings`. Please make sure to add this action name to the list of
...
If you have created a custom action that extends the rasa-sdk `Action` class instead of `ValidationAction`,
you will have to indicate this action in the mapping via the `action` key. For example:
