...
them for it. A [text slot](domain.mdx#text-slot) will influence the assistant's behavior
depending on whether the slot has a value. The specific value of
a `text` slot (e.g. Bangalore or New York or Hong Kong) doesn't make any difference.
...
By default all slot types will influence the conversation except slots of type
...
* **Description**
  ...
  Slots of type `any` are always ignored during conversations. The property `influence_conversation` cannot be set to `true` for this slot type. If you want to store
