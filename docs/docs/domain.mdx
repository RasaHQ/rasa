...
A custom slot class is implemented as an independent python module, 
separate from custom action code. It should be saved in a directory 
...
For example, say the code above is saved in "addons/my_custom_slots.py", a directory relative to your bot project:
...
The custom slot type's module path
