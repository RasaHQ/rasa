...
your bot should know about. The `Domain` also defines a `session_config` to configure
...
All the intents you've defined in your [NLU training data](./nlu-training-data.mdx) need
to be listed under the `intents` key in your `Domain`.
...
:::note
If you want these entities not to influence action prediction make the slots
with the same name of type `unfeaturized`.
...
* **Description**
  ...
  The value of `Text Slot` will not influence the dialogue. What matters is if the
  value was set at all or not.
...
* **Description**
  ...
  Both the value of `Boolean Slot` and the fact if it was set will influence
  the dialogue.
...
* **Description**
  ...
  What matters is the fact if the list length is zero or non-zero.
...
In the code below, you can see a slot class called `NumberOfPeopleSlot`.
...
Now Rasa Core needs some training stories to learn from how to handle the
