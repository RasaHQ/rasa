...
It specifies the `intents`, `entities`, `slots`, `responses`, `forms`, and `actions`
your bot should know about. It also defines a `session_config` to configure
conversation sessions.

As an example, here the domain of the 
[concertbot](https://github.com/RasaHQ/rasa/tree/master/examples/concertbot):
...
used in your NLU and conversation training data. 
...
If you list your intents as normal without this parameter, the entities will be
...
slot with the same name as the entity type, the slot will be set automatically
with the entity value. For example:
...
The `config` key in the domain file keeps track of one parameter, `store_entities_as_slots`.
When an entity is recognized by the NLU model, if the entity type matches a slot name,
this value defines whether the entity value should be placed in that slot.
By default, entities will auto-fill slots of the same name. 

You can turn off all slot auto-filling by changing the `store_entities_as_slots` parameter: 
...
You can also turn off this behavior for a specific slot by using the [`auto_fill`](./domain.mdx#slot-auto-fill)
