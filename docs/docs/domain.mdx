...
This change removes the implicit mechanism of setting slots via auto-fill which is replaced with a new explicit
...
Please read through the [migration guide](./migration-guide.mdx#slot-mappings) to update your assistant.
...
A slot with `from_trigger_intent` mapping must always be listed in the form's `required_slots`.
...
You can define custom slot mappings using custom actions when none of the predefined mappings fit your use case.
There are two options available through which you can achieve this:

A) implementing a custom action inheriting from the `Action` interface from the Rasa SDK: In addition to adding the
custom action name in the `actions` section of the domain, the custom slot mapping must specify the custom action
which implements the desired logic. For example:
...
B) implementing custom extraction and / or validation code in a custom action inheriting from the `ValidationAction`
interface from the Rasa SDK: The name of the action is already implemented and should not be overridden:
`action_validate_slot_mappings`. It is not required to specify this action name in the slot mapping, however you must
add `action_validate_slot_mappings` to the list of actions in the domain.
...
When an entity is recognized by the NLU model and it matches the `from_entity` slot mapping, `store_entities_as_slots`
