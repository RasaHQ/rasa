...
* `check_for_contradictions` (default: `true`): After training, the `RulePolicy` will
   perform a check to make sure that the rules don't contradict the stories and each
   other. The following snippet is an example for two contradicting rules:
 ...
 * `restrict_rules` (default: `true`): Rules are restricted to one user turn, but
    ...
    subsequent submission. It is **not recommended** to change this parameter to
    `false`. Overusing rules for purposes out of the [recommended use cases](rules.mdx)
