...
(TODO: why we do it in number 2. then?)
...
(see [Featurization of Conversations](./policies.mdx#featurizers) for details).
...
* `epochs`:
  ...
  policies:
  - name: TEDPolicy
    # if not an appropriate number, it'll use the default value
    epochs: 50
  ```
...
See [Featurization of Conversations](./policies.mdx#featurizers) for
