...
The default value is `None`, which means that all dialogue history since session
...
2. **Convert all the features into numeric vectors**:
    ...
    `SingleStateFeaturizer` uses rasa nlu to convert the intent and
    bot action or bot utterance names into numeric vectors.
    Please see [NLU Model Configuration](./model-configuration.mdx)
    for the details on how features are represented.
    Bot action or bot utterance names are featurized by nlu pipeline
    similarly as intent names.
...
represented as index of a corresponding action in the domain.
...
The smaller dialogues are padded with `0` for all features, indicating
...
If `max_history` is not specified, the algorithm takes into account
the whole length of a dialogue.
The smaller dialogues are padded with `0` for all features, indicating
no values for a policy.
