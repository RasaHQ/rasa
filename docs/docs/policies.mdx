...
* `use_gpu`:
  This parameter defines the use of GPU on the training. Default `use_gpu` for this policy is `True`, when set to `False` will use the CPU for the training instead.
