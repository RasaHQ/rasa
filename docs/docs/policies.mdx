...
* 2 - `MemoizationPolicy` or `AugmentedMemoizationPolicy`
...
It is recommended to use `LabelTokenizerSingleStateFeaturizer`
(see [Featurization](./policies.mdx#featurizers) for details).
...
* `epochs`:
  ...
  The lower the number of epochs the faster the model is trained. Here is how the config would look like:
...
[`FullDialogueTrackerFeaturizer`](#1-full-dialogue). We recommend to set `max_history` to a finite value in order to
use [`MaxHistoryTrackerFeaturizer`](#2-max-history) for **faster training**.
 ...
 * `restrict_rules` (default: `true`): Rules are restricted to one user turn, but
    ...
    Changing this parameter to `false` may result in unexpected behaviors.
...
<!-- TODO: below bullet points are not appearing - showing as one block of text instead --->
...
2. **Convert all the features into numeric vectors**:
    ...
    See our [NLU Model Configuration](./model-configuration.mdx) documentation
