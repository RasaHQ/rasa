...
slots and active loops set by actions are defined consistently for all rules. 
   The following snippet is an example of an incomplete rule:
   ...
   - if you want to cover the case when `action_search_venues` could not find
   anything and therefore `venues` should not be set,
   you should explicitly set the value of the slot to `null`.
   Then in the following story `RulePolicy` will predict `utter_venues_not_found`
   ...
   you should add `wait_for_user_input: false` in the the end of the rule snippet.
