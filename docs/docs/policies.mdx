...
`IntentTEDPolicy` helps you review conversations and also allows your bot to react
...
`IntentTEDPolicy` has the same model architecture as [`TEDPolicy`](./policies.mdx#ted-policy).
The difference is at a task level. Instead of learning the best action to be triggered next,
`IntentTEDPolicy` learns the set of intents that are most likely to be expressed by the user
...
by NLU is indeed likely to occur given the conversation context, `IntentTEDPolicy` does not trigger
...
`IntentTEDPolicy` should be viewed as an aid for `TEDPolicy`. Since, `TEDPolicy` is expected to improve
with better coverage of unique conversation paths that the assistant is expected to handle in the training data,
`IntentTEDPolicy` helps to surface these unique conversation paths from past conversations. For example, if you had
...
As soon as `deny` intent gets triggered, the policy handling the form will keep requesting for `cuisine` slot
...
To help you identify that a special story that handles the user's `deny` intent might be missing at this point,
 `IntentTEDPolicy` can trigger an `action_unlikely_intent` action right after `deny` intent.
 Subsequently, you can improve your assistant by adding a new training story that handles this particular case.

To reduce false warnings, `IntentTEDPolicy` has two mechanisms in place at inference time:

1. `IntentTEDPolicy`'s [priority](./policies.mdx#policy-priority) is intentionally kept lower than all
[rule based policies](./policies.mdx#rule-based-policies). This ensures that if an `action_unlikely_intent`
is predicted for the last intent, but there is a rule that can be triggered to predict
the next action, the ensemble of policies is not affected by it and the winning prediction is
still the one made by `RulePolicy`.

2. `IntentTEDPolicy` does not predict an `action_unlikely_intent` if the last predicted intent
isn't present in any of the training stories, which might happen if an intent is only used in rules.

As mentioned earlier, while `TEDPolicy` learns the next best action, `IntentTEDPolicy` learns the
next likely intent that a user can express. Given a conversation context, there can only be a single correct action,
whereas there can be multiple intents that are likely to occur in the same conversation context. Hence,
`IntentTEDPolicy` uses a multi-label cross entropy loss function during training. Moreover, it uses
the [`IntentMaxHistoryTrackerFeaturizer`](#3-intent-max-history) to treat intents as target labels instead of assistant actions or utterances.

#### Prediction of `action_unlikely_intent`

`IntentTEDPolicy` is invoked immediately after a user utterance and can either
trigger `action_unlikely_intent` or abstain (in which case other policies will predict actions).
To determine if `action_unlikely_intent` should be triggered, `IntentTEDPolicy` computes a score
 for the user's intent in the current dialogue context and checks if this score is below a
 certain threshold score.

This threshold score is learned at training time but can be adjusted at test time
via the `tolerance` parameter.  The higher the `tolerance`, the lower the intent's
score (the more unlikely the intent) needs to be before `IntentTEDPolicy` triggers
the `action_unlikely_intent` action.

**Configuration:**

You can pass configuration parameters to the `IntentTEDPolicy` using the `config.yml` file.
...
* `max_history`:
  ...
  since session restart is taken into account. If you want to limit the model
  to only see a certain number of previous
  dialogue turns, you can set `max_history` to a finite value.
  Please note that you should pick `max_history` carefully, so that the model has enough
  previous dialogue turns to create a correct prediction.
  Higher values of `max_history` can result in more frequent prediction of `action_unlikely_intent`
  as the number of unique possible conversation paths increases as more dialogue context is taken
  into account. Lowering the value of `max_history` will result in `action_unlikely_intent` being
  triggered less often but can be a stronger indicator that the corresponding conversation path
...
* `ignore_intents_list`:
    This parameter lets you configure `IntentTEDPolicy` to ignore the prediction of `action_unlikely_intent` for
...
* `tolerance`:
    ...
    It helps to adjust the threshold score used during prediction of `action_unlikely_intent`
    at inference time. Here, `0.0` means that the score threshold is the one
    that `IntentTEDPolicy` had determined at training time.
    A tolerance of `1.0` means that the threshold score is so low that `IntentTED` would not
    trigger `action_unlikely_intent` for any of the "negative examples" that it has encountered
    during training. These negative examples are combinations of dialogue contexts and user
    intents that are _incorrect_. `IntentTEDPolicy` generates these negative examples from your
...
|                                       |                        | all available features are used.                             |
+---------------------------------------+------------------------+--------------------------------------------------------------+
| ignore_intents_list                   | []                     | This parameter lets you configure `IntentTEDPolicy` to ignore|
...
|                                       |                        | is the one that `IntentTEDPolicy` had determined at training |
...
|                                       |                        | intents that are _incorrect_. `IntentTEDPolicy` generates    |
...
When [reviewing real conversations](./conversation-driven-development.mdx#review), users are
 encouraged to tune the `tolerance` parameter in `IntentTEDPolicy`'s configuration to reduce the number
of false warnings. As you increase the value of `tolerance` from `0` to `1` in steps of `0.05`,
the number of false warnings should decrease. However, increasing the `tolerance` will
also result in fewer triggers of `action_unlikely_intent` and hence more number of conversation
paths not present in training stories will be missing in the set of flagged conversations.

:::note
`IntentTEDPolicy` is only trained on [stories](./stories.mdx) and not [rules](./rules.mdx) from the training data.
...
1. The next most appropriate action to be triggered by the assistant. For example, [`TEDPolicy`](#tedpolicy) is trained to do this.
2. The next most likely intent that a user can express. For example, [`IntentTEDPolicy`](#intenttedpolicy) is trained to do this.

Hence, a tracker can be featurized to learn one of the above kind of labels.
Depending on the policy, the target labels correspond to bot actions or bot utterances
represented as index in a list of all possible actions or
set of intents represented as index in a list of all possible intents.

There are three different tracker featurizers:
...
[`IntentTEDPolicy`](#intenttedpolicy), the target labels that it creates is the set of intents that can be
...
list of target labels with `-1` values on the right to return an equally sized list of target labels
