...
* `max_history`:
  ...
  which means that the complete dialogue history since session restart is taken in the
  account. If you want to limit the model to see only certain number of previous
  ...
  See [Featurizers](#featurizers) for details.
  Here is how the config would look like:
