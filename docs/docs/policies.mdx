...
given the conversation context from training stories. It uses the learnt information at inference time by
checking if the predicted intent by NLU is the most likely intent. If the intent predicted
by NLU is indeed likely to occur given the conversation context, `IntentTEDPolicy` does not predict
any action. Otherwise, it predicts a special action named
[`action_unlikely_intent`](./default-actions.mdx#action_unlikely_intent) with a confidence of `1.00`.
...
`IntentTEDPolicy` helps in surfacing these unique conversation paths from past conversations. For example, if you had
...
to be filled, as there is no other way to handle input inside the form according to the training stories.
Reviewing such conversations are important so that you can account for them in the training data and
subsequently, the assistant is able to handle them gracefully. `IntentTEDPolicy` can trigger an
`action_unlikely_intent` action right after `deny` intent which is an indication that this conversation
has taken a user turn that is not well represented by training stories and needs a review.
...
isn't present in any of the training stories.
...
`IntentTEDPolicy` uses a multi-label cross entropy loss function during training.

#### Prediction of `action_unlikely_intent`

After training the internal ML model, `IntentTEDPolicy` does the following for each intent:

1. Collect conversation contexts from training stories for which the corresponding intent is not the correct intent.
2. Run these conversation contexts through the trained ML model and capture the predicted scores for the corresponding intent.
3. Compute multiple quantiles
...
* `ignore_intents_list`:
    ...
    are too many false warnings generated by `IntentTEDPolicy`.
...
Users are also encouraged to tune the `tolerance` parameter in `IntentTEDPolicy`'s configuration to reduce the number
...
paths not present in training stories missing from the set of flagged conversations. This is particularly helpful
when you are [reviewing real conversations](./conversation-driven-development.mdx#review) that
your assistant has processed.
