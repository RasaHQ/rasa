...
```yaml-rasa {10-13} title="stories.yml"
...
This threshold score is learned at training time but can be adjusted at inference time
...
* `ignore_intents_list`:
    This parameter lets you configure `IntentTEDPolicy` to not predict `action_unlikely_intent` for
...
* `tolerance`:
    ...
    A tolerance of `1.0` means that the threshold score is so low that `IntentTEDPolicy` would not
...
to tune the `tolerance` parameter in `IntentTEDPolicy`'s configuration to reduce the number
...
2. The next most likely intent that a user can express. For example, [`IntentTEDPolicy`](#intented-policy) is trained to do this.
...
[`IntentTEDPolicy`](#intented-policy), the target labels that it creates are the intents that can be
