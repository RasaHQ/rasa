...
2. The next most likely intent that a user can express. For example, [`IntentTEDPolicy`](#intentted-policy) is trained to do this.
...
[`IntentTEDPolicy`](#intentted-policy), the target labels that it creates are the intents that can be
