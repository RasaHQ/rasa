...
```python
@classmethod
def collect_intent_properties(cls, intents: List[Union[Text, Dict[Text, Any]]], entities: List[Text], roles: Dict[Text, List[Text]], groups: Dict[Text, List[Text]]) -> Dict[Text, Dict[Text, Union[bool, List]]]
```
...
```python
def cleaned_domain() -> Dict[Text, Any]
```
...
```python
def persist_clean(filename: Union[Text, Path]) -> None
```
...
```python
def as_yaml(clean_before_dump: bool = False) -> Text
```
...
```python
def slot_mapping_for_form(form_name: Text) -> Dict[Text, Any]
```
