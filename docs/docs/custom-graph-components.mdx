...
Rasa Open Source provides a variety of [NLU components](components) and
[policies](policies) out of the box. You can customize them or create your
...
- It has to be [registered](#registering-components-with-the-model-configuration)
...
Rasa Open Source uses the passed in [model configuration](model-configuration) to build a
...
[policies](policies) and [NLU components](components) become nodes within this graph.
...
[`GraphComponent`](custom-graph-components#the-graphcomponent-interface)
...
[NLU component](components) or a [policy](policies). If you are implementing
...
- `model_storage`: You can use this to persist and load your graph components. See the
...
- `execution_context`: This provides additional information about the current
   ...
   - `is_finetuning`: If `True` then the graph component is trained using
     [finetuning](command-line-interface#incremental-training).
...
```python {5-9} (docs/sources/data/test_classes/registered_component.py)
```
...
- `component_types`: This specifies what purpose your graph component fulfills within the
   ...
   * `ComponentType.MODEL_LOADER`: Component type for
      [language models](components#language-models). Graph components of this type provide
   ...
   * `ComponentType.MESSAGE_TOKENIZER`: Component type for
      [tokenizers](components#tokenizers). This graph component is run during training and
   ...
   * `ComponentType.MESSAGE_FEATURIZER`: Component type for
      [featurizers](components#featurizers). This graph component is run during training and
      inference. Rasa Open Source will use the graph component's `train` method if
      `is_trainable=True` is specified. Rasa Open Source will use
   ...
   * `ComponentType.INTENT_CLASSIFIER`: Component type for
      [intent classifiers](components#intent-classifiers). This graph component is run only
   ...
   * `ComponentType.ENTITY_EXTRACTOR`: Component type for
      [entity extractors](components#entity-extractors). This graph component is run only
      during training if `is_trainable=True`. The graph component is always run during
      inference.
      Rasa Open Source will use the graph component's `train` method if
      `is_trainable=True` is specified. Rasa Open Source will use
