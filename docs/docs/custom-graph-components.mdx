...
results. A simple example would be a component that averages the output of two
previous intent classifiers or a fallback classifier that sets it's threshold according
to the confidence of the intent classifier on training examples.
...
as that components' output needs to be consumed by another component and define how
the predictions of that intent classifier should be added to the message data specifying
the `process_training_data` method. Make sure to not overwrite the true labels for
the intents. Here's a template that shows how to subclass DIET for this purpose:
