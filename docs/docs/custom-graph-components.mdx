...
- `uses_tracker_during_prediction`: If the component requires the tracker during processing
   (e.g. to interpret a message in its context) this flag can be set. This flag is only
   relevant if the graph component is used during prediction. Defaults to `false`. If set 
   to `true`, the interface for the components `process` method changes to
   ```python
    def process(self, messages: List[Message],
                tracker: DialogueStateTracker) -> List[Message]:
        ...
    ```
