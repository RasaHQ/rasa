...
This page will introduce you to the basics of Rasa. It's intended for newcomers and for people
familiar with earlier versions of Rasa looking to understand our new approach.
...
The example project uses fixed responses defined in the `domain.yml` file. 
...
In DM2, the modules are split differently. 
...
Instead, the output of the *conversation handling* component is a command like `SetSlot("late_delivery", True)`.
...
However, the conversation handling component will still correctly output `SetSlot("late_delivery", True)`. 
