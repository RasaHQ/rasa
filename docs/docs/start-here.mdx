...
Once you have installed Rasa, you can run the `init` command to create a new Rasa project:
...
Run the `shell` command talk to your assistant on the command line:
...
If you want your assistant to sound a bit more natural, you can activate [contextual rephrasing] of responses.
...
To build more advanced flows, you can add conditional logic, link to other flows, and more. See [add link]
...
When a user answers "yes" or "no", an NLU model predicts an intent like `affirm` or `deny`.
...
Instead, the *conversation handling* component directly outputs the command `SetSlot("late_delivery", True)`.
...
you full control over your business logic, and the ability to override and customize all bahavior.
