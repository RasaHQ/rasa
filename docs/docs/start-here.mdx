...
The names used in this documentation are likely going to change. E.g. _DM2_
...
More information on the [`rasa train` command reference](./building-assistants/command-line-interface.mdx#rasa-train).
...
To build more advanced flows, you can add conditional logic, link to other flows, and more. Read more on how to handle [Business Logic with Flows](./building-assistants/flows.mdx)
...
- The _conversation handling_ component (name TBD) interprets the conversation so far and predicts
  a series of commands to progress the state of the conversation.
- The _business logic_ component executes those commands and the logic of your flows.
...
Instead, _conversation handling_ considers the conversation as a whole (not just one message),
and predicts the _intended effect_ of the user's message.
...
Instead, the _conversation handling_ component directly outputs a command to set the `late_delivery` slot to `True`.

The _conversation handling_ approach requires much less work to set up, since you don't need to worry about
...
The _conversation handling_ component will correctly output `SetSlot("late_delivery", True)`.
