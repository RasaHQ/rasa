...
Conditions are simple expressions that describe an event, and are used like variables in a logical expression, for example `action: utter_greet`. Operators allow you to combine conditions or sub-marker definitions which include nested operators and conditions.
...
```bash
rasa evaluate markers --config markers.yaml extracted_markers.csv all
```
