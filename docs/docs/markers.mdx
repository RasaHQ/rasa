...
Markers are conditions that allow you to describe and mark points of interest in dialogues. In Rasa, a dialogue or a user interaction is represented as a sequence of events,
which include bot actions that were executed, intents that were detected, and slots that were set.
Markers allow you to describe conditions over such events. When these conditions are met, the dialogues are `marked` for further analysis or inspection.
...
and surface relevant dialogues for review as part of [Conversation Driven Development (CDD)](https://rasa.com/blog/conversation-driven-development-a-better-approach-to-building-ai-assistants/).

Marker definitions are written in `YAML`. For example, here are markers that define dialogue completion and task success for Carbon Bot:
...
The next sections explain how write marker definitions, how to apply them to your existing dialogues, and what the output format looks like.

## Defining Markers

Markers are defined in a marker configuration file written in `YAML`.
...
- A marker definition can contain a single operator with nested conditions, as shown in `marker_mood_expressed`, `marker_cheer_up_failed`, `marker_bot_not_challenged`, and `marker_cheer_up_attempt`.
...
Markers are extracted from dialogues already stored a tracker store. To learn how to store interactions with your bot in a tracker store, read the [Tracker Store](https://rasa.com/docs/rasa/next/tracker-stores) page.
...
This script will process the marker definitions provided in the `marker.yml` file. To specify a different marker configuration file, use the optional `--config` argument.

By default, the script will validate the marker definitions against your bot's `domain.yml` file. To specify a different domain file, use the optional `--domain` argument.

By default, the script will retrieve the tracker store specified in your bot's `endpoint.yml` file. However, you can specify a different endpoint file using the optional `--endpoint` argument.
...
For more information on the usage of each strategy, type the following:
```bash
rasa evaluate markers <strategy> --help
```
replacing `<strategy>` with one of: `all`, `first_n`, and `sample_n`.
...
The script above will output the extracted markers in the specified output file: `extracted_markers.csv`. It will also compute overall summary statistics and per-session summary statistics and output them in the files `stats-overall.csv` and `stats-per-session.csv`.
You can change prefix `stats` in the file names using the optional argument `--stats-file-prefix `.
