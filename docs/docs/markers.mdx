...
Markers are conditions that allow you to describe and mark points of interest in dialogues to evaluate your bot.
...
Markers allow you to describe conditions over such events, and when the conditions are met, the relevant events are marked for further analysis or inspection.
...
- `num_preceding_user_turns`: an integer specifying the number of user turns preceding the event at which the marker applied.
