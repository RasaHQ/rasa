...
Markers can also contain operators, which allow you to express more complex behavior or
...
marker_mood_expressed:
    or:
        ...
        - intent: mood_great
```
The unique marker identified is `marker_mood_expressed`. The marker definition contains one operator, `or`,
and two event conditions `intent: mood_unhappy` and `intent: mood_great`.

### Event conditions

The following event conditions are supported:
...
as well as their negated forms:
...
- `not`: the condition did not apply. `not` can have only 1 condition.
...
- `never`: the listed conditions definitions never occurred.

### Marker Configuration

Markers are defined in a markers configuration file written in `yaml`. Here is an example created using mood bot with a new slot `name`:
...
marker_mood_expressed_and_name_not_provided:
    and:
        ...
        - or:
            ...
            - intent: mood_great
```
Note how each marker has a unique identifier, for example `marker_name_provided`.
A marker definition can contain a single condition, as shown in `marker_name_provided`.
Marker definitions can contain a single operator with nested conditions, as shown in `marker_mood_expressed`, `marker_cheer_up`, `marker_bot_not_challenged`, and `marker_faq`,
or multiple nested operators, as shown in `marker_mood_expressed_and_name_not_provided`.

Note that the values assigned the event conditions should exist in your bot's `domain.yml` file. For example, in `marker_mood_expressed`, the intents `mood_unhappy` and `mood_unhappy` exist the mood bot's `domain.yml` file.
...
Markers are extracted from the dialogues already stored in your [tracker store](https://rasa.com/docs/rasa/next/tracker-stores). Once you've created your Marker definitions in the Markers configuration file, you can extract them by running the following command:

```bash
rasa evaluate markers --config markers.yaml output.csv all
```
