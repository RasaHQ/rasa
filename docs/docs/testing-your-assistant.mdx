...
However, if a test story originally included an `action_unlikely_intent`, for example to ensure [a rule triggers what
happens after an `action_unlikely_intent`](./default-actions.mdx#customization-1) but the ensemble of
...
[`IntentTEDPolicy`](./policies.mdx#intented-policy) which employs a machine learning based model
