...
effective acceptance or integrations tests. The main features of end-to-end testing are:
...
- verifying response texts or names: you can assert that the bot response text (including interpolated responses with
slot values and conditional response variations) or `utter` name is as expected.
...
If `-o` flag is specified in the command, the results are also written to the `tests/e2e_results.yml` file, which will be
a list of test results with the following keys:
