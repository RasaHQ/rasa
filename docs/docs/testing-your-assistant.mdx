...
To test your assistant, you will need to write test conversations. These are like
...
You should always test your machine learning model on data which it hasn't been trained on.
...
Next, you can see how well your NLU model predicts the
test cases using:
...
To compare different pipeline configurations, specify them in the CLI command:
...
1. Create a global train/test split from `data/nlu.yml`.
...
The above process is repeated three times and the f1-scores for each exclusion percentage is recorded.
A graph with the mean and standard deviations of f1-scores across the three runs
for each exclusion percentage and configuration is plotted.

This will create multiple train/test splits from your data (`data/nlu.yml`) with each split
excluding a pre-defined percentage of data from training.
