...
abstract: Rasa Open Source lets you test dialogues end-to-end by running through
  test conversations. In addition to full test conversations, you can
  also test the dialogue management and the message processing (NLU)
  separately.
---

## Writing Test Conversations

To test your assistant completely, the best approach is to write test conversations. These are like
stories in your training data, but with the actual user utterance included as well.
...
Rasa Open Source looks for test conversations in all files with the prefix `test_`, e.g. `tests/test_stories.yml`.
...
See the [CLI documentation on rasa test command](./command-line-interface.mdx#rasa-test) for
more configuration options.

:::caution Testing Custom Actions
[Custom Actions](./custom-actions.mdx) are not executed as part of test conversations. If your custom
actions append any events to the conversation, this has to be reflected in your test conversation
(e.g. by adding `slot_was_set` events to your test conversation).
...
In addition to end-to-end testing, you can also test the message processing model(NLU) separately.
Once your assistant is deployed in the real world, it will be processing messages that it hasn't seen
in the training data. Hence, you should always set aside some part of your training data for testing.
You can [split your NLU data](./command-line-interface.mdx#create-a-train-test-split)
...
The above process is repeated with different percentages of training data excluded in step 2 above.
Additionally, the whole process is repeated three times for each configuration specified.
...
Inspecting the f1-score graph can help in understanding if you have enough data for your NLU model.

You can also specify different number of runs and different exclusion percentages with:
...
The `rasa test` script will produce a report, confusion matrix, and confidence histogram for your intent classification model.
...
The histogram that the script produces allows you to visualize the confidence distribution for all predictions,
with the volume of correct and incorrect predictions being displayed by blue and red bars respectively.
Improving the quality of your training data will move the blue histogram bars to the right and the
red histogram bars to the left of the plot.

#### Response Selectors

Similar to intent classifiers, the evaluation script outputs the same set of artifacts for response selectors -
report, confusion matrix, confidence histogram and errors.
If you have multiple response selectors in your pipeline, the evaluation for all of them is combined in a single report.

The report logs precision, recall and f1 measure for
each sub-intent of a [retrieval intent](./glossary.mdx#retrieval-intent), as well as providing an overall average.
You can save these reports as JSON files using the `--report` argument.

#### Entity Extraction

`rasa test` also reports recall, precision, and f1-score for each entity type that
...
evaluated by the `rasa test` script. If you use pretrained extractors like the `DucklingHTTPExtractor`,
...
Rasa Open Source counts a story as failed if at least one of the actions
...
Similar to how the NLU model was evaluated the above command trains the dialogue model
on multiple configurations and different amounts of training data.
...
This will evaluate each of the models on the provided stories in `stories_folder`
