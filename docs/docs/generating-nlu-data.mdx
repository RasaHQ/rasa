...
When you are creating a new intent in your assistant, you may initially face the
problem where you do not have enough training examples from your users to create a
reliable model. In this situation you can use data augmentation to supplement the
initial training dataset with synthetic examples.

Data augmentation is different from generating data from a script because data
augmenation is able to create the variability necessary to help a model generalise,
which a static script cannot do. Nonetheless, messages from real users are *always*
preferable to synthetic data of any kind. Therefore, you should always remove the
synthetic data once you have collected a sufficient amount of real user data to train
an effective model.

 The process for adding augmented data to your model is:


#### Recommended Use

1. While reviewing conversations from your users in
[Rasa X](https://rasa.com/docs/rasa-x/), you identify the need for a new intent.
2. You want to add this intent to your bot, but you don't have enough real user
messages yet.
3. You generate a small amount of paraphrases from the existing real user messages
with the paraphraser from [https://github.com/RasaHQ/paraphraser](https://github.com/RasaHQ/paraphraser).
4. You manually inspect the generated paraphrases and discard the ones that do not
express the new intent.
5. You use the data augmentation [CLI tool](./command-line-interface.mdx#)
(`rasa data augment nlu`) to evaluate the performance impact of adding the generated
data to your model.
6. Once you are happy with the results, you store the additional generated data
in a spearate file, e.g. `synthetic.yml`.
7. Once you have put this intent live, and more and more users express this intent,
you annotate more real user messages for it.
8. When you have enough training data from real user messages, you delete the
`synthetic.yml` file.

::: note
Always prefer data from real users over automatically generated ones.

Automatically generated data can be noisy, we recommend to always
manually check any generated paraphrases.
:::
