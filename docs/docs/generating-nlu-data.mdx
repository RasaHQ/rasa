...
Once you have obtained a first set of real user data, you can use data augmentation to
generate more "realistic" data from that first set of real data. Data augmentation is
different from static templates as it is able to introduce syntactic (e.g. word order)
and semantic (e.g. word choice) variation in your training data and thereby help the
model to generalise more effectively with only little amounts of real user data. It
needs to be noted though, that data augmentation can also introduce noise and therefore
requires a substantial amount of care and inspection before it is applied.

However, it is important to note that data augmentation is not a replacement for real
user data. It is intended as a way to rapidly bootstrap an NLU model in order for you to
share your assistant with real users more quickly - and therefore enable you to collect
conversations with real users more quickly.
