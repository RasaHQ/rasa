...
development. For building a great NLU model, this means:
...
this process of training & evaluating on user data will enable your model to generalize
...
you've added to your NLU pipeline. When different intents contain similar
words ordered in the same fashion, this can create confusion for the intent classifier.

To address this problem, you should consider separating by entities as opposed to by
intents. An example of this is when designing intents that are contextualized by
entities such as "How do I migrate to Rasa from IBM Watson?" A simple approach would
be to create separate intents for each migration entity, e.g. watson_migration,
dialogflow_migration, lex_migration, when all of these intents should actually be
grouped as a single migration intent with several possible entity values. By defining
a single intent, you also gain the ability to handle cases when no entity is provided,
e.g. "How do I migrate from Rasa?"

Extracted entity values can be used to affect the conversation flow using categorical
slots. For example:

TODO: replace with new story format
```story
- migration{"product": "IBM Watson"}
    - slot{"product": "Watson"}
    - utter_watson_migration

- migration{"product": "Dialogflow"}
    - slot{"product": "Dialogflow"}
    - utter_dialogflow_migration

- migration
   - ask_migration_product
```
...
data for an NLU model to generalize effectively. Rasa provides two great options for
...
Adding synonyms to the training data is useful for mapping certain entity values to a
single normalized entity. Synonyms, however, are not meant for improving your model's
entity recognition and has no effect on NLU performance.
...
Coming across misspellings is inevitable and your bot needs an effective way to
...
sufficient to account for misspellings. Adding a character-level featurizer provides
...
Use version control systems such as Github and Bitbucket to track changes to your
