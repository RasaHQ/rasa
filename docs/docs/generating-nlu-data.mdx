...
Most importantly, data augmentation is not a replacement for real user data.
It is intended as a way to rapidly generate additional training examples
in order to help you to create a new intent despite not yet having enough
real data. Once you have collected enough real data, we recommend you remove
the generated data.

#### Recommended Use

1. While reviewing conversations from your users in [Rasa X](https://rasa.com/docs/rasa-x/), you identify the need for a new intent. 
...
4. You use the data augmentation CLI tool (`rasa data augment nlu`) to evaluate
the performance impact of adding the generated data to your model.
5. Once you are happy with the results, you store the additional generated data
in a spearate file, e.g. `synthetic.yml`.
6. Once you have put this intent live, and more and more users express this intent, you annotate more
real user messages for it.
7. When you have enough training data from real user messages, you delete the `synthetic.yml` file.
