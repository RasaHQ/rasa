...
[LLMCommandGenerator](../concepts/dialogue-understanding.mdx), use the description to
...
- [generation prompt step](#generation-prompt): a prompt to generate
  a message that will be sent to the user
- [set slots step](#set-slots): a step to set slots
- [collect information step](#collect-information): a question to be
  asked to the user to collect user input in a slot
- [link step](#link): a step to link to another flow

### Branching Conditions

A branching condition can be used in the `next` property of a step.
...
- id: "some_question"
  ...
  next:
    - if: age < 18
      then: "under_18_step"
    - if: (age > 18 and (consent = 'yes' or consent = 'y'))
      then: "consent_accept"
...
Sets can be used to check for multiple conditions. For example:
...
[LLM Command Generator](../concepts/dialogue-understanding.mdx). The description of
...
A `branch` step allows you to branch the logic of a flow based on a condition.
Here is an example:
...
Every step type supports branching using the `next` property. The `branch`
step is **only needed if you want to branch based on a condition without executing
an action or sending a message**.

E.g. it can be used at the beginning of a flow to directly branch to a specific
step based on a condition.
...
By default, a `collect_information` step is skipped if the slot is already filled.
If a `collect_information` step should always be asked, no matter if the underlying slot
is already filled, you can set `ask_before_filling: true` on the `collect_information` step:
...
the `collect_information` step and fill the slot again. This ensures the confirmation
`collect_information` step is always asked.

#### Reset slot value after flow execution

You can configure the reset slot value behaviour after the flow completes by setting the optional
`reset_after_flow_ends` property to `true` or `false`. If the property is not defined for the
`collect` step, the default value will be `true`.
...
The following configuration would clear the answer to the `collect` step when a
flow is completed.

```yaml
- collect: "my_custom_slot" # slot name
  reset_after_flow_ends: true
```

#### Define a different response for the `collect` step

By default, the assistant will ask the user to fill the slot with the `utter_ask_{slot_name}` response. You can define
a different response for the `collect` step by adding a `utter` property to the step. The `utter` property
is optional and represents the name of the [response](./responses.mdx) the assistant will use to collect the information from the
user. Make sure to have this response included in the domain.
...
:::note
For more complex validation logic, you can also define slot validation in a [custom action](./custom-actions.mdx).
Note that this custom action must follow this naming convention: `validate_{slot_name}`.
