...
- id: "another_confirmation_step"
  collect_information: "my_custom_slot" # slot name
...
a different response for the `collect_information` step by adding a `utter` property to the step. The `utter` property
is optional and represents the name of the [response](./responses.mdx) the assistant will ask the user. Make sure to
have this response included in the domain.
