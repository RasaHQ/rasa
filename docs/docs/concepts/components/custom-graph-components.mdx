...
Rasa provides a variety of [NLU components](../../nlu-based-assistants/components.mdx) and
...
Rasa uses the passed in [model configuration](../../nlu-based-assistants/model-configuration.mdx) to build a
...
[policies](../policies.mdx) and [NLU components](../../nlu-based-assistants/components.mdx) become nodes within this graph.
...
<div align="center">
  <img
    alt="Visualization of the Rasa Graph Architecture"
    src={useBaseUrl("/img/graph_architecture.png")}
    width="100%"
  />
</div>
...
[NLU component](../../nlu-based-assistants/components.mdx) or a [policy](../policies.mdx). If you are implementing
...
- `component_types`: This specifies what purpose your graph component fulfills within the
  ...
  - `ComponentType.MODEL_LOADER`: Component type for
    [language models](../../nlu-based-assistants/components.mdx#language-models). Graph components of this type provide
  ...
  - `ComponentType.MESSAGE_TOKENIZER`: Component type for
    [tokenizers](../../nlu-based-assistants/components.mdx#tokenizers). This graph component is run during training and
  ...
  - `ComponentType.MESSAGE_FEATURIZER`: Component type for
    [featurizers](../../nlu-based-assistants/components.mdx#featurizers). This graph component is run during training and
    inference. Rasa will use the graph component's `train` method if
    `is_trainable=True` is specified. Rasa will use
  ...
  - `ComponentType.INTENT_CLASSIFIER`: Component type for
    [intent classifiers](../../nlu-based-assistants/components.mdx#intent-classifiers). This graph component is run only
  ...
  - `ComponentType.ENTITY_EXTRACTOR`: Component type for
    [entity extractors](../../nlu-based-assistants/components.mdx#entity-extractors). This graph component is run only
    during training if `is_trainable=True`. The graph component is always run during
    inference.
    Rasa will use the graph component's `train` method if
    `is_trainable=True` is specified. Rasa will use
  ...
  - `ComponentType.POLICY_WITHOUT_END_TO_END_SUPPORT`: Component type for
    ...
    (see [end-to-end training](../../nlu-based-assistants/stories.mdx#end-to-end-training) for more information).
...
- `model_from`: Specifies if a pretrained
  [language model](../../nlu-based-assistants/components.mdx#language-models) needs to be provided to the `train`,
  ...
  [SpacyNLP](../../nlu-based-assistants/components.mdx#spacynlp) language model to your other NLU components.
...
[model configuration](../../nlu-based-assistants/model-configuration.mdx). The only change is that you have to specify
...
When you [define metadata for your intent examples in your training data](../../nlu-based-assistants/training-data-format.mdx#training-examples),
...
The following is the example of a dense [message featurizer](../../nlu-based-assistants/components.mdx#featurizers)
