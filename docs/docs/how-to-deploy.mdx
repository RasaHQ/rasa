...
The recommended deployment method described below make it easy to share your assistant
...
The [Rasa OSS Helm chart](https://github.com/RasaHQ/helm-charts/tree/main/charts/rasa) is the supported method to deploy
your assistant. It can also allows you to connect Rasa OSS with a separate Rasa X or
Rasa Enterprise deployment and is production ready. See the [Alternative Deployment Methods](https://rasa.com/docs/rasa/how-to-deploy#alternative-deployment-methods)
for details on building your bot locally, with Docker and deploying with docker-compose.
...
## Use Rasa Ephemeral Installer to create a local Kubernetes cluster

If you don't have a Kubernetes cluster, but you'd like to try out the [rasa helm chart](https://artifacthub.io/packages/helm/rasa/rasa), you can use
Rasa Ephemeral Installer that installs all required components and creates a Kubernetes cluster via [`kind`](https://kind.sigs.k8s.io/) on your machine.

1. Use the REI script to install all components. Simply run the command below.
...
2. Follow [the installation steps](#installation) for the rasa helm chart.

:::note
The Rasa Ephemeral Installer doesn't support Windows.
:::
...
It is also possible to deploy a Rasa assistant using Docker Compose. To do so, you can build your
