...
In addition to a manual step to create a new Action Server Image. You can use [Rasa Action Server GitHub Action](https://github.com/RasaHQ/action-server-gha) to automate image builds.
...
1. Create GitHub Secrets with login name and password for DockerHub.
   [Here](https://docs.github.com/en/actions/configuring-and-managing-workflows/creating-and-storing-encrypted-secrets#creating-encrypted-secrets-for-a-repository) you can find details on how to create encrypted secrets for a repository.
...
The GitHub Action workflow below builds a new docker image every time when new changes are pushed into the `master` branch.
...
4. Now, you can use your new brand docker image or, for example, extend your workflow.
The example below shows how to extend your workflow by a new step that updates a Rasa X deployment based on [Helm Chart](https://rasa.com/docs/rasa-x/installation-and-setup/customize/#adding-a-custom-action-server)
...
More examples on how to use and customize [Rasa Action Server GitHub Action](https://github.com/RasaHQ/action-server-gha) you can find in the [Rasa Action Server GitHub Action](https://github.com/RasaHQ/action-server-gha) repository.
