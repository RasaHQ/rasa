...
Let's assume you want to output a different sentence depending on what the user is saying about his location. E.g.
if the user informs you that he just arrived from London, you might want to ask how he liked London. But if he
is saying that is on his way to Madrid, you might want to wish him a good stay. You can achieve this with the
following two stories.

```yaml-rasa
- story: The user mentioned that he just arrived from another city.
  steps:
    - intent: greet
    - action: utter_greet
...
- story: The user is saying that he is going to another city.
  steps:
    - intent: greet
    - action: utter_greet
    - intent: inform_location
