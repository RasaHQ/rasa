...
migrating from Rasa Open Source 1.0 to 2.0.
...
At the end of this loop, the default action will run the custom validation action
`action_validate_slot_mappings` if it is present in the domain actions. Otherwise it will return the already extracted slots.
Note that any event returned by custom actions, which is not of type `SlotSet` or `BotUttered` is not permitted and
will be filtered out from the output with which the `DialogueStateTracker` instance gets updated.

This default action replaces the slot extraction previously executed by `FormAction`. If you wish to set a slot based on
information extracted from intents that trigger forms, you will have to explicitly specify a mapping that does not
contain the `conditions` key. A failure to do so will cause the form to re-ask for this slot if it is part of the
form's `required_slots`.

:::note
If `action_default_fallback` is the next action predicted and executed by the assistant, this will result in an
