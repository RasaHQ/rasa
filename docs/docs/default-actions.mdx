...
Rasa Open Source triggers `action_unlikely_intent` if [`IntentTEDPolicy`](./policies.mdx#intenttedpolicy)
predicts it and no other [rule-based policy](./policies.mdx#rule-based-policies) has predicted an action. You can control how often this action is predicted
by tuning the [`tolerance`](./policies.mdx#intenttedpolicy) parameter of `IntentTEDPolicy`.
