...
Rasa Open Source triggers `action_unlikely_intent` via [`IntentTEDPolicy`](./policies.mdx#intented-policy).
You can control how often this action is predicted by tuning the [`tolerance`](./policies.mdx#intented-policy)
parameter of `IntentTEDPolicy`.
...
Alternatively, you can also override it's behaviour as a [`custom action`](./custom-actions.mdx):

```python
class ActionUnlikelyIntent:
...
all policies which are trained on only story data, for example - `TEDPolicy`, `IntentTEDPolicy`,
