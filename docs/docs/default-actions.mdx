...
Rasa Open Source triggers `action_unlikely_intent` via [`IntentTEDPolicy`](./policies.mdx#intentted-policy).
You can control how often this action is predicted by tuning the [`tolerance`](./policies.mdx#intentted-policy)
