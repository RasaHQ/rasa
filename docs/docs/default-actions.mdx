...
information extracted from intents that trigger forms, you must explicitly specify a mapping that does not
contain the `conditions` key. A slot mapping with `conditions` applies only once the specified form is active.
`action_extract_slots` runs directly after each user message, and thus before activation of the form.
