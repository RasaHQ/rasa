...
You can find the method's signature [here](https://github.com/RasaHQ/rasa/blob/main/rasa/core/lock_store.py#L59).
- `save_lock`: commit `lock` object to storage; requires `lock` parameter which is of type `TicketLock`and returns `None`.
You can find the method's signature [here](https://github.com/RasaHQ/rasa/blob/main/rasa/core/lock_store.py#L67).
- `delete_lock`: deletes lock for `conversation_id` from storage; requires `conversation_id` text parameter and returns `None`.
You can find the method's signature [here](https://github.com/RasaHQ/rasa/blob/main/rasa/core/lock_store.py#L63).
