...
`ConcurrentRedisLockStore` is the new implementation of the abstract interface `LockStore` that uses Redis as a persistence layer.
You can read through the [migration guide](./migration-guide.mdx#concurrentredislockstore) to learn how to switch to this lock store implementation.
