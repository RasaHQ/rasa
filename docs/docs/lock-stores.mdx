...
Your custom lock store class will have to override these base class methods that require implementation:
...
- `save_lock`: commit `lock` object to storage; requires `lock` parameter which is of type `TicketLock`.
