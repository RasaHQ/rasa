...
* **Description**
  ...
  `ConcurrentRedisLockStore` uses Redis as a persistence layer for instances of issued [tickets](https://rasa.com/docs/rasa/reference/rasa/core/lock/#ticket-objects) and the last issued ticket number.
  This Redis implementation of the `LockStore` can handle messages received in parallel for the same conversation by different Rasa servers.
  Therefore, `ConcurrentRedisLockStore` ensures that only one Rasa instance can handle a conversation at any point in time.
  This is the recommended lock store for running a replicated set of Rasa servers.

  The ticket number initialization begins at 1, in contrast to `RedisLockStore` which begins at 0.
...
* **Parameters**
  ...
  * `concurrent_mode`: boolean required to be set as `True` in order to make use of concurrent message handling for the same conversation.
