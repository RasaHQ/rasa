...
sidebar_label: Entity Extraction
title: Entity Extraction
description: Use open source named entity recognition like Spacy or Duckling and customize them according to your needs to build contextual AI assistants
---
...
|`RegexEntityExtractor`  |                  |N/A                                              |extract entities using the lookup table and regexes defined in the training data |
...
some training data.
See [Training Data Format](./training-data-format.mdx) for details on how to include entities in your training data.
...
See [Training Data Format](./training-data-format.mdx) for details on how to define entities with roles and groups in your training data.
...
:::note
Composite entities are currently only supported by the [DIETClassifier](./components.mdx#diet-classifier) and [CRFEntityExtractor](./components.mdx#crfentityextractor).
...
[Forms](../core/forms.mdx) or use [Custom Actions](../core/actions.mdx#custom-actions) to extract the corresponding entity directly from the tracker.
...
(see [DucklingHTTPExtractor](./components.mdx#ducklinghttpextractor)).

## Regular Expressions (regex)

You can use regular expressions to help the CRF model learn to recognize entities.
In your training data (see [Training Data Format](./training-data-format.mdx)) you can provide a list of regular expressions, each of which provides
...
<a aria-hidden="true" tabIndex="-1" className="anchor enhancedAnchor" id="entity-extraction-custom-features"></a>
