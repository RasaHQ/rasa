...
what pipeline is best for your use case, check out [Choosing a Pipeline](./choosing-a-pipeline).
...
via the instructions [here](./choosing-a-pipeline#section-supervised-embeddings-pipeline).

After you define the `supervised_embeddings` processing pipeline and generate some [NLU training data](./training-data-format)
...
With the `pretrained_embeddings_spacy` [pipeline](./choosing-a-pipeline#section-pretrained-embeddings-spacy-pipeline), you can use spaCy’s
...
You can also pre-train your own word vectors from a language corpus using [MITIE](./choosing-a-pipeline#section-mitie-pipeline). To do so:
...
3. Set the path of your new `total_word_feature_extractor.dat` as the `model` parameter in your
   [configuration](./choosing-a-pipeline#section-mitie-pipeline).
