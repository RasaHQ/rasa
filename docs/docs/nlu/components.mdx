...
<a aria-hidden="true" tabIndex="-1" className="anchor enhancedAnchor" id="intent-classifiers"></a>
...
<a aria-hidden="true" tabIndex="-1" className="anchor enhancedAnchor" id="fallback-classifier"></a>

### FallbackClassifier

* **Short**

  Classifies a message with an intent ``nlu_fallback`` if the NLU intent classification
  scores were ambiguous.
...
* **Description**
    ...
    of the `FallbackClassifier`. In addition it can also predict the fallback intent in
    case the confidence scores of the two top ranked intents are closer than the the
    `ambiguity_threshold`.
