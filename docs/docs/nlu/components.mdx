...
out [Custom NLU Components](../api/custom-nlu-components).
...
* **Description**
  ...
  Creates tokens using the ConveRT tokenizer. Must be used whenever the [ConveRTFeaturizer](./components#convertfeaturizer) is used.
...
* **Requires**
  ...
  [HFTransformersNLP](./components#hftransformersnlp)



* **Description**

  Creates tokens using the pre-trained language model specified in upstream [HFTransformersNLP](./components#hftransformersnlp) component.
  Must be used whenever the [LanguageModelFeaturizer](./components#languagemodelfeaturizer) is used.
...
* **Requires**
  ...
  [ConveRTTokenizer](./components#converttokenizer)
...
* **Requires**
  ...
  [HFTransformersNLP](./components#hftransformersnlp) and [LanguageModelTokenizer](./components#languagemodeltokenizer)
...
* **Description**
  ...
  Creates features for entity extraction, intent classification, and response selection.
  Uses the pre-trained language model specified in upstream [HFTransformersNLP](./components#hftransformersnlp) component to compute vector
  representations of input text.
...
* **Configuration**
  ...
  Include [HFTransformersNLP](./components#hftransformersnlp) and [LanguageModelTokenizer](./components#languagemodeltokenizer) components before this component. Use
  [LanguageModelTokenizer](./components#languagemodeltokenizer) to ensure tokens are correctly set for all components throughout the pipeline.
...
* **Description**
  ...
  Regex features for entity extraction are currently only supported by the [CRFEntityExtractor](./components#crfentityextractor) and the
  [DIETClassifier](./components#diet-classifier) components!
...
* **Requires**
  ...
  `tokens` for user message and [MitieNLP](./components#mitienlp)
...
* **Description**
  ...
  You can find the detailed description of the [DIETClassifier](./components#diet-classifier) under the section
...
* **Requires**
  ...
  [MitieNLP](./components#mitienlp) and `tokens`
...
* **Requires**
  ...
  An extractor from [Entity Extractors](./components#entityextractors)
...
* **Description**
  ...
  a set of candidate responses. The prediction of this model is used by [Retrieval Actions](../core/retrieval-actions).
  It embeds user inputs and response labels into the same space and follows the exact same
  neural network architecture and optimization as the [DIETClassifier](./components#diet-classifier).
  ...
  `0.0`. This might happen if you only use the [CountVectorsFeaturizer](./components#countvectorsfeaturizer) with a `word` analyzer
...
* **Configuration**
  ...
  The algorithm includes almost all the hyperparameters that [DIETClassifier](./components#diet-classifier) uses.
