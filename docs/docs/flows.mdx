...
and should describe the purpose of the flow. Other components, such as the 
[LLMCommandGenerator](./llms/llm-command-generator.mdx), use the 
...
[entry prompt step](./flows.mdx#entry-prompt). 
Entry prompts trigger an LLM call with the given context and a yes/no question 
to determine whether to start the flow.
