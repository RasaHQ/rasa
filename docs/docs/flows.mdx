...
The `description` field is an optional human readable description of the flow.
...
proceed to step `"3"` or `"4"`, respectively. This allows you to create complex
and dynamic conversational flows that adapt to user inputs.

The `next` field can be either a string or a list of conditionals. If it is a
string, it is interpreted as the ID of the next step to be executed. If it is a
list of conditionals, the flow will proceed to the step that matches the first
conditional.
...
- [user message step](./flows.mdx#user-message): a user utterance that will
  trigger the flow
...
How a flow can be triggered, depends on the first step in the flow. If the first
step is a [user message step](./flows.mdx#user-message), the flow will be
triggered by a user utterance that matches the intent or entities specified in
the step.

If the first step is an [entry prompt step](./flows.mdx#entry-prompt), the flow
will be triggered if the prompt will evaluate to 'yes' when send to an LLM.

Flows that start with a different step cannot be triggered by a user directly.
They can only be triggered by other flows, e.g. by linking to them.

## Step Types

Every step in a flow has a type. The type determines what the step does and 
...
Entry prompts should be used infrequently, as the prompt will be sent to an LLM
for every incoming user message. Having to many entry prompts can slow down the
bot.
