...
so the name must not be changed. If you are using Rasa SDK, you should extend the Rasa SDK [`ValidationAction` class](https://rasa.com/docs/action-server/next/validation-action#how-to-subclass-validationaction).
If you are using a different action server, you will need to implement an action class with equivalent functionality to
the Rasa SDK class. Please see [the action server docs](https://rasa.com/docs/action-server/next/validation-action#validationaction-class-implementation) for details.
...
If you are using Rasa SDK, the custom action should inherit from the Rasa SDK [`FormValidationAction` class](https://rasa.com/docs/action-server/next/validation-action#formvalidationaction-class).
If you are not using Rasa SDK, you will need to implement an action or action class with equivalent functionality to the
`FormValidationClass` in your custom action server. Please see [the action server docs](https://rasa.com/docs/action-server/next/validation-action#formvalidationaction-class-implementation) for details.
...
events for custom slot extraction. Use this option if neither [`action_validate_slot_mappings`](#action-validate-slot-mappings) or
[`validate_<form name>`](#validate-form-name) meet your needs.
