...
Rasa SDK provides three base classes that you can use to inherit from in order to define your custom slot extraction and
validation code:

## `Action` class

You can use this class to build a [custom action](./custom-actions.mdx) that is limited to returning only
`SlotSet` and `BotUttered` events. Any other event type will be filtered out by the default action
[`action_extract_slots`](./default-actions.mdx#action_extract_slots).
The name of the custom action will have to be specified in the `action` key of the
...
You can extend the `ValidationAction` in the Rasa SDK to define custom extraction and / or validation of slots that
...
You must write functions named `validate_<slot_name>` for every predefined slot that you need validated.
...
If you prefer that certain custom slot mappings should only be set within the context of a form,
make sure your custom action inherits from `FormValidationAction` rather than from `ValidationAction`.
It is not required to use mapping conditions for custom slot mappings defined in a custom action extending
`FormValidationAction`.
Note that this custom action will only run if the form is activated.

To learn more about how to implement this class, see Forms [Advanced Usage](./forms.mdx#advanced_usage).
