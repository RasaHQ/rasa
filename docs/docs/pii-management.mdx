...
retain a source of truth for pristine data required for non-repudiation purposes. In addition, because the tracker store
data plays a vital role in the assistant's dialogue management at inference time, anonymizing data in flight could have
undesirable consequences for the assistant's action predictions.
...
The `language` field specifies the language of the text to be anonymized. The `model_name` and `model_provider` fields
specify the name and provider of the Presidio model to be used for anonymization.
The available model providers are `spacy`, `stanza` and `transformers`.
...
You must install the model name you declare to the `model_name` field in your Rasa Pro environment.
...
In the case of the `transformers` model provider, you must install the model name you declare to the `model_name` field
in your Rasa Pro environment. For example, if you declare `model_name: dslim/bert-base-NER` in `endpoints.yml`, you must
install the `dslim/bert-base-NER` model in your Rasa Pro environment. You can find model installation instructions for
...
The `rule_lists` section contains a list of anonymization rule lists. Each rule list must have a unique `id` and a list
of `rules`. Each rule must have an `entity` field and a `substitution` field. The `entity` field specifies the Presidio
entity type to be anonymized and must be in uppercase.

The `substitution` field specifies the anonymization method to be used. Currently, the
following anonymization methods are supported: `text`, `mask`, and `faker`. If no substitution method is specified,
the default substitution method is `mask`.
...
If the `value` field is not specified, the original entity value to be anonymized will be replaced with a null value.

The `faker` anonymization method uses the [Faker](https://faker.readthedocs.io/en/master/) library to generate fake data.
