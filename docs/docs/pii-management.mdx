...
to the Kafka topic that is mapped to the first Kafka anonymization topic that contains the `rasa_x_consumer` key-value pair.
...
is done asynchronously as a background task and completes after the assistant has already predicted and executed the
bot response.
