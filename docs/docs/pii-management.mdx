...
entity type to be anonymized. The `substitution` field specifies the anonymization method to be used. Currently, the
following anonymization methods are supported: `text`, `mask`, and `faker`. The `value` field is only required for the
