...
Your custom tracker store class will also have to override three base class methods that require implementation:
- `save`: saves the conversation to the tracker store.
- `retrieve`: retrieves tracker for the latest conversation session.
- `keys`: returns the set of values for the tracker store's primary key.
