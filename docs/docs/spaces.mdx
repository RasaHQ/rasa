...
abstract: Spaces are a way to modularize your assistant that increases isolation between
subparts, and classification performance for intents and entities that are only
...
potential for conflicts between intents, entities, and other [primitives](./glossary.mdx#rasa-primitive) grows alongside.
...
and deactivation of groups of [rasa primitives](./glossary.mdx#rasa-primitive).
Every of these groups is called a space.
...
a single assistant. Oftentimes, this leads to a multitude of forms, entities, and inform
...
Because the Spaces feature splits a rasa bot into multiple parts, it is interacting with almost
every of the existing features of rasa. We made it work with almost all of them, but
there are some exceptions:

* if you have created extensive customization and introduced your own data hierarchies
or post-processing steps, these might not be compatible with Spaces.
* Spaces currently do not support stories. This is a big limitations for many
existing bots. However, because stories and their existing policies work with a fixed
event horizon (`max_history`), they are at this point not compatible with the idea of
being able to describe encapsulated units of logic well.
* Entity extraction is currently limited to an adapted version of
the `CRFEntityExtractor` which works with Spaces boundaries. You can find more on this
...
subspaces for transferring money, investment, and credit card payments. Each path value
...
is prefixed with this space's name. For example, an intent
`ask_transfer_charge` in the `transfer_money` space would become
...
For responses, we use infixing, because their name has to start with the `utter_`
prefix. `utter_transfer_charge` in the `transfer_money` space would become
`utter_transfer_money.transfer_charge`, introducing the space name inside the original name.
...
Another exception are [rules](./rules.mdx). They don't have a name that can be prefixed. Instead,
we add a condition to each rule that it is only applicable while it's space is active.

### How does space activation work?

A space is activated when any of their entry intents is predicted. Only a single space
can be active at a given time. So when space A is active and an entry intent of space B
...
classification components](./components.mdx#intent-classifiers). It accesses the [tracker](./sdk-tracker.mdx)
...
returned by your action. All of this ensures that from the viewpoint of your custom
...
custom actions from the main spaces are not inherited by specialised spaces.

### Response Selection

Response selection works as before. The only small difference is that in your
`config.yml` you'll need to specify the retrieval intent including its final prefix.
