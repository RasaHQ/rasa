...
data. One file can contain multiple keys, but each top-level key can only appear
...
Training data files with a Rasa Open Source version greater than what you have
...
- steps:
  ...
  - action: respond_faq
```
Test stories use the same format as the story training data.
...
The `metadata` key can contain arbitrary key-value data that is tied with an example.
The `metadata` key is accessible by the components in the NLU pipeline.
