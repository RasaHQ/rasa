...
Each step can be one of the following:
  ...
  - A [user message](#user-messages), represented by a combination of **intent** and **entities**, or **user**.
...
There are two ways to specify user messages. You can provide the `intent:`
key and an optional `entities:` key, or you can provide the `user:` key.
...
Only `RulePolicy` and `TEDPolicy` allow end-to-end training.
For end-to-end inputs, intents classified in the NLU pipeline and extracted entities
are ignored.
...
- `TEDPolicy` passes user text through additional Neural Network to create
  hidden representation of the text. In order to obtain robust performance you
  need to create sufficient number of stories that have different user texts
  during this dialogue turn.
...
In addition, you can add entity tags that will be extracted by the end-to-end policy.
...
`I want to check my credit balance`. By using the syntax from the NLU training data
`[credit](account_type)`, you can mark `credit` as an entity of type `account_type`.
...
The only difference to creating `utter_` template is how `TEDPolicy` featurizes this
utterance. In case of a template, `TEDPolicy` sees only the name of the template, while
if you provide actual utterance using `bot:` key, `TEDPolicy` will featurize it as textual
input depending on the NLU configuration.
...
A story with only bot utterance might look like this:
