...
sidebar_label: NLU Data
title: NLU Data
description: Read more about how to format training data with Rasa NLU for open source natural language processing.
---

TODO: describe each different piece of the training data in a processable format. move usage information
about e.g. improving classification to NLU best practices.
...
To understand what the labels `role` and `group` are for, see section [Entities Roles and Groups](./entity-extraction.mdx#entities-roles-groups).
...
the user message (using the [RegexEntityExtractor](./components.mdx#regexentityextractor)). For the zipcode example it might look like this:
...
:::note
If you are using lookup tables in combination with [RegexFeaturizer](./components.mdx#regexfeaturizer), there must be a few examples of matches
...
`EntitySynonymMapper` component (see [Components](./components.mdx)).
