...
We can also provide lookup values in an external text file as well. The supplied lookup table files must be in a newline-delimited format. For example, data/lookups/plates.txt may contain:
```
tacos
beef
mapo tofu
