...
Each step can be one of the following:
  ...
  - A [user message](#user-messages), represented by **intent** and **entities** or **user**.
...
For example, the following stories contains the user utterance
`I want to check my credit balance`. By using the syntax from the NLU training data `[credit](account_type)`, we mark `credit` as an entity of type `account_type`.
