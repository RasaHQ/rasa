...
:::note
Composite entities are currently only supported by the [DIETClassifier](./components/intent-classifiers.mdx#diet-classifier) and [CRFEntityExtractor](./components/entity-extractors.mdx#crfentityextractor).
