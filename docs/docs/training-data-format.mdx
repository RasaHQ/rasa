...
[markdown NLU data](https://legacy-docs-v1.rasa.com/nlu/training-data-format/) and
[markdown stories](https://legacy-docs-v1.rasa.com/core/stories/).
:::
...
[Entities](glossary.mdx#entity) are structured pieces of information that can be extracted from a user's message. For entity extraction to work, you need to either specify training data to train an ML model or you need to define [regular expressions](#regular-expressions-for-entity-extraction) to extract entities using the [`RegexEntityExtractor`](components.mdx#regexentityextractor) based on a character pattern.
...
[`EntitySynonymMapper`](components.mdx#entitysynonymmapper) component. You
