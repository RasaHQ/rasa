...
(read more about other [featurized slots](./domain.mdx#slots), whereas slots with
the property `influence_conversation` set to `false` are only used for storing information.
