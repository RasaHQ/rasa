...
(and entities when necessary), or as user textual inputs directly,
while the assistant's responses and actions are expressed as action names or text outputs.
...
While writing stories, you have two options to represent user input.
...
While writing stories, you have two options to represent bot actions.

1. All actions executed by the bot, including [responses](./responses.mdx) are listed
...
2. You can put bot textual utterances directly in the stories under `bot` key.
