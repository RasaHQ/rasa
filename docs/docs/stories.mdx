...
See the [training data format](./training-data-format.mdx#stories)
for detailed description of how to write stories.
...
or with separate `utter_` templates in the domain file.
Instead, you can simply put the texts of the user and/or bot messages directly in the stories.
See the [training data format](./training-data-format.mdx#end-to-end-training-stories)
...
It means that you can have mixed stories with some steps defined by actions
and other steps defined by user or bot utterance directly.

We call it end-to-end training because actual text is fed into policies.

:::note nlu is ignored during e2e
...
:::note next utterance selection
Rasa policies are trained for next utterance selection.
The only difference to creating `utter_` template is how `TEDPolicy` featurizes
bot utterances.
In case of a template, `TEDPolicy` sees only the name of the template, while
...
Which can help in case of similar utterances in slightly different situations.
But can also make things harder to learn.

:::

:::note increased train time
