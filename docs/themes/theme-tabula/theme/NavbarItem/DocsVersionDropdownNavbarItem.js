...
export default function DocsVersionDropdownNavbarItem({ mobile, docsPluginId, nextVersionLabel, ...props }) {
  ...
  const dropdownLabel = mobile ? 'Versions' : versionLabel(dropdownVersion, nextVersionLabel);
  const dropdownTo = mobile ? undefined : getVersionMainDoc(dropdownVersion).path;
  return <DefaultNavbarItem {...props} mobile={mobile} label={dropdownLabel} to={dropdownTo} items={items} />;
}
