...
what happened before. a
 a
You can alter this behavior with the ``--augmentation`` flag. a
...
expected outcome in the case of a tie. They look like this, where higher numbers have higher priority: a
 a
    ...
    | 2. ``MappingPolicy`` a
    | 1. ``TEDPolicy``, ``EmbeddingPolicy``, ``KerasPolicy``, and ``SklearnPolicy`` a
 a
...
`Keras <http://keras.io>`_ to select the next action. a
The default architecture is based on an LSTM, but you can override the a
``KerasPolicy.model_architecture`` method to implement your own architecture. a
 a
 a
.. literalinclude:: ../../rasa/core/policies/keras_policy.py a
 ...
 a
and the training is run here: a
 a
.. literalinclude:: ../../rasa/core/policies/keras_policy.py a
   :dedent: 4 a
   :pyobject: KerasPolicy.train a
 a
You can implement the model of your choice by overriding these methods, a
or initialize ``KerasPolicy`` with pre-defined ``keras model``. a
 a
In order to get reproducible training results for the same inputs you can a
set the ``random_seed`` attribute of the ``KerasPolicy`` to any integer. a
 a
 a
.. _embedding_policy: a
 a
...
**Configuration:** a
 ...
 a
            ...
            ``"batch_size": [8, 32]`` (default behavior). If constant ``batch_size`` is required, pass an ``int``, a
            e.g. ``"batch_size": 8``. a
 a
        .. note:: a
 a
...
**Configuration:** a
 ...
 a
       fallback = FallbackPolicy(fallback_action_name="action_default_fallback", a
                                 ...
                                 ambiguity_threshold=0.1) a
 a
       agent = Agent("domain.yml", policies=[KerasPolicy(), fallback]) a
 a
    .. note:: a
 a
