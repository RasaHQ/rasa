...
-------------------------- a
 a
For each response, you can have multiple **response templates** (see :ref:`variations`). a
...
found, it will choose from any response templates that do not have a defined ``channel``. a
...
multiple **response templates** and Rasa will randomly pick one of them, e.g.: a
 a
.. code-block:: yaml a
 a
  responses: a
...
.. note:: a
 a
  ...
  for instance be used to initialize the tracker with slots from an external API a
