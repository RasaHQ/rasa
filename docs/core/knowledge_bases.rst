...
Additionally, user do not only refer to objects by their names, but also use terms, such as "the first one" or "that
restaurant", to refer to a specific object.
Objects need to be recognised and reused at a later point in the conversation.

We recommend that you create a ``ActionQueryKnowledgeBase``.
...
.. code-block:: yaml
    ...
    schema = {
        "hotel": {
            ...
            "key": "name",
            "representation": lambda e: "{} ({})".format(e["name"], e["city"])
        },
    }
...
And ``representation`` is a lamdba function that is used to map the object to a string representation.
Thus function is used whenever an object is output by the bot.
...
Just inherit ``KnowledgeBase`` and implement the methods ``
...
In order to use the ``ActionQueryKnowledgeBase``, you need to perform a couple steps:
