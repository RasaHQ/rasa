...
import Prototyper, { DownloadButton, TrainButton } from '@theme/custom/Prototyper';
import AssistantBuilder from '@theme/custom/AssistantBuilder';
...
[Stories](./stories.mdx) are example conversations that train an assistant to respond correctly depending on what the user has said previously in the conversation. The story format shows the intent of the user message followed by the assistant’s action or response.
...
Check out other docs pages to learn more about [Rasa’s CLI](./command-line-interface.mdx), [Domains](./domain.mdx), [Actions](./actions.mdx), and your config’s [Pipeline](./tuning-your-model.mdx) and [Policies](./policies.mdx).
