...
```yaml-rasa (versioned_docs/version-2.x/sources/data/configs_for_docs/default_spacy_config.yml)
```
...
```yaml-rasa (versioned_docs/version-2.x/sources/data/configs_for_docs/default_config.yml)
```
...
```yaml-rasa (versioned_docs/version-2.x/sources/data/configs_for_docs/config_featurizers.yml)
```
...
To do this, you need to access the `diagnostic_data` field of the [Message](reference/rasa/shared/nlu/training_data/message.md)
and [Prediction](reference/rasa/core/policies/policy.md#predict_action_probabilities) objects, which contain
