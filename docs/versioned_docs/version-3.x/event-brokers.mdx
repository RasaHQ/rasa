...
```yaml-rasa (versioned_docs/version-3.x/sources/data/test_endpoints/event_brokers/pika_endpoint.yml)
```
...
```yaml-rasa (versioned_docs/version-3.x/sources/data/test_endpoints/event_brokers/kafka_sasl_plaintext_endpoint.yml)
```

Using the `PLAINTEXT` protocol the endpoints file must have the following entries:

```yaml-rasa (versioned_docs/version-3.x/sources/data/test_endpoints/event_brokers/kafka_plaintext_endpoint.yml)
```

If using the `SSL` protocol, the endpoints file should look like:

```yaml-rasa (versioned_docs/version-3.x/sources/data/test_endpoints/event_brokers/kafka_ssl_endpoint.yml)
```

If using the `SASL_SSL` protocol, the endpoints file should look like:

```yaml-rasa (versioned_docs/version-3.x/sources/data/test_endpoints/event_brokers/kafka_sasl_ssl_endpoint.yml)
```
