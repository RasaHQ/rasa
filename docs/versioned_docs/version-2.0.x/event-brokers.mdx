...
```yaml-rasa (versioned_docs/version-2.0.x/sources/data/test_endpoints/event_brokers/pika_endpoint.yml)
```
...
It is possible to use [Kafka](https://kafka.apache.org/) as the main broker for your
...
```yaml-rasa (versioned_docs/version-2.0.x/sources/data/test_endpoints/event_brokers/kafka_plaintext_endpoint.yml)
```

If using the `SSL` protocol, the endpoints file should look like:

```yaml-rasa (versioned_docs/version-2.0.x/sources/data/test_endpoints/event_brokers/kafka_ssl_endpoint.yml)
```
