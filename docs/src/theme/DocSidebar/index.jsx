...
function DocSidebar({path, sidebar, sidebarCollapsible = true}) {
  ...
  const {
    siteConfig: {
      themeConfig: { versions, algolia, navbar: {title = '', hideOnScroll = false} = {}} = {},
    } = {},
    isClient,
    ...
    <div
      ...
      <div
        ...
        {algolia ? (
          ...
          />
        ) : (<Ph h="40" bg="beige">search toggle</Ph>)}
        <div style={{marginTop: 20}}>
          <h1>Rasa Open Source</h1>
          <p>Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Nullam id dolor id nibh ultricies vehicula ut id elit.</p>
        </div>
        <NavbarItem {...versions} style={{marginTop: 20}}/>
        {/* <hr style={{ height: 0, borderBottom: '1px solid #ddd'}}/> */}
        <ul className="menu__list" style={{marginTop: 20}}>
          {sidebar.map((item) => (
            <DocSidebarItem
              key={item.label}
              item={item}
