...
prepare-transformers:
	...
	while read -r MODEL; do poetry run python -c 'from huggingface_hub import snapshot_download; snapshot_download(repo_id="$$MODEL", allow_patterns=["*.txt", "*.json", "*.h5", "*.model"])'; done < $$TRANSFORMERS_LIST
