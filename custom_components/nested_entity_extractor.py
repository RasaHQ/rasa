...
class NestedEntityExtractor(EntityExtractor):
  ...
  def train(self, training_data, cfg, **kwargs):
    ...
    self.add_composite_entities_from_entities_synonyms(training_data.entity_synonyms.items())
  ...
  def split_by_lookup_tables(self, composite_child, broad_value):
    ...
    for each_lookup in self.nested_entities['lookup_tables']: # Checking through our list of lookup tables
      if(each_lookup['name'] == composite_child_name ): #if this split composite entity is in the lookup tables
        for lookup_entry in each_lookup['entries']: # kia, honda, toyota
          ...
          if(find_index > -1): # when we find a match of our lookup example in the broad value
            broken_entity[composite_child_name] = lookup_entry # split['make'] = 'kia'
            break
  ...
  def split_by_sys(self, composite_child, broad_value):
    ...
    if(composite_child[0:11] == '@sys.number'):
      ...
      #TODO ['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten', 'eleven', 'twelve'] find the examples of one, twi, three, four, etc
    return broken_entity

  def split_one_level(self, composite_child, broad_value):
    broken_entity = {}
    broken_entity = self.merge_two_dicts(broken_entity, self.split_by_lookup_tables(composite_child, broad_value))
    broken_entity = self.merge_two_dicts(broken_entity, self.split_by_sys(composite_child, broad_value))
    return broken_entity
            ...
            if(composite_child[0] == '@'):
              ...
              for each_nested in self.nested_entities['composite_entries']:
                if(each_nested['value'] == composite_child_name ): # if it is
                  nested_composites = each_nested['synonyms'] # @doors:doors, @seats:seats, @mpg:mpg
                  ...
                  for nested_composite in nested_composites: # Try to find the most related nested entity
                    ...
                    relevance_score = self.get_broad_value_nested_composite_examples_relevance(broad_value, child_synonymns)
                    if(relevance_score > highest_relevance_score):
                      correct_nested_composite = nested_composite_value
                  ...
                  if(highest_relevance_score > 0): 
                    ...
                    for nested_composite_child in correct_nested_composite_examples:
                      if(nested_composite_child[0] == '@'):
                        broken_entity[composite_child_name] = self.merge_two_dicts(broken_entity[composite_child_name], self.split_one_level(nested_composite_child, broad_value))
                    
          each_entity["value"] = { entity : broken_entity }
      ...
      if(value[0] == '@' and synonym.startswith(value)):
        ...
        for composite_index, element in enumerate(self.nested_entities['composite_entries']):
          if element['value'] == value:
            self.nested_entities['composite_entries'][composite_index]['synonyms'].append(synonym)
            is_present = True
        if(is_present == False):
