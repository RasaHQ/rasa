...
- [#6226](https://github.com/rasahq/rasa/issues/6226>): Added more debugging logs to the [Lock Stores](./lock-stores) to simplify debugging in case of
  connection problems.
...
* [#5976](https://github.com/rasahq/rasa/issues/5976): Fix server crashes that occurred when Rasa Open Source pulls a model from a
  [model server](./model-storage#server-fetch-from-server) and an exception was thrown during
...
* [#3765](https://github.com/rasahq/rasa/issues/3765): Add support for entities with roles and grouping of entities in Rasa NLU.
  ...
  For more details see [Entities Roles and Groups](./training-data-format#entities-roles-groups).
  ...
  `from_entity` (see [Forms](./forms)).

  :::note
  Composite entities are currently just supported by the [DIETClassifier](./components#dietclassifier) and [CRFEntityExtractor](./components#crfentityextractor).
...
* [#5465](https://github.com/rasahq/rasa/issues/5465): Update training data format for NLU to support entities with a role or group label.
  ...
  For more information about the new data format see [Training Data Format](./training-data-format).
...
* [#5578](https://github.com/rasahq/rasa/issues/5578): Added `socketio` to the compatible channels for [Reminders and External Events](./reaching-out-to-user).
...
* [#5533](https://github.com/rasahq/rasa/issues/5533): Support for
  ...
  [SQLTrackerStore](./tracker-stores#sqltrackerstore). The `SQLTrackerStore`
...
* [#5492](https://github.com/rasahq/rasa/issues/5492): Fix an issue where the deprecated `queue` parameter for the [Pika Event Broker](./event-brokers#pika-event-broker)
...
* [#5389](https://github.com/rasahq/rasa/issues/5389): Add an optional path to a specific directory to download and cache the pre-trained model weights for [HFTransformersNLP](./components#hftransformersnlp).
...
* [#5258](https://github.com/rasahq/rasa/issues/5258): Added `PikaEventBroker` ([Pika Event Broker](./event-brokers#pika-event-broker)) support for publishing to
...
* [#5446](https://github.com/rasahq/rasa/issues/5446): The endpoint `PUT /conversations/<conversation_id>/tracker/events` no longer
  ...
  see [Session configuration](./domain#session-config)) in addition to the events which were sent in the request
...
* [#5205](https://github.com/rasahq/rasa/issues/5205): Fixed the bug when FormPolicy was overwriting MappingPolicy prediction (e.g. `/restart`).
  Priorities for [Mapping Policy](./policies#mapping-policy) and [Form Policy](./policies#form-policy) are no longer linear:
...
* [#5215](https://github.com/rasahq/rasa/issues/5215): Fixed issue related to storing Python `float` values as `decimal.Decimal` objects
  ...
  Added a new docs section on [DynamoTrackerStore](./tracker-stores#dynamotrackerstore).
...
* [#2237](https://github.com/rasahq/rasa/issues/2237): Restructure the “Evaluating models” documentation page and rename this page to [Testing Your Assistant](./testing-your-assistant).
...
* [#5404](https://github.com/rasahq/rasa/issues/5404): Explain how to run commands as `root` user in Rasa SDK Docker images since version
  ...
  [Building an Action Server Image](./how-to-deploy#building-an-action-server-image).
...
* [#5065](https://github.com/rasahq/rasa/issues/5065): Add [LexicalSyntacticFeaturizer](./components#lexicalsyntacticfeaturizer) to sparse featurizers.
...
* [#5187](https://github.com/rasahq/rasa/issues/5187): Integrate language models from HuggingFace's [Transformers](https://github.com/huggingface/transformers) Library.
  ...
  Add a new NLP component [HFTransformersNLP](./components#hftransformersnlp) which tokenizes and featurizes incoming messages using a specified
  pre-trained model with the Transformers library as the backend.
  Add [LanguageModelTokenizer](./components#languagemodeltokenizer) and [LanguageModelFeaturizer](./components#languagemodelfeaturizer) which use the information from
  [HFTransformersNLP](./components#hftransformersnlp) and sets them correctly for message object.
...
* [#5225](https://github.com/rasahq/rasa/issues/5225): Added a new CLI command `rasa export` to publish tracker events from a persistent
  tracker store using an event broker. See [Export Conversations to an Event Broker](./command-line-interface#export-conversations-to-an-event-broker), [Tracker Stores](./tracker-stores)
  and [Event Brokers](./event-brokers) for more details.

* [#5230](https://github.com/rasahq/rasa/issues/5230): Refactor how GPU and CPU environments are configured for TensorFlow 2.0.

  Please refer to the [documentation](./tuning-your-model#configuring-tensorflow) to understand
...
* [#5266](https://github.com/rasahq/rasa/issues/5266): Added a new NLU component [DIETClassifier](./components#dietclassifier) and a new policy [TEDPolicy](./policies#ted-policy).

  DIET (Dual Intent and Entity Transformer) is a multi-task architecture for intent classification and entity
  recognition. You can read more about this component in our [documentation](./components#dietclassifier).
  The new component will replace the `EmbeddingIntentClassifier` and the
  [CRFEntityExtractor](./components#crfentityextractor) in the future.
  Those two components are deprecated from now on.
  See [migration guide](./migration-guide#migration-to-rasa-1-8) for details on how to
  switch to the new component.

  [TEDPolicy](./policies#ted-policy) is the new name for [EmbeddingPolicy](./policies#embedding-policy).
...
* [#5211](https://github.com/rasahq/rasa/issues/5211): Messages sent to the [Pika Event Broker](./event-brokers#pika-event-broker) are now persisted. This guarantees
...
* [#4601](https://github.com/rasahq/rasa/issues/4601): Fixed issue with `action_restart` getting overridden by `action_listen` when the `MappingPolicy` and the
  [TwoStageFallbackPolicy](./policies#two-stage-fallback-policy) are used together.
...
* [#5197](https://github.com/rasahq/rasa/issues/5197): Fixed incompatibility of Oracle with the [SQLTrackerStore](./tracker-stores#sqltrackerstore), by using a `Sequence`
  for the primary key columns. This does not change anything for SQL databases other than Oracle.
  If you are using Oracle, please create a sequence with the instructions in the [SQLTrackerStore](./tracker-stores#sqltrackerstore) docs.
...
* [#4902](https://github.com/rasahq/rasa/issues/4902): The `EmbeddingPolicy`, `EmbeddingIntentClassifier`, and `ResponseSelector` now by default normalize confidence
  levels over the top 10 results. See [Rasa 1.6 to Rasa 1.7](./migration-guide#migration-to-rasa-1-7) for more details.
...
* [#4957](https://github.com/rasahq/rasa/issues/4957): To [use custom features in the `CRFEntityExtractor`](./components#passing-custom-features-to-crfentityextractor)
...
* [#4935](https://github.com/rasahq/rasa/issues/4935): Preparation for an upcoming change in the `EmbeddingIntentClassifier`:
  ...
  a matrix of size (token-length x feature-dimension). See [Text Featurizers](./components#featurizers).
  ...
  See [passing custom features to the `CRFEntityExtractor`](./components#passing-custom-features-to-crfentityextractor)
  ...
  Read more: [Text Featurizers](./components#featurizers) .
...
* experimental training data importer which supports training with data of multiple
  ...
  [docs](./training-data-importers) for more
  information.
