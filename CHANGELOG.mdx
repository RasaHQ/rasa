...
- [#10989](https://github.com/rasahq/rasa/issues/10989): [NLU training data](./building-classic-assistants/nlu-training-data.mdx) in JSON format is deprecated and will be
...
- [#8868](https://github.com/rasahq/rasa/issues/8868): Follow through on deprecation warnings for the `Domain`. Most importantly this will
  enforce the schema of the [`forms` section](./building-classic-assistants/forms.mdx) in the domain file.
...
- [#9803](https://github.com/rasahq/rasa/issues/9803): Rasa Open Source now uses the [model configuration](./building-classic-assistants/model-configuration.mdx) to build a
...
- [#8459](https://github.com/rasahq/rasa/issues/8459): 1) Failed test stories will display full retrieval intents.
  ...
  steps:
    ...
    - action:
        utter_chitchat/ask_name # predicted: utter_chitchat
...
- [#5657](https://github.com/rasahq/rasa/issues/5657): Add List handling in the `send_custom_json` method on `channels/facebook.py`.
  ...
  Example 2: instead of being a Dict, _blocks_ is a List when there are 2 _type_
  keys under it
...
- [#9045](https://github.com/rasahq/rasa/issues/9045): The option `model_confidence=linear_norm` is deprecated and will be removed in Rasa Open Source `3.0.0`.
  ...
  we also recommend to [re-tune the thresholds for fallback components](./building-classic-assistants/fallback-handoff.mdx#fallbacks).
...
- [#8724](https://github.com/rasahq/rasa/issues/8724): Added `sasl_mechanism` as an optional configurable parameters for the [Kafka Producer](./production/event-brokers.mdx#kafka-event-broker).
- [#8913](https://github.com/rasahq/rasa/issues/8913): Introduces a new policy called [`UnexpecTEDIntentPolicy`](./building-classic-assistants/policies.mdx#unexpected-intent-policy).
  ...
  [`action_unlikely_intent`](./building-assistants/default-actions.mdx#action_unlikely_intent) action.
  ...
  - [`IntentMaxHistoryTrackerFeaturizer`](./building-classic-assistants/policies.mdx#3-intent-max-history)
  ...
  - A new default action called [`action_unlikely_intent`](./building-assistants/default-actions.mdx#action_unlikely_intent).
...
- [#8232](https://github.com/rasahq/rasa/issues/8232): Users don't need to specify an additional buffer size for sparse featurizers anymore during incremental training.
  ...
  This means there's no need to specify `additional_vocabulary_size` for [`CountVectorsFeaturizer`](./building-classic-assistants/components.mdx#countvectorsfeaturizer) or
  `number_additional_patterns` for [`RegexFeaturizer`](./building-classic-assistants/components.mdx#regexfeaturizer). These parameters are now deprecated.
...
- [#8295](https://github.com/rasahq/rasa/issues/8295): Speed up the contradiction check of the [`RulePolicy`](./building-classic-assistants/policies.mdx#rule-policy)
  by a factor of 3.
- [#8801](https://github.com/rasahq/rasa/issues/8801): Change the confidence score assigned by [`FallbackClassifier`](./building-classic-assistants/components.mdx#fallbackclassifier) to fallback intent to be the same as the fallback threshold.
...
- [#8364](https://github.com/rasahq/rasa/issues/8364): Fixed a bug where [`ListSlot`](./building-assistants/domain.mdx#list-slot)s were filled with single items
  in case only one matching entity was extracted for this slot.

  Values applied to [`ListSlot`](./building-assistants/domain.mdx#list-slot)s will be converted to a `List`
...
- [#261](https://github.com/rasahq/rasa/issues/261): Added an optional `ignored_intents` parameter in forms.
  ...
  - To use it, add the `ignored_intents` parameter in your `domain.yml` file after the forms name and provide a list of intents to ignore. Please see [Forms](./building-classic-assistants/forms.mdx) for more information.
...
- [#8892](https://github.com/rasahq/rasa/issues/8892): Added `sasl_mechanism` as an optional configurable parameters for the [Kafka Producer](./production/event-brokers.mdx#kafka-event-broker).
...
- [#7869](https://github.com/rasahq/rasa/issues/7869): Upgraded Rasa to be compatible with spaCy 3.0.
  ...
  **Fallback**
  ...
  correct spaCy model. We've also added a warning to the [SpaCyNLP](./building-classic-assistants/components.mdx#spacynlp)
...
- [#8335](https://github.com/rasahq/rasa/issues/8335): Added partition_by_sender flag to [Kafka Producer](./production/event-brokers.mdx#kafka-event-broker) to optionally associate events with Kafka partition based on sender_id.
...
- [#8258](https://github.com/rasahq/rasa/issues/8258): Fixed the bug that events from previous conversation sessions would be re-saved in the [`SQLTrackerStore`](./production/tracker-stores.mdx#sqltrackerstore) or [`MongoTrackerStore`](./production/tracker-stores.mdx#mongotrackerstore) when `retrieve_events_from_previous_conversation_sessions` was true.
...
- [#6484](https://github.com/rasahq/rasa/issues/6484): NLG Server
  ...
  - The terminology `template` is deprecated and replaced by `response`. Support for `template` from the NLG response will be removed in Rasa Open Source 3.0.0. Please see [here](./production/nlg.mdx) for more details.
  ...
  - The field `template_name` will be deprecated in Rasa Open Source 3.0.0. Please use `utter_action` instead. Please see [here](./building-classic-assistants/components.mdx#selectors) for more details.
  - The field `response_templates` will be deprecated in Rasa Open Source 3.0.0. Please use `responses` instead. Please see [here](./building-classic-assistants/components.mdx#selectors) for more details.
...
- [#8100](https://github.com/rasahq/rasa/issues/8100): Improved the [lock store](./production/lock-stores.mdx) debug log message when the process has to
...
- [#8860](https://github.com/rasahq/rasa/issues/8860): Added `sasl_mechanism` as an optional configurable parameters for the [Kafka Producer](./production/event-brokers.mdx#kafka-event-broker).
...
- [#8014](https://github.com/rasahq/rasa/issues/8014): Setting `model_confidence=cosine` in `DIETClassifier`, `ResponseSelector` and `TEDPolicy` is deprecated and will no longer be available. This was introduced in Rasa Open Source version `2.3.0` but post-release experiments suggest that using cosine similarity as model's confidences can change the ranking of predicted labels which is wrong.
  ...
  This should ease up [tuning fallback thresholds](./building-classic-assistants/fallback-handoff.mdx#fallbacks) as confidences for wrong predictions are better distributed across the range `[0, 1]`.
  ...
  `model_confidence=cosine` is removed from the configuration generated by [auto-configuration](./building-classic-assistants/model-configuration.mdx#suggested-config).
...
- [#7420](https://github.com/rasahq/rasa/issues/7420): User message metadata can now be accessed via the default slot
  ...
  [custom `action_session_start`](./building-assistants/default-actions.mdx#customization).
...
- [#7616](https://github.com/rasahq/rasa/issues/7616): Added two new parameters `constrain_similarities` and `model_confidence` to machine learning (ML) components - [DIETClassifier](./building-classic-assistants/components.mdx#dietclassifier), [ResponseSelector](./building-classic-assistants/components.mdx#dietclassifier) and [TEDPolicy](./building-classic-assistants/policies.mdx#ted-policy).
  ...
  The default [auto-configuration](./building-classic-assistants/model-configuration.mdx#suggested-config) is changed to use `constrain_similarities=True` and `model_confidence=cosine` in ML components so that new users start with the recommended configuration.
...
- [#7848](https://github.com/rasahq/rasa/issues/7848): Added support to configure `exchange_name` for [pika event broker](./production/event-brokers.mdx#pika-event-broker).
...
- [#7764](https://github.com/rasahq/rasa/issues/7764): Fixes a bug in [forms](./building-classic-assistants/forms.mdx) where the next slot asked was not consistent after returning to a form from an unhappy path.
...
- [#7520](https://github.com/rasahq/rasa/issues/7520): Improve the warning in case the [RulePolicy](./building-classic-assistants/policies.mdx#rule-policy) or the deprecated
...
- [#7684](https://github.com/rasahq/rasa/issues/7684): Fixed a problem when using the `POST /model/test/intents` endpoint together with a
  [model server](./production/model-storage.mdx#load-model-from-server). The error looked as follows:
...
- [#7557](https://github.com/rasahq/rasa/issues/7557): Fixed a problem where a [form](./building-classic-assistants/forms.mdx) wouldn't reject when the
  `FormValidationAction` re-implemented `required_slots`.
- [#7585](https://github.com/rasahq/rasa/issues/7585): Fixed an error when using the [SQLTrackerStore](./production/tracker-stores.mdx#sqltrackerstore)
...
- [#6971](https://github.com/rasahq/rasa/issues/6971): Incremental training of models in a pipeline is now supported.
  ...
  training](./building-assistants/command-line-interface.mdx#incremental-training).
  ...
  and `number_additional_patterns` to [`RegexFeaturizer`](./building-classic-assistants/components.mdx#regexfeaturizer).
...
- [#7496](https://github.com/rasahq/rasa/issues/7496): Make [TED Policy](./building-classic-assistants/policies.mdx#ted-policy) an end-to-end policy. Namely, make it possible to train TED on stories that contain
...
- [#7136](https://github.com/rasahq/rasa/issues/7136): The [`Policy`](./building-classic-assistants/policies.mdx) interface was changed to return a `PolicyPrediction` object when
...
- [#7263](https://github.com/rasahq/rasa/issues/7263): The [Pika Event Broker](./production/event-brokers.mdx#pika-event-broker) no longer supports
  ...
  The method [`Event Broker.close`](./production/event-brokers.mdx) was changed to be asynchronous.
...
- [#7136](https://github.com/rasahq/rasa/issues/7136): [Policies](./building-classic-assistants/policies.mdx) can now return obligatory and optional events as part of their
...
- [#5715](https://github.com/rasahq/rasa/issues/5715): If you are using the feature [Entity Roles and Groups](./building-classic-assistants/nlu-training-data.mdx#entities-roles-and-groups), you should now also list the roles and groups
  ...
  [Entity Roles and Groups influencing dialogue predictions](./building-classic-assistants/nlu-training-data.mdx#entity-roles-and-groups-influencing-dialogue-predictions).
...
- [#6474](https://github.com/rasahq/rasa/issues/6474): Adapt the training data reader and emulator for wit.ai to their latest format.
  ...
  [migrate from wit.ai documentation](./building-classic-assistants/migrate-from/facebook-wit-ai-to-rasa.mdx)
...
- [#6860](https://github.com/rasahq/rasa/issues/6860): Fetching test stories from the HTTP API endpoint
  ...
  of the
  [conversation session](./building-assistants/domain.mdx#session-configuration).
  ...
  kwarg for the [Tracker Store](./production/tracker-stores.mdx) class is deprecated and will be
...
- [#6966](https://github.com/rasahq/rasa/issues/6966): Added warning for when an option is provided for a [component](./building-classic-assistants/components.mdx) that is not listed as a key in the defaults for that component.
- [#6977](https://github.com/rasahq/rasa/issues/6977): [Forms](./building-classic-assistants/forms.mdx) no longer reject their execution before a potential custom
...
- [#7068](https://github.com/rasahq/rasa/issues/7068): Slot mappings for [Forms](./building-classic-assistants/forms.mdx) in the domain are now optional. If you do not
  provide any slot mappings as part of the domain, you need to provide
  [custom slot mappings](./building-classic-assistants/forms.mdx#custom-slot-mappings) through a custom action.
  ...
  The action for [forms](./building-classic-assistants/forms.mdx) can now be overridden by defining a custom action
...
- [#7122](https://github.com/rasahq/rasa/issues/7122): Add validations for [slot mappings](./building-classic-assistants/forms.mdx#slot-mappings).
...
- [#7132](https://github.com/rasahq/rasa/issues/7132): Adapt the training data reader and emulator for LUIS to
  ...
  Update the instructions in the
  ["Migrate from LUIS" documentation page](./building-classic-assistants/migrate-from/microsoft-luis-to-rasa.mdx)
  to reflect the recent changes made to the UI of LUIS.
- [#7160](https://github.com/rasahq/rasa/issues/7160): Adapt the [training data reader and emulator for DialogFlow](./building-classic-assistants/migrate-from/google-dialogflow-to-rasa.mdx) to
...
- [#7263](https://github.com/rasahq/rasa/issues/7263): The [Pika Event Broker](./production/event-brokers.mdx#pika-event-broker) was reimplemented with
...
- [#7278](https://github.com/rasahq/rasa/issues/7278): The confidence of the [`FallbackClassifier`](./building-classic-assistants/components.mdx#fallbackclassifier)
...
- [#7176](https://github.com/rasahq/rasa/issues/7176): Fixed a bug in the [`TwoStageFallback`](./building-classic-assistants/fallback-handoff.mdx#two-stage-fallback) action which
...
- [#7033](https://github.com/rasahq/rasa/issues/7033): - Fix `YAMLStoryReader` not being able to represent [`OR` statements](./building-classic-assistants/stories.mdx#or-statements) in conversion mode.
...
- [#6453](https://github.com/rasahq/rasa/issues/6453): Removed support for `queue` argument in `PikaEventBroker` (use `queues` instead).
  ...
  Removed several config options for [TED Policy](./building-classic-assistants/policies.mdx#ted-policy), [DIETClassifier](./building-classic-assistants/components.mdx#dietclassifier) and [ResponseSelector](./building-classic-assistants/components.mdx#responseselector):
...
- [#6809](https://github.com/rasahq/rasa/issues/6809): [Slots](./building-assistants/domain.mdx#slots) of type `unfeaturized` are
...
- [#6934](https://github.com/rasahq/rasa/issues/6934): [Conversation sessions](./building-assistants/domain.mdx#session-configuration) are now enabled by default
  if your [Domain](./building-assistants/domain.mdx) does not contain a session configuration.
...
- [#6952](https://github.com/rasahq/rasa/issues/6952): Using the [default action](./building-assistants/default-actions.mdx) `action_deactivate_form` to deactivate
  the currently active loop / [Form](./building-classic-assistants/forms.mdx) is deprecated.
...
- [#5510](https://github.com/rasahq/rasa/issues/5510): You can now define what kind of features should be used by what component
  (see [Choosing a Pipeline](./building-classic-assistants/tuning-your-model.mdx)).
  ...
  [DIETClassifier](./building-classic-assistants/components.mdx#dietclassifier) what features from which
...
- [#5957](https://github.com/rasahq/rasa/issues/5957): Add new entity extractor `RegexEntityExtractor`. The entity extractor extracts entities using the lookup tables
  and regexes defined in the training data. For more information see [RegexEntityExtractor](./building-classic-assistants/components.mdx#regexentityextractor).
...
- [#6065](https://github.com/rasahq/rasa/issues/6065): Add support for [rules](./building-classic-assistants/rules.mdx) data and [forms](./building-classic-assistants/forms.mdx) in YAML
  format.
- [#6066](https://github.com/rasahq/rasa/issues/6066): The NLU `interpreter` is now passed to the [Policies](./building-classic-assistants/policies.mdx) during training and
...
- [#6088](https://github.com/rasahq/rasa/issues/6088): Added the new dialogue policy RulePolicy which will replace the old “rule-like”
  ...
  [rules documentation](./building-classic-assistants/rules.mdx) for more information.

  Added new NLU component [FallbackClassifier](./building-classic-assistants/components.mdx#fallbackclassifier)
...
- [#5368](https://github.com/rasahq/rasa/issues/5368): Changed the project structure for Rasa projects initialized with the
  [CLI](./building-assistants/command-line-interface.mdx) (using the `rasa init` command):
...
- [#5805](https://github.com/rasahq/rasa/issues/5805): The debug-level logs when instantiating an
  [SQLTrackerStore](./production/tracker-stores.mdx#sqltrackerstore)
...
- [#6323](https://github.com/rasahq/rasa/issues/6323): [Response selector templates](./building-classic-assistants/components.mdx#responseselector) now support all features that
...
- [#6809](https://github.com/rasahq/rasa/issues/6809): [Slots](./building-assistants/domain.mdx#slots) of any type can now be ignored during a conversation.
  ...
  [documentation for slots](./building-assistants/domain.mdx#slots) for more information.

  A new slot type [`any`](./building-assistants/domain.mdx#any-slot) was added. Slots of this type can store
...
- [#5737](https://github.com/rasahq/rasa/issues/5737): Pinned the library version for the Azure
  [Cloud Storage](./production/model-storage.mdx#load-model-from-cloud) to 2.1.0 since the
...
- [#6535](https://github.com/rasahq/rasa/issues/6535): [Forms](./building-classic-assistants/forms.mdx) with slot mappings defined in `domain.yml` must now be a
...
- [#6819](https://github.com/rasahq/rasa/issues/6819): Update ["Validating Form Input" section](./building-classic-assistants/forms.mdx#validating-form-input) to include details about
...
- [#8876](https://github.com/rasahq/rasa/issues/8876): Added `sasl_mechanism` as an optional configurable parameter for the [Kafka Producer](./production/event-brokers.mdx#kafka-event-broker).
...
- [#8345](https://github.com/rasahq/rasa/issues/8345): Improved the [lock store](./production/lock-stores.mdx) debug log message when the process has to queue because other messages have to be processed before this item.
...
- [#6226](https://github.com/rasahq/rasa/issues/6226): Added more debugging logs to the [Lock Stores](./production/lock-stores.mdx) to simplify debugging in case of
...
- [#5976](https://github.com/rasahq/rasa/issues/5976): Fix server crashes that occurred when Rasa Open Source pulls a model from a
  [model server](./production/model-storage.mdx#load-model-from-server) and an exception was thrown during
...
- [#5758](https://github.com/rasahq/rasa/issues/5758): Fixed a bug in [`rasa export`](./building-assistants/command-line-interface.mdx#rasa-export)) which caused Rasa Open Source to only migrate conversation events from the last [Session configuration](./building-assistants/domain.mdx#session-configuration).
...
- [#3765](https://github.com/rasahq/rasa/issues/3765): Add support for entities with roles and grouping of entities in Rasa NLU.
  ...
  For more details see [Entities Roles and Groups](./building-classic-assistants/nlu-training-data.mdx#entities-roles-and-groups).
  ...
  `from_entity` (see [Forms](./building-classic-assistants/forms.mdx)).

  :::note
  Composite entities are currently just supported by the [DIETClassifier](./building-classic-assistants/components.mdx#dietclassifier) and [CRFEntityExtractor](./building-classic-assistants/components.mdx#crfentityextractor).
...
- [#5465](https://github.com/rasahq/rasa/issues/5465): Update training data format for NLU to support entities with a role or group label.
  ...
  For more information about the new data format see [Training Data Format](./building-classic-assistants/training-data-format.mdx).
...
- [#5578](https://github.com/rasahq/rasa/issues/5578): Added `socketio` to the compatible channels for [Reminders and External Events](./building-classic-assistants/reaching-out-to-user.mdx).
...
- [#5533](https://github.com/rasahq/rasa/issues/5533): Support for
  ...
  [SQLTrackerStore](./production/tracker-stores.mdx#sqltrackerstore). The `SQLTrackerStore`
...
- [#5492](https://github.com/rasahq/rasa/issues/5492): Fix an issue where the deprecated `queue` parameter for the [Pika Event Broker](./production/event-brokers.mdx#pika-event-broker)
...
- [#5258](https://github.com/rasahq/rasa/issues/5258): Added `PikaEventBroker` ([Pika Event Broker](./production/event-brokers.mdx#pika-event-broker)) support for publishing to
...
- [#5446](https://github.com/rasahq/rasa/issues/5446): The endpoint `PUT /conversations/<conversation_id>/tracker/events` no longer
  ...
  see [Session configuration](./building-assistants/domain.mdx#session-configuration)) in addition to the events which were sent in the request
...
- [#5215](https://github.com/rasahq/rasa/issues/5215): Fixed issue related to storing Python `float` values as `decimal.Decimal` objects
  ...
  Added a new docs section on [DynamoTrackerStore](./production/tracker-stores.mdx#dynamotrackerstore).
...
- [#5065](https://github.com/rasahq/rasa/issues/5065): Add [LexicalSyntacticFeaturizer](./building-classic-assistants/components.mdx#lexicalsyntacticfeaturizer) to sparse featurizers.
...
- [#5187](https://github.com/rasahq/rasa/issues/5187): Integrate language models from HuggingFace's [Transformers](https://github.com/huggingface/transformers) Library.
  ...
  Add [LanguageModelTokenizer](https://rasa.com/docs/rasa/2.x/components#languagemodeltokenizer) and [LanguageModelFeaturizer](./building-classic-assistants/components.mdx#languagemodelfeaturizer) which use the information from
...
- [#5225](https://github.com/rasahq/rasa/issues/5225): Added a new CLI command `rasa export` to publish tracker events from a persistent
  tracker store using an event broker. See [Export Conversations to an Event Broker](./building-assistants/command-line-interface.mdx#rasa-export), [Tracker Stores](./production/tracker-stores.mdx)
...
- [#5230](https://github.com/rasahq/rasa/issues/5230): Refactor how GPU and CPU environments are configured for TensorFlow 2.0.
  ...
  Please refer to the documentation on [Configuring TensorFlow](./building-classic-assistants/tuning-your-model.mdx#configuring-tensorflow) to understand
...
- [#5266](https://github.com/rasahq/rasa/issues/5266): Added a new NLU component [DIETClassifier](./building-classic-assistants/components.mdx#dietclassifier) and a new policy [TEDPolicy](./building-classic-assistants/policies.mdx#ted-policy).

  DIET (Dual Intent and Entity Transformer) is a multi-task architecture for intent classification and entity
  recognition. You can read more about this component in the [DIETClassifier](./building-classic-assistants/components.mdx#dietclassifier) documentation.
  The new component will replace the `EmbeddingIntentClassifier` and the
  [CRFEntityExtractor](./building-classic-assistants/components.mdx#crfentityextractor) in the future.
  ...
  [TEDPolicy](./building-classic-assistants/policies.mdx#ted-policy) is the new name for EmbeddingPolicy.
...
- [#5211](https://github.com/rasahq/rasa/issues/5211): Messages sent to the [Pika Event Broker](./production/event-brokers.mdx#pika-event-broker) are now persisted. This guarantees
...
- [#5197](https://github.com/rasahq/rasa/issues/5197): Fixed incompatibility of Oracle with the [SQLTrackerStore](./production/tracker-stores.mdx#sqltrackerstore), by using a `Sequence`
  for the primary key columns. This does not change anything for SQL databases other than Oracle.
  If you are using Oracle, please create a sequence with the instructions in the [SQLTrackerStore](./production/tracker-stores.mdx#sqltrackerstore) docs.
...
- [#4957](https://github.com/rasahq/rasa/issues/4957): To [use custom features in the `CRFEntityExtractor`](./building-classic-assistants/components.mdx#crfentityextractor)
...
- [#4935](https://github.com/rasahq/rasa/issues/4935): Preparation for an upcoming change in the `EmbeddingIntentClassifier`:
  ...
  a matrix of size (token-length x feature-dimension). See [Text Featurizers](./building-classic-assistants/components.mdx#featurizers).
  ...
  See [passing custom features to the `CRFEntityExtractor`](./building-classic-assistants/components.mdx#crfentityextractor)
  ...
  Read more: [Text Featurizers](./building-classic-assistants/components.mdx#featurizers) .
...
- experimental training data importer which supports training with data of multiple
  ...
  [docs](./building-assistants/training-data-importers.mdx) for more
  information.
