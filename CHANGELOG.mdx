...
- [#7616](https://github.com/rasahq/rasa/issues/7616): Added two new parameters `constrain_similarities` and `model_confidence` to machine learning (ML) components - [DIETClassifier](components.mdx#dietclassifier), [ResponseSelector](components.mdx#dietclassifier) and [TEDPolicy](policies.mdx#ted-policy).
  ...
  **EDIT**: From some post-release experiments we found that using `model_confidence=cosine` is wrong as it can change the order of predicted labels. Hence this option has been removed from Rasa Open Source version `2.3.3`. You can read more about the change [here](CHANGELOG.mdx#233---2021-02-11).
