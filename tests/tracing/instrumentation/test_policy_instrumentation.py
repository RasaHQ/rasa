...
from rasa.tracing.constants import ENABLE_TRACING_DEBUGGING_ENV_VAR_NAME
...
def test_tracing_flow_policy_prediction_sanitizes_action_metadata(
    tracer_provider: TracerProvider,
    span_exporter: InMemorySpanExporter,
    previous_num_captured_spans: int,
    default_model_storage: ModelStorage,
    ...
    assert captured_span.name == "FlowPolicy._prediction"
    ...
    expected_attributes = {
        ...
        "is_no_user_prediction": False,
        "diagnostic_data": "{}",
        "action_metadata": '{"message": {"agent_name": "FinX Travel Agent"}}',
    }

    assert captured_span.attributes == expected_attributes


def test_tracing_policy_prediction_debugging_enabled(
    tracer_provider: TracerProvider,
    span_exporter: InMemorySpanExporter,
    previous_num_captured_spans: int,
    default_model_storage: ModelStorage,
    ...
    monkeypatch.setenv(ENABLE_TRACING_DEBUGGING_ENV_VAR_NAME, "true")
    component_class = FlowPolicy
    ...
    expected_attributes = {
        ...
        "flights, FinX has got you covered. "
        "No need to switch between multiple apps \\u2013 it's "
        'all right here at your fingertips.", "agent_name": "FinX Travel Agent"}}',
    }
