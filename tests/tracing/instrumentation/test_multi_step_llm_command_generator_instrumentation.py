...
async def test_tracing_multi_step_llm_command_generator_default_attrs(
    ...
    await mock_multi_step_llm_command_generator.invoke_llm(
...
async def test_tracing_multi_step_llm_command_generator_prompt_tokens(
    ...
    """OpenAI prompt token length on the span when using non–model-group LLM config."""
    component_class = MockMultiStepLLMCommandGenerator
...
@pytest.mark.usefixtures("mock_configuration")
async def test_tracing_multi_step_llm_command_generator_prompt_tokens_llm_model_group_router_openai(  # noqa: E501
    default_model_storage: ModelStorage,
    tracer_provider: TracerProvider,
    span_exporter: InMemorySpanExporter,
    previous_num_captured_spans: int,
...
@pytest.mark.usefixtures("mock_configuration")
async def test_tracing_multi_step_llm_command_generator_prompt_tokens_router_group_no_openai(  # noqa: E501
    default_model_storage: ModelStorage,
    tracer_provider: TracerProvider,
    span_exporter: InMemorySpanExporter,
    previous_num_captured_spans: int,
...
async def test_tracing_multi_step_llm_command_generator_training_and_inference_health_checks(  # noqa: E501
    default_model_storage: ModelStorage,
    tracer_provider: TracerProvider,
    span_exporter: InMemorySpanExporter,
    previous_num_captured_spans: int,
    ...
    multi_step_llm_command_generator = component_class(
        config=config,
        model_storage=default_model_storage,
        resource=Resource("llm-command-generator"),
    )
    resource = multi_step_llm_command_generator.train(
        training_data=Mock(),
        flows=flows,
        domain=domain,
    )
    ...
    span_training_llm_health_check = next(
        ...
        if span.name == "MockMultiStepLLMCommandGenerator.perform_llm_health_check"
