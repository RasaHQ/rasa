...
async def test_multi_node_graph_run(eager: bool, default_model_storage: ModelStorage):
    ...
    results = await runner.run(inputs={"first_input": 3, "second_input": 4})
    assert results["subtract_2"] == 5


@pytest.mark.parametrize("eager", [True, False])
async def test_target_override(eager: bool, default_model_storage: ModelStorage):
    graph_schema = GraphSchema(
        {
            "add": SchemaNode(
                needs={"i1": "first_input", "i2": "second_input"},
    ...
    results = await runner.run(
        inputs={"first_input": 3, "second_input": 4}, targets=["add"]
    )
...
async def test_default_config(
    ...
    results = await runner.run(inputs={"input": 5})
    assert results["subtract"] == output


async def test_empty_schema(default_model_storage: ModelStorage):
...
async def test_no_inputs(default_model_storage: ModelStorage):
    graph_schema = GraphSchema(
        {
            "provide": SchemaNode(
                needs={},
...
async def test_no_target(default_model_storage: ModelStorage):
    graph_schema = GraphSchema(
        {
            "provide": SchemaNode(
                needs={},
...
async def test_unused_node(default_model_storage: ModelStorage):
    graph_schema = GraphSchema(
        {
            "provide": SchemaNode(
                needs={},
    ...
    results = await runner.run(inputs={"input": "some_other_value"})
    assert results == {"provide": 1}
...
async def test_execution_context(default_model_storage: ModelStorage):
    ...
    result = (await runner.run())["execution_context_aware"]
    assert result.model_id == "some_id"
    assert result.node_name == "execution_context_aware"


async def test_input_value_is_node_name(default_model_storage: ModelStorage):
    graph_schema = GraphSchema(
        {
            "provide": SchemaNode(
                needs={},
    ...
    with pytest.raises(GraphRunError):
        await runner.run(inputs={"input": "provide"})


async def test_loading_from_previous_node(default_model_storage: ModelStorage):
