...
@pytest.fixture(scope="module")
def template_sender_tracker(domain_path):
...
async def test_remote_action_utterances_with_none_values(
    default_channel, default_tracker, domain
):
    endpoint = EndpointConfig("https://example.com/webhooks/actions")
    remote_action = action.RemoteAction("my_action", endpoint)
...
async def test_response(default_channel, default_nlg, default_tracker, domain):
...
async def test_response_channel_specific(default_nlg, default_tracker, domain):
...
async def test_action_default_ask_affirmation(default_channel, default_nlg, domain):
    initial_events = [
        ActionExecuted(ACTION_LISTEN_NAME),
        # User triggers a restart manually by triggering the intent
        UserUttered(
