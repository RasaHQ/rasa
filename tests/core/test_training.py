...
@pytest.mark.parametrize(
    "config_file_names, fingerprint_file_names, save_fingerprints_calls_count",
    [
        (["config.yml"], ["fingerprint.json"], 1),
        (
            ["config.yml", "config2.yml"],
            ["config_fingerprint.json", "config2_fingerprint.json"],
            2,
        ),
        (["config.yml", "config.yml"], ["fingerprint.json"], 1),
        ([], [], 0),
    ],
)
async def test_save_fingerprint(
    ...
    config_file_names: List[Text],
    fingerprint_file_names: List[Text],
    save_fingerprints_calls_count: int,
):
    import rasa.model
    ...
    assert persist_fingerprint.call_count == save_fingerprints_calls_count
    for i in range(save_fingerprints_calls_count):
