...
def test_hangouts_channel():
    ...
    assert routes_list.get("hangouts_webhook.receive").startswith("/webhooks/hangouts/webhook")
...
def test_hangouts_extract_functions():
    ...
    def create_req(app):
      return Request(b"http://127.0.0.1:42101/webhooks/hangouts/webhook", None, None, "POST", None, app=app)
...
async def test_hangouts_output_channel_functions():
    ...
    await output_channel.send_attachment(recipient_id="Chuck Norris", attachment="Attachment")

    assert len(output_channel.messages) == 1
    # two text messages are appended with space inbetween
    assert output_channel.messages["text"] == "Test: Attachment"

    await output_channel.send_quick_replies(recipient_id="Chuck Norris",
                                            text="Test passing?",
                                            quick_replies=[{"title": "Yes", "payload":"/confirm"}, 
                                                          {"title": "No", "payload":"/deny"}])
    assert len(output_channel.messages) == 1
    # for text and cards, text is turned to card format and two cards are returned 
    assert output_channel.messages["cards"][1]["sections"][0]["widgets"][0]["textParagraph"]["text"] == "Test passing?"
    assert output_channel.messages["cards"][1]["sections"][0]["widgets"][1]["buttons"][0]["textButton"]["onClick"]["action"]["actionMethodName"] == "/confirm"

    await output_channel.send_image_url(recipient_id="Chuck Norris",
                                        image="test.png")
    assert len(output_channel.messages) == 1
    assert output_channel.messages["cards"][2]["sections"][0]["widgets"][0]["image"]["imageUrl"] == "test.png"
