...
def test_spacy_featurizer_cls_vector(spacy_nlp):
    ...
    assert 5 == len(seq_vecs.features)
    assert 1 == len(sen_vecs.features)
    assert np.allclose(seq_vecs.features[0][:5], expected, atol=1e-5)
    assert np.allclose(sen_vecs.features[-1][:5], expected_cls, atol=1e-5)
