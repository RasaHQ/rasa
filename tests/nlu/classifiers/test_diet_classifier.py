...
async def test_adjusting_layers_incremental_training(
    ...
    def test_signatures(
        new_data_signature: Dict[Text, Dict[Text, List[FeatureArray]]],
        old_data_signature: Dict[Text, Dict[Text, List[FeatureArray]]],
    ):
        ...
        for attribute, signatures in new_data_signature.items():
            ...
            for feature_type, feature_signatures in signatures.items():
                ...
                if feature_type == "sequence_lengths":
                    ...
                    assert feature_signatures == old_data_signature.get(attribute).get(
                        feature_type
                    )
                else:
                    for index, feature_signature in enumerate(feature_signatures):
                        if (
                            ...
                            assert (
                                ...
                                == old_data_signature.get(attribute)
                                .get(feature_type)[index]
                                .units
                            )

    test_signatures(new_data_signature, old_data_signature)
    test_signatures(new_predict_data_signature, old_predict_data_signature)
