...
RASA_EXECUTABLE = os.environ.get("RASA_EXECUTABLE", "rasa")


@pytest.fixture
def run(testdir: Testdir) -> Callable[..., RunResult]:
    def do_run(*args):
        args = [shutil.which(RASA_EXECUTABLE)] + list(args)
...
def trained_simple_project(tmpdir_factory: TempdirFactory) -> Text:
    ...
    check_call([shutil.which(RASA_EXECUTABLE), "train"], cwd=path.strpath)
