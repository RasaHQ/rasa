...
def test_label_tokenizer_featurizer_handles_on_non_existing_features():
    ...
    f.num_features = (len(f.user_vocab) +
                      len(f.slot_labels) +
                      len(f.bot_vocab))
    encoded = f.encode({"a_d": 1.0, "c_b": 0.0, "e": 1.0, "prev_action_listen": 1.0})
...
def test_label_tokenizer_featurizer_uses_correct_dtype_int():
    ...
    f.num_features = (len(f.user_vocab) +
                      len(f.slot_labels) +
                      len(f.bot_vocab))
    encoded = f.encode({"a_d": 1.0, "c_b": 0.0, "prev_action_listen": 1.0})
    assert encoded.dtype == np.int32
...
def test_label_tokenizer_featurizer_uses_correct_dtype_float():
    ...
    f.num_features = (len(f.user_vocab) +
                      len(f.slot_labels) +
                      len(f.bot_vocab))
    encoded = f.encode({"a_d": 0.2, "c_b": 0.0, "prev_action_listen": 1.0})
    assert encoded.dtype == np.float64
...
def test_label_tokenizer_featurizer_handles_on_non_existing_probabilistic():
    ...
    f.num_features = (len(f.user_vocab) +
                      len(f.slot_labels) +
                      len(f.bot_vocab))
    encoded = f.encode({"a_d": 0.2, "c_b": 1.0, "c": 0.0, "e": 1.0,
...
def test_label_tokenizer_featurizer_handles_probabilistic_intents():
    ...
    f.num_features = (len(f.user_vocab) +
                      len(f.slot_labels) +
                      len(f.bot_vocab))
    encoded = f.encode({"intent_a": 0.5, "b": 0.2, "intent_d": 1.0,
                        "prev_action_listen": 1.0})
