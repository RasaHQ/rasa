...
PATH_PYTEST_MARKER_MAPPINGS = {
    ...
    "category_anonymization": [
        Path("tests", "anonymization").absolute(),
        Path("tests", "privacy").absolute(),
    ],
    "category_builder_tests": [Path("tests", "builder").absolute()],
...
def calm_domain_yaml():
    with open("data/upload/calm/domain/domain.yml", "r") as f:
        return f.read()


@pytest.fixture(scope="session")
def calm_flows_yaml():
    with open("data/upload/calm/data/flows.yml", "r") as f:
        return f.read()
